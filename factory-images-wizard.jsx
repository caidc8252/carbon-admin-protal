...
const IsvAppPicker = ({ apps, selectedRefs, onToggle, onChangeVersion }) => {
  const [q, setQ] = React.useState('');          // draft (what's typed)
  const [appliedQ, setAppliedQ] = React.useState(''); // applied (drives filter)
  const [showSelectedOnly, setShowSelectedOnly] = React.useState(false);
  const [page, setPage] = React.useState(1);
  const [pageSize, setPageSize] = React.useState(25);

  const runSearch = () => { setAppliedQ(q.trim()); setPage(1); };
  const pending = q.trim() !== appliedQ;   // typed text not yet applied

  const selectedIds = React.useMemo(() => new Set(selectedRefs.map(r => r.appId)), [selectedRefs]);

  const filtered = React.useMemo(() => {
    return apps.filter(a => {
      if ((a.versions || []).filter(v => v.status === 'published').length === 0) return false;
  ...
  }, [apps, appliedQ, showSelectedOnly, selectedIds]);
  ...
  return (
    <div className="fw-isv">
      <div className="fw-isv__toolbar">
        <Input prefix={<Icon name="search" size={13}/>} placeholder="Search by app name, package, or publisher…"
          value={q}
        ...
        <label className={`fw-isv__chip ${showSelectedOnly ? 'is-on' : ''}`}>
          <input type="checkbox" checked={showSelectedOnly} onChange={e => { setShowSelectedOnly(e.target.checked); setPage(1); }}/>
          Selected only
      ...
      <div className="fw-isv__list">
        {filtered.length === 0 ? (
          <div className="empty" style={{ padding: 18, fontSize: 12 }}>
            {showSelectedOnly && selectedIds.size === 0 ? 'No apps selected yet.' : 'No apps match your search.'}
          </div>
        ) : pageRows.map(app => {
      ...
      {filtered.length > 0 && (
        <window.Pagination
          ...
          totalUnfiltered={apps.filter(a => (a.versions || []).filter(v => v.status === 'published').length > 0).length}
  ...
  return (
    <div className="fw-mbc">
        ...
        <div className="fw-mbc__body">
          ...
          <div className="fw-mbc__section">
            <div className="fw-mbc__section-head">
              <span>Firmware</span>
              {fwForModel.length === 0 && (
                <span className="muted" style={{ fontSize: 12, fontWeight: 400 }}>No firmware available</span>
              )}
            </div>
            {fwForModel.length === 0 ? (
              <div className="muted" style={{ padding: '8px 0', fontSize: 12 }}>No firmware available for {modelCode}.</div>
            ) : (
                ...
                return (
                  <div key={app.id} className={`fw-app-card ${currentRef ? 'is-on' : ''}`}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 500, fontSize: 12 }}>{app.name}</div>
                        <div className="muted" style={{ fontSize: 12, fontFamily: 'var(--font-family-mono)' }}>{app.package || app.pkg}</div>
                      </div>
                    </label>
          ...
          <div className="fw-mbc__section">
            ...
            {isvApps.length === 0 ? (
              <div className="muted" style={{ padding: '8px 0', fontSize: 12 }}>No published ISV apps.</div>
            ) : (
              <IsvAppPicker
                apps={isvApps}
      ...
      <style>{`
        ...
        .fw-mbc__model {
          font-family: var(--font-family-mono); font-weight: 600; font-size: 14px;
          color: var(--color-text-primary);
        ...
        .fw-fw-pick__opt.is-on {
          border-color: oklch(58% 0.15 262);
          background: oklch(96% 0.03 262);
          box-shadow: 0 0 0 1px oklch(58% 0.15 262);
        }
        [data-theme="dark"] .fw-fw-pick__opt.is-on {
          border-color: oklch(64% 0.15 262);
          background: oklch(30% 0.06 262);
          box-shadow: 0 0 0 1px oklch(64% 0.15 262);
        }
        ...
        [data-theme="dark"] .fw-app-card.is-on {
          border-color: oklch(64% 0.15 262);
          background: oklch(30% 0.06 262);
          box-shadow: 0 0 0 1px oklch(64% 0.15 262);
        }
