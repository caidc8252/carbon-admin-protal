...
const CONTRACT_KIND_TO_LEGACY = {
  ADMIN:       'Admin',
  ISO:         'ISO',
  'ISO-PILOT': 'ISO',          // revised model: pilot is a type; legacy UI sees kind 'ISO'
  ISV:         'ISV',
  'ISV-PILOT': 'ISV',          // ditto for the ISV pilot track
  MERCHANT:    'Merchant',
};
const CUSTOMER_CONTRACT_KINDS    = ['ISV', 'ISO', 'Merchant'];     // 3 customer kinds (ADMIN excluded — unique platform contract)
...
function getEntityView(entityId) {
  ...
  const contractRows = (_CONTRACTS_BY_ENTITY[entityId] || []).map(c => {
    ...
    // Revised model: PILOT is a CONTRACT TYPE ('ISO-PILOT' / 'ISV-PILOT'),
    // not a status. Each contract type is independent (no pilot↔active
    // linkage). We surface the real type as `kind` and the real persisted
    // status (ACTIVE/SUSPENDED/TERMINATED) — pilot-ness is derived from the
    // type via `isPilot`, never from status.
    const isPilot = window.isPilotContractType
      ? window.isPilotContractType(c.authorizedContractType)
      : /-PILOT$/.test(String(c.authorizedContractType || '').toUpperCase());
    return {
      id: c.id,
      kind: c.authorizedContractType,           // real type, e.g. 'ISO-PILOT'
      contractType: c.authorizedContractType,   // raw new-model type (may be *-PILOT)
      isPilot,                                   // derived: true for *-PILOT types
      pilotType: isPilot ? c.authorizedContractType : null,
      status: c.status,                          // real status: ACTIVE/SUSPENDED/TERMINATED
      rawStatus: c.status,                       // persisted: ACTIVE/SUSPENDED/TERMINATED
      authorizingEntityId: c.authorizingEntityId,
      authorizingEntityName: authEntity?.name || c.authorizingEntityId,
      authorizingContractType: c.authorizingPartnerContractType || null,
  ...
  const operators = eurs.map(eur => {
    ...
    return {
      ...
      disabled: user.status === 'DISABLED',     // revised model: admin-deactivated account
      userStatus: user.status,                  // raw USER status (ACTIVE/LOCKED/DISABLED)
      invitedAt: eur.authorizingFrom,
      activatedAt: eur.authorizingTimestamp,
      invitedBy: eur.authorizingUserName,
      // MFA INFO status is now a string enum ('ENABLED' | 'PENDING').
      mfaEnabled: !!user.mfaEnable && !!mfa && mfa.status === 'ENABLED',
    };
  }).filter(Boolean);
  ...
  const hasUsableOperator = operators.some(op => !op.locked && !op.disabled);
  const status = entity.status
    ? (String(entity.status).toLowerCase() === 'active' ? 'Active' : 'Onboarding')
    : (hasUsableOperator ? 'Active' : 'Onboarding');

  // Primary email (revised model): EMAIL is now a persisted field on the
  // PARTNER/entity row. Prefer it; fall back to the Admin operator's email,
  // then the first pending-invite recipient, then '' (empty).
  const adminOp     = operators.find(op => op.role === 'Admin');
  const derivedEmail = entity.email || adminOp?.email || pendingInvite?.recipientEmail || '';
    ...
    .map(u => {
      ...
      return {
        ...
        status: u.status === 'LOCKED'
          ...
          : (u.status === 'DISABLED'
              ? 'DISABLED'
              : (eur.status === 'LOCKED' ? 'LOCKED' : 'ACTIVE')),
