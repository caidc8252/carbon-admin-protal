...
const ModelTile = ({ model, px }) => {
  ...
  const hue = h % 360;
  return (
    <span className="dm-tile dm-tile--placeholder" style={{ width: px, height: px, '--dm-h': hue, fontSize }}>
      {model.name}
    </span>
  );
};
...
function exportDeviceModelsCsv(rows, filename) {
  const header = ['Model', 'Family', 'Description', 'OS', 'Orientation', 'Primary screen', 'Secondary screen'];
  const lines = [header.map(_dmCsvCell).join(',')];
  rows.forEach((m) => {
    lines.push([
      m.name, m.family, m.desc, m.os, m.orientation,
      `${m.primary.size} ${m.primary.resolution}`,
      m.secondary ? `${m.secondary.size} ${m.secondary.resolution}` : '',
    ].map(_dmCsvCell).join(','));
  });
...
const DeviceModelList = ({ models, onNew, onEdit, onDelete }) => {
  ...
  // Stat-card shortcut: set draft AND immediately apply (treated as a one-click search).
  // clearOne writes both draft + applied for the given key in a single update.
  const applyOsShortcut = (next) => { clearOne('os', next); setPage(1); };
  ...
  return (
    <div className="page">
      ...
      <window.StatGrid cols={4}>
        <window.StatCard label="All models" value={stats.total} description="in catalog"
          selected={applied.os === 'All'} onClick={() => applyOsShortcut('All')} />
        <window.StatCard label="Android" value={stats.android} variant="success" description="smart terminals"
          selected={applied.os === 'ANDROID'} onClick={() => applyOsShortcut('ANDROID')} />
        <window.StatCard label="Linux" value={stats.linux} description="embedded terminals"
          selected={applied.os === 'LINUX'} onClick={() => applyOsShortcut('LINUX')} />
        <window.StatCard label="RTOS" value={stats.rtos} variant="warning" description="unattended / kiosk"
          selected={applied.os === 'RTOS'} onClick={() => applyOsShortcut('RTOS')} />
      </window.StatGrid>
      ...
      <div ref={toolbarRef} className="list-toolbar-wrap">
        <SearchBar onSearch={runSearch} sticky={false}>
          <SearchInput
            ...
            onSearch={runSearch}
            placeholder="Search by model, family or description…"/>
          <SearchSelect
            value={draft.os}
        ...
        <div className="dm-drop">
          <div className="dm-drop__has">
            ...
            <div className="dm-drop__has-main">
              <div style={{ fontWeight: 600, fontSize: 14 }}>Image saved</div>
      ...
      <div className="wizard-grid">
          ...
          <div className="info-card">
            ...
            <div className="info-card__body">
              <div className="dm-screen">
                ...
                <div className="form-grid form-grid--2">
                  <Field label="Size" required>
                    <Input value={m.primary.size} onChange={(e) => updatePrimary('size', e.target.value)} placeholder='e.g. 5.5"' suffix={<span style={{ fontSize: 12 }}>inch</span>}/>
                  </Field>
                  <Field label="Resolution" required>
                    <Input value={m.primary.resolution} onChange={(e) => updatePrimary('resolution', e.target.value)} placeholder="720 × 1280" suffix={<span style={{ fontSize: 12 }}>px</span>}/>
                  </Field>
                </div>
              </div>
              ...
              <div className="dm-toggle-row">
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>Has secondary screen</div>
              ...
              {hasSecondary && m.secondary && (
                <div className="dm-screen dm-screen--alt">
                  ...
                  <div className="form-grid form-grid--2">
                    <Field label="Size" required>
                      <Input value={m.secondary.size} onChange={(e) => updateSecondary('size', e.target.value)} placeholder='e.g. 4.3"' suffix={<span style={{ fontSize: 12 }}>inch</span>}/>
                    </Field>
                    <Field label="Resolution" required>
                      <Input value={m.secondary.resolution} onChange={(e) => updateSecondary('resolution', e.target.value)} placeholder="800 × 480" suffix={<span style={{ fontSize: 12 }}>px</span>}/>
                    </Field>
                  </div>
                </div>
              )}
        ...
        <aside className="wizard-aside">
          ...
          <div style={{ textAlign: 'center', marginBottom: 14 }}>
            <div style={{ fontSize: 16, fontWeight: 600, letterSpacing: '-0.01em' }}>{m.name || 'Model name'}</div>
...
// ─── Styles ────────────────────────────────────────────────
const dmStyles = `
.dm-tile { display: grid; place-items: center; border-radius: 8px; overflow: hidden; flex: none; font-weight: 700; letter-spacing: -0.02em; font-family: var(--font-family-sans); background-image:
...
.dm-tile--placeholder { background-image: none; background: oklch(96% 0.03 var(--dm-h)); color: oklch(38% 0.08 var(--dm-h)); }
[data-theme="dark"] .dm-tile--placeholder { background: oklch(30% 0.035 var(--dm-h)); color: oklch(80% 0.06 var(--dm-h)); }
.dm-tile--check { border: 1px solid var(--color-border-default); }

.dm-orient { display: inline-flex; align-items: center; gap: 6px; font-size: 14px; color: var(--color-text-primary); }
...
.dm-drop__title { font-size: 14px; font-weight: 500; color: var(--color-text-primary); }
...
.dm-err { display: flex; gap: 8px; align-items: flex-start; padding: 10px 12px; background: var(--color-error-50); border: 1px solid oklch(70% 0.16 25 / 0.3); border-radius: 8px; font-size: 12px; color: var(--color-error-700); }
.dm-err svg { flex: none; margin-top: 2px; }
.dm-notice { display: flex; gap: 8px; align-items: flex-start; padding: 10px 12px; background: var(--color-success-50); border: 1px solid oklch(58% 0.14 152 / 0.25); border-radius: 8px; font-size: 12px; color: var(--color-success-700); }
...
.dm-rules__title { font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-tertiary); margin-bottom: 8px; }
.dm-rules ul { margin: 0; padding-left: 18px; font-size: 12px; color: var(--color-text-secondary); line-height: 1.6; }
...
.dm-size { background: var(--color-bg-3); border: 1px solid var(--color-border-subtle); border-radius: 8px; padding: 14px 12px; text-align: center; }
...
.dm-size__use { font-size: 12px; color: var(--color-text-tertiary); margin-top: 2px; }
...
.dm-seg__btn { padding: 7px 16px; border-radius: 6px; font-size: 14px; font-weight: 500; color: var(--color-text-secondary); background: transparent; border: 0; cursor: pointer; transition: all var(--duration-fast); display: inline-flex; align-items: center; gap: 6px; font-family: inherit; }
...
.dm-orientpick__btn { display: flex; flex-direction: column; gap: 4px; padding: 14px 16px; background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 8px; cursor: pointer; text-align: left; font-family: inherit; transition: all var(--duration-fast); align-items: flex-start; }
...
.dm-orientpick__lbl { font-size: 14px; font-weight: 600; color: var(--color-text-primary); }
.dm-orientpick__sub { font-size: 12px; color: var(--color-text-tertiary); }

.dm-screen { background: var(--color-bg-3); border: 1px solid var(--color-border-subtle); border-radius: 8px; padding: 14px 16px 8px; margin-bottom: 14px; }
...
.dm-screen__tag { font: 600 10px/1 var(--font-family-sans); padding: 4px 8px; border-radius: 999px; background: var(--color-bg-2); color: var(--color-text-primary); border: 1px solid var(--color-border-default); letter-spacing: 0.06em; text-transform: uppercase; }
.dm-screen__tag--alt { background: oklch(60% 0.14 230 / 0.18); color: var(--color-info-700); border-color: oklch(60% 0.14 230 / 0.3); }
.dm-screen__hint { font-size: 12px; color: var(--color-text-tertiary); }

.dm-toggle-row { display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 12px 14px; background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 8px; margin: 4px 0 14px; }

.dm-checks { display: flex; gap: 8px; flex-wrap: wrap; }
.dm-check { display: inline-flex; align-items: center; gap: 6px; padding: 7px 12px; border-radius: 999px; background: var(--color-bg-3); border: 1px solid var(--color-border-default); font-size: 12px; color: var(--color-text-secondary); cursor: pointer; transition: all var(--duration-fast); }
...
.dm-frame { width: 280px; height: 280px; position: relative; background: #fff;
  ...
  border-radius: 8px; overflow: hidden; cursor: grab; user-select: none; touch-action: none; box-shadow: inset 0 0 0 1px var(--color-border-default); }
...
.dm-frame__overlay { position: absolute; inset: 0; pointer-events: none; box-shadow: inset 0 0 0 2px var(--color-primary-700); border-radius: 8px; }
...
.dm-zoom__lbl { font-size: 12px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-text-tertiary); width: 50px; flex: none; }
.dm-zoom input { flex: 1; accent-color: var(--color-primary-700); height: 6px; }
.dm-zoom__val { font: 500 12px var(--font-family-mono); color: var(--color-text-primary); min-width: 48px; text-align: right; font-variant-numeric: tabular-nums; }

.dm-livepreview { background: var(--color-bg-3); border: 1px solid var(--color-border-subtle); border-radius: 8px; padding: 12px 14px; }
.dm-livepreview__lbl { font-size: 12px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-text-tertiary); margin-bottom: 10px; }
...
.dm-livepreview__sub { font: 500 12px var(--font-family-mono); color: var(--color-text-tertiary); margin-top: 10px; font-variant-numeric: tabular-nums; }
