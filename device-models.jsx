...
// ════════════════════════════════════════════════════════════
// Device Models — strict model (per business spec):
//   id · modelCode · os · logos[] · attributes[] · description
// No extra columns (no vendor / status / orientation / resolution / price / family).
...
// OS Version presets — per the model spec the OS VERSION attribute only ships a
// fixed set of options (Android 10/12/14/16). LINUX / RTOS have no spec-defined
// preset versions, so they offer no presets (operators hand-type via "other").
...
function exportDeviceModelsCsv(rows, filename) {
  const header = ['Model code', 'OS', 'Attributes', 'Description'];
  const lines = [header.map(_dmCsvCell).join(',')];
  rows.forEach((m) => {
    lines.push([
      m.modelCode, m.os, attrSummaryText(m), m.description,
    ].map(_dmCsvCell).join(','));
  });
...
const DeviceModelList = ({ models, onNew, onEdit, onOpenFirmware, onDelete, deleteBlockReason }) => {
  ...
  return (
    <div className="page page--list">
      <window.TitleBar
        title="Device Models"
        subtitle="The hardware catalog. Each model defines its OS, configurable attributes and the artwork shown across the admin, customer portal and packing slips."
      ...
      <div ref={toolbarRef} className="list-toolbar-wrap">
        <SearchBar onSearch={runSearch} sticky={false}>
          <SearchInput
            ...
            onSearch={runSearch}
            placeholder="Search by model code or description…"/>
          <SearchSelect
            value={draft.os}
              ...
              return (
                <tr key={m.id} onClick={() => onEdit(m.id)} style={{ cursor: 'pointer' }}>
                  ...
                  <td>
                    ...
                    <div style={{ marginTop: 4 }}><Badge tone={OS_TONE[m.os] || 'neutral'} dot>{m.os}</Badge></div>
                  </td>
                  <td>
                    {chips.length === 0 ? (
  ...
  return (
    <div className="page">
      <window.TitleBar
        ...
        subtitle="Maintain the OS, configurable attributes and artwork for this hardware model."
        actions={
...
Object.assign(window, {
  ...
  ModelTile, ATTRIBUTE_CATALOG, attrSummaryChips, attrSummaryText,
});
