...
// =====================================================================
const AdminUsers = ({ users: usersProp, setUsers: setUsersProp, currentUserId }) => {
  ...
  // Deleted users (status === 'DELETE') are a terminal, hidden state: they
  // never appear in the list or any filter segment. The underlying row is
  // kept in `users` so audit / relationship references stay attributable.
  const liveUsers = useMemoAU(() => users.filter(u => u.status !== 'DELETE'), [users]);
  ...
  }, [liveUsers, query, statusFilter]);

  // Re-sync selection — also advances off a user that was just deleted
  // (it drops out of liveUsers), so the detail pane never shows a DELETE row.
  useEffAU(() => {
    if (!liveUsers.find(u => u.id === selectedId) && liveUsers.length) {
      setSelectedId(liveUsers[0].id);
    }
  }, [liveUsers, selectedId]);

  const selected = liveUsers.find(u => u.id === selectedId) || filtered[0];
  ...
  const stats = useMemoAU(() => ({
    ...
    active: liveUsers.filter(u => u.status === 'ACTIVE').length,
    locked: liveUsers.filter(u => u.status === 'LOCKED').length,
    pending: liveUsers.filter(u => u.status === 'PENDING').length,
  }), [liveUsers]);
  ...
  // Soft-delete an activated user (ACTIVE / LOCKED). The row's status flips
  // to the terminal 'DELETE' state: sign-in is refused and it can't be
  // reactivated. Guardrails (no self, no root admin) are enforced in the
  // detail pane, which is the only place that calls this.
  const deleteUser = (user) => {
    setUsers(us => us.map(u => u.id === user.id
      ? { ...u, status: 'DELETE', updatedAt: new Date().toISOString() }
      : u));
    toast({
      kind: 'success',
      title: 'User deleted',
      msg: `${user.displayName || user.email} removed · sign-in refused. Record retained for audit.`,
    });
  };
  ...
  return (
    <div className="page" data-screen-label="Users">
      <window.TitleBar
        title="Users"
        subtitle={<>Carbon platform staff accounts. Roles are picked from <strong>System → Roles</strong>.</>}
        actions={<Btn variant="primary" size="sm" icon="plus" onClick={() => setShowNew(true)}>New user</Btn>}
      />
...
const UserDetail = ({ user, users = [], currentUserId, onSave, onResetPassword, onToggleLock, onDelete }) => {
  ...
  const [confirmDelete, setConfirmDelete] = useStateAU(false);
  ...
  // Delete guardrails: you can't delete yourself, and the root administrator
  // (u-1) can never be deleted. The button stays visible but disabled so the
  // restriction is discoverable.
  const isSelf = user.id === currentUserId;
  const isRoot = user.id === 'u-1';
  ...
  return (
    <section className="pu-detail">
      <header className="pu-detail__head">
        ...
        <div className="pu-detail__actions">
          ...
          <span title={deleteHint} style={{ display: 'inline-flex' }}>
            <Btn variant="ghost" size="sm" icon="trash" disabled={!canDelete} title={deleteHint}
                 onClick={() => setConfirmDelete(true)}>Delete</Btn>
          </span>
      ...
      <Modal open={confirmDelete} onClose={() => setConfirmDelete(false)}
        title="Delete user?" width={440}
        footer={<>
          <Btn variant="ghost" size="sm" onClick={() => setConfirmDelete(false)}>Cancel</Btn>
          <Btn variant="danger" size="sm" icon="trash"
               onClick={() => { setConfirmDelete(false); onDelete(); }}>Delete user</Btn>
        </>}>
        <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.55 }}>
          Permanently remove <strong>{user.displayName}</strong> from the platform.
        </p>
        <ul style={{ margin: '12px 0 0', paddingLeft: 18, fontSize: 13, lineHeight: 1.7, color: 'var(--color-text-secondary)' }}>
          <li>The account can no longer sign in.</li>
          <li>This is <strong>permanent</strong> — a deleted user cannot be reactivated.</li>
          <li>The record is retained for audit; role and company history stay attributable.</li>
        </ul>
      </Modal>
    </section>
  );
