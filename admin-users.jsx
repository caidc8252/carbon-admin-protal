  ...
  return (
    <div className="page" data-screen-label="Users">
      ...
      <window.StatGrid cols={4}>
        <window.StatCard label="Total" value={stats.total} />
        <window.StatCard label="Active" value={stats.active} variant="success" />
        <window.StatCard label="Pending" value={stats.pending} variant={stats.pending ? 'warning' : 'default'} />
        <window.StatCard label="Locked" value={stats.locked} variant={stats.locked ? 'error' : 'default'} />
      </window.StatGrid>
      ...
      <div className="pu-detail__body">
        ...
        <section className="info-card">
          ...
          <div className="info-card__body">
            ...
            <dl className="kv2">
              <div className="kv2__row"><dt>Email</dt><dd>{user.email}</dd></div>
              <div className="kv2__row"><dt>Invite token</dt><dd><code style={{ fontFamily: 'var(--font-family-mono)', fontSize: 12 }}>{user.inviteToken || user.id}</code></dd></div>
              <div className="kv2__row"><dt>Invited by</dt><dd>{user.invitedBy}</dd></div>
              <div className="kv2__row"><dt>Sent</dt><dd>{user.invitedAt ? fmtDateTime(user.invitedAt) : '—'}</dd></div>
              <div className="kv2__row"><dt>Expires</dt><dd>{user.inviteExpiresAt ? fmtDateTime(user.inviteExpiresAt) : '—'}</dd></div>
              {user.remark && <div className="kv2__row kv2__row--full"><dt>Remark</dt><dd>{user.remark}</dd></div>}
            </dl>
            </div>
          </div>
        </section>
  ...
  return (
    <section className="pu-detail">
              ...
              {(user.passwordHistory || []).map((h, i) => {
                ...
                return (
                  <div key={h.id} className="pu-history__row">
                    ...
                    <code className="muted" style={{ fontSize: 12 }}>#{h.id}</code>
                  </div>
                );
              })}
            </div>
      ...
      <Modal open={confirmReset} onClose={() => setConfirmReset(false)} title="Send password reset link?" width={460}
        ...
        <ul style={{ margin: '12px 0 0', paddingLeft: 18, fontSize: 14, lineHeight: 1.7, color: 'var(--color-text-secondary)' }}>
...
const PasswordResetRecord = ({ user }) => {
  ...
  if (!latest) {
    return (
      <div className="empty" style={{ padding: '24px 16px', fontSize: 14 }}>
              ...
              return (
                <label key={r.id}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--color-text-tertiary)' }}>{r.description} · {r.permissions.length} perms</div>
                  </div>
                </label>
              );
            })}
