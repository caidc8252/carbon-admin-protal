/* global React, Btn, Badge, Input, Icon, Modal, CompanyLogo, fmtDate, maskEmail, maskPhone */

// ─── Terminal-drawer context ────────────────────────
// MerchantViewReadonly opens the device-detail screen in a right-side
// drawer instead of full-page navigation so the operator never loses
// the merchant context. TerminalSnLink consumes this context — if a
// handler is present, the drawer opens; otherwise we fall back to the
// classic global navigation (used outside the merchant detail view).
const MerchantTerminalCtx = React.createContext(null);
// Exposed so other modules (customer-detail) can provide their own
// openTerminal handler when rendering MerchantStoresTab outside of the
// legacy MerchantViewReadonly screen.
window.MerchantTerminalCtx = MerchantTerminalCtx;

// ─── Relative-time label (e.g. "12 min ago") for a timestamp string ──
// `ts` is a "YYYY-MM-DD HH:MM:SS" or any string Date can parse. Returns
...
// ─── Two-line cell: timestamp on top (mono), relative beneath (muted) ─
const LastSeenCell = ({ ts }) => {
  if (!ts) return <span className="muted" style={{ fontSize: 12 }}>—</span>;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 1, lineHeight: 1.25 }}>
      <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 11.5, color: 'var(--color-text-primary)', whiteSpace: 'nowrap' }}>{ts}</span>
      <span className="muted" style={{ fontSize: 10.5 }}>{_relTime(ts)}</span>
    </div>
  );
};
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
const { useState: useStateCM, useMemo: useMemoCM, useEffect: useEffectCM } = React;

// ─── PII masking helpers (admin portal) ────────────────────
// merchant.mid is a payment-processor identifier — mask the middle.
// merchant.stores[].address is a physical address — mask the street part
// and keep the city/state so it's still recognizable.
const maskMid = (m) => {
  if (!m) return '';
  const s = String(m);
...
const maskAddress = (a) => {
  ...
  // Replace the most identifying part (street line) with bullets;
  // keep city / state / country so the row keeps geographic context.
  parts[0] = '••• ••• •••';
  return parts.join(', ');
};
...
const TerminalSnLink = ({ sn }) => {
  const ctx = React.useContext(MerchantTerminalCtx);
  if (!sn) return <span style={{ fontStyle: "italic", color: "var(--color-text-tertiary)" }}>— not bound</span>;
  const handleClick = (e) => {
    e.stopPropagation();
    if (ctx && ctx.openTerminal) ctx.openTerminal(sn);
    else openDeviceDetail(sn);
  };
  return (
...
// ─── Terminal drawer ─ slide-in panel that hosts DeviceDetailScreen ──
// Width: ~88% of viewport, full height. Backdrop closes on click or Esc.
// We re-mount when `sn` changes so DeviceDetailScreen resets its internal
// tab state — this is what the comment meant by "data loads in the
// popup". A loading shimmer is shown until findDeviceBySn returns.
const TerminalDrawer = ({ sn, onClose, maskOn = false }) => {
  const [tab, setTab] = useStateCM('basic');
  const [device, setDevice] = useStateCM(null);
  const [loading, setLoading] = useStateCM(true);

  // Lookup is synchronous in the seed but we still gate it behind a tick
  // so the drawer can paint its frame first — visual cue that data is
  // being fetched, matching the comment’s expectation of live loading.
  useEffectCM(() => {
    if (!sn) return;
    setLoading(true);
    setDevice(null);
    ...
    const handle = setTimeout(() => {
      // The devices module is part of the lazy fleet chunk — make sure it
      // is in before looking the SN up (no-op once loaded / prefetched).
      const ready = window.loadFleetModules ? window.loadFleetModules() : Promise.resolve();
      ready.catch(() => {}).then(() => {
        if (cancelled) return;
        setDevice(window.findDeviceBySn ? window.findDeviceBySn(sn) : null);
        setLoading(false);
      });
    }, 180);
  ...
  // Synthetic route + nested navigation. Tab clicks inside the embedded
  // DeviceDetailScreen rewrite our local `tab` state instead of mutating
  // the host app’s route — the drawer is self-contained.
  const detailRoute = { screen: 'deviceDetail', deviceSn: sn, tab };
  const drawerNavigate = (target) => {
    if (!target) return;
    // Tab change inside DeviceDetailScreen ("basic" → "apps" etc.)
    if (target.screen === 'deviceDetail' && target.deviceSn === sn) {
      if (target.tab) setTab(target.tab);
      return;
    }
    // Anything else — leave merchant context entirely; close drawer
    // and delegate to the host navigator.
    onClose();
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          animation: 'cmDrawerFadeIn 160ms ease-out',
        }} />
      {/* Panel */}
      <div
        role="dialog"
        ...
        style={{
          ...
          boxShadow: '-12px 0 32px -8px oklch(0% 0 0 / 0.18)',
          display: 'flex', flexDirection: 'column',
          animation: 'cmDrawerSlideIn 220ms cubic-bezier(.16,.84,.30,1)',
        }}>
        {/* Header strip — sits ABOVE the embedded screen */}
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            <span style={{ fontSize: 11, color: 'var(--color-text-tertiary)', letterSpacing: '0.04em', textTransform: 'uppercase', fontWeight: 600 }}>Terminal</span>
            <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 13, fontWeight: 500, color: 'var(--color-text-primary)' }}>{sn}</span>
          </div>
          <div style={{ flex: 1 }} />
          <button
            type="button"
            ...
            style={{
              display: 'grid', placeItems: 'center',
              width: 32, height: 32, padding: 0, borderRadius: 6, border: 0,
              background: 'transparent', cursor: 'pointer',
        ...
        {/* Body — hosts DeviceDetailScreen, scrolls independently */}
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto', background: 'var(--color-bg-1)' }}>
          {loading ? (
            <DrawerLoadingState sn={sn} />
          ) : !device ? (
            <div className="empty" style={{ padding: '60px 24px', textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>
                Terminal <span style={{ fontFamily: 'var(--font-family-mono)' }}>{sn}</span> not found in the fleet.
              </div>
            </div>
            ...
            <div className="empty" style={{ padding: '60px 24px', textAlign: 'center' }}>
              Device detail module not loaded.
            </div>
          )}
        </div>
      </div>
...
// ─── Lightweight skeleton for the brief load tick ────────────
const DrawerLoadingState = ({ sn }) => (
  <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <div className="cm-skel" style={{ width: 56, height: 56, borderRadius: 12 }} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
        <div className="cm-skel" style={{ width: '40%', height: 18, borderRadius: 4 }} />
        <div className="cm-skel" style={{ width: '24%', height: 12, borderRadius: 4 }} />
      </div>
    </div>
    <div className="cm-skel" style={{ width: '100%', height: 1 }} />
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div className="cm-skel" style={{ width: '40%', height: 10, borderRadius: 3 }} />
          <div className="cm-skel" style={{ width: '85%', height: 14, borderRadius: 4 }} />
        </div>
      ))}
    </div>
    <div style={{ marginTop: 6, fontSize: 12, color: 'var(--color-text-tertiary)', textAlign: 'center' }}>
      Loading terminal {sn}…
    </div>
  </div>
);
  ...
  return (
    <div className="info-card">
      ...
      <table className="tds-table">
          <thead>
            <tr>
              <th style={{ width: "30%" }}>Serial (SN)</th>
              <th style={{ width: "26%" }}>VarSheet (TID)</th>
              ...
              <tbody>
                {offTarget.map(t => (
                  <tr key={t.sn} style={{ cursor: "default" }}>
                    ...
                    <td><LastSeenCell ts={t.lastSeen} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const MerchantViewReadonly = ({ merchant, customerName, onBack, maskOn = false }) => {
  const [tab, setTab] = useStateCM("stores");
  const [revealed, setRevealed] = useStateCM({});
  const [drawerSn, setDrawerSn] = useStateCM(null);
  const toggleReveal = (k) => setRevealed((r) => ({ ...r, [k]: !r[k] }));
  const terminalCtx = useMemoCM(() => ({
    openTerminal: (sn) => {
      // Prefer full-page navigation when the host provides it — opens the
      // device under Devices > Device detail and the back-arrow returns
      // here with this merchant's tab + scroll state preserved.
      if (window.__openTerminalFromMerchant) {
        window.__openTerminalFromMerchant(sn, merchant.id);
      } else {
        setDrawerSn(sn);
      }
    },
  ...
  // ─── Per-field masking (header) ──────────────────────────
  // Only email + phone are partially masked. MID, address and everything
  // else are always shown in full.
  const showPhone  = !maskOn || revealed.phone;
  const showEmail  = !maskOn || revealed.email;
  const rawPhone   = merchant.phone ? `${merchant.phoneCountryCode || ''} ${merchant.phone}` : '';
  const dispMid    = merchant.mid;
  const dispAddr   = merchant.stores?.[0]?.address;
  const dispPhone  = showPhone ? rawPhone                           : maskPhone(rawPhone);
  const dispEmail  = showEmail ? merchant.email                     : maskEmail(merchant.email);
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <span className="muted" style={{ fontSize: 12, fontFamily: "var(--font-family-mono)" }}>
                · <span style={{ fontSize: 12 }}>{dispMid}</span>
              </span>
            ...
            <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", rowGap: 6, columnGap: 18, fontSize: 12.5, color: "var(--color-text-secondary)" }}>
              ...
              {merchant.phone &&
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 11, color: "var(--color-text-tertiary)", fontWeight: 500 }}>TEL</span>
                  <span className="op-mask">{dispPhone}</span>
                  {maskOn && (
                    <button className="reveal-btn" onClick={() => toggleReveal('phone')}>{revealed.phone ? 'Hide' : 'Reveal'}</button>
                  )}
                </span>
              }
              {merchant.email &&
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span className="op-mask">{dispEmail}</span>
                  {maskOn && (
                    <button className="reveal-btn" onClick={() => toggleReveal('email')}>{revealed.email ? 'Hide' : 'Reveal'}</button>
                  )}
                </span>
              }
      ...
      {tab === "stores" && (
        <MerchantTerminalCtx.Provider value={terminalCtx}>
          <MerchantStoresTab merchant={merchant} />
        </MerchantTerminalCtx.Provider>
      )}
      {tab === "apps" && (
        <MerchantTerminalCtx.Provider value={terminalCtx}>
          <MerchantAppsTabRO merchant={merchant} />
        </MerchantTerminalCtx.Provider>
      )}

      {drawerSn && (
        <TerminalDrawer sn={drawerSn} maskOn={maskOn} onClose={() => setDrawerSn(null)} />
      )}
    </div>);
...
// ─── Tab entry-point ───────────────────────────────────────
const TabCustomerMerchants = ({ customer }) => {
  // Customer Detail → Merchants tab (post-merge data source).
  //
  // Lists every non-terminated MERCHANT contract whose authorizingEntityId
  // is this customer. Each row is the downstream merchant Entity + its
  // optional legacy MERCHANTS bridge (for stores/terminals counts). Click
  // → navigate to that merchant's standard customer detail via
  // window.__navigate — the old inline MerchantViewReadonly drill-in has
  // been retired.
  const rows = useMemoCM(() => {
    const ecs = window.SEED_ENTITY_CONTRACTS || [];
    const entities = window.SEED_ENTITIES || [];
    const customers = window.SEED_CUSTOMERS || [];
    const byId = new Map();
    [...entities, ...customers].forEach((e) => { if (e && !byId.has(e.id)) byId.set(e.id, e); });
    return ecs
      .filter((c) =>
        c.authorizingEntityId === customer.id &&
        c.authorizedContractType === 'MERCHANT' &&
        String(c.status || '').toUpperCase() !== 'TERMINATED'
      )
      .map((c) => {
        const entity = byId.get(c.authorizedEntityId);
        const bridge = window.findMerchantByCustomerId
          ? window.findMerchantByCustomerId(c.authorizedEntityId)
          : null;
        return { contract: c, entity, bridge };
      })
      .filter((r) => r.entity);
  }, [customer.id]);

  const openMerchant = (row) => {
    if (window.__navigate) {
      window.__navigate({ screen: 'customerDetail', customerId: row.entity.id });
    }
  };

  const [q, setQ] = useStateCM('');
  const [aQ, setAQ] = useStateCM('');
  const [page, setPage] = useStateCM(1);
  const [pageSize, setPageSize] = useStateCM(25);
  const toolbarRef = React.useRef(null);
  const runSearch = () => { setAQ(q.trim()); setPage(1); };

  const _searchable = (r) =>
    `${r.entity?.name || ''} ${r.entity?.country || ''} ${r.bridge?.mid || ''}`.toLowerCase();
  const filtered = aQ
    ? rows.filter((r) => _searchable(r).includes(aQ.toLowerCase()))
    : rows;
    ...
    return (
      <div className="empty" style={{ padding: '60px 24px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 10, fontSize: 14, fontWeight: 500, color: 'var(--color-text-primary)' }}>
          No downstream merchants yet
        </div>
        <div style={{ fontSize: 12.5, color: 'var(--color-text-tertiary)', marginTop: 4 }}>
          When this ISO authorizes a MERCHANT contract, that merchant will appear here.
        </div>
      </div>
    );
  }
      ...
      <div ref={toolbarRef}>
        <window.SearchBar onSearch={runSearch} sticky={false}>
          <window.SearchInput value={q} onChange={setQ} onSearch={runSearch}
            placeholder="Search merchant name, country, MID" width={320} />
        </window.SearchBar>
      </div>

      <window.ListCard toolbarRef={toolbarRef}>
        <window.ListCardHead count={filtered.length} unit={`merchant${filtered.length === 1 ? '' : 's'}`} hasFilters={!!aQ} />
        <table className="tds-table">
          <thead>
            <tr>
          ...
          <tbody>
            {filtered.length === 0 ?
              <tr><td colSpan="6"><div className="empty" style={{ padding: '32px 20px', textAlign: 'center' }}>No merchants match your search.</div></td></tr> :
              pageRows.map((r) => {
                const name = r.entity.name;
                const status = String(r.contract.status || '').toUpperCase();
                const statusTone = status === 'ACTIVE' ? 'success' : status === 'SUSPENDED' ? 'warning' : 'neutral';
                return (
                  <tr key={r.contract.id} onClick={() => openMerchant(r)} style={{ cursor: 'pointer' }}>
                    <td>
                      <div className="al-app">
                        <MerchantMonogram name={name} size={32} />
                        <div style={{ minWidth: 0 }}>
                          <div className="al-app__name">{name}</div>
                          <div className="al-app__pkg" style={{ fontFamily: 'var(--font-family-mono)' }}>
                            {r.entity.id}{r.bridge?.mid ? ` · ${r.bridge.mid}` : ''}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td><span style={{ fontSize: 13 }}>{r.entity.country || '—'}</span></td>
                    <td><Badge tone={statusTone} dot>{status}</Badge></td>
                    <td>
                      {r.bridge
                        ? (r.bridge.stores?.length || 0) <= 1
                          ? <span className="muted" style={{ fontSize: 11.5 }}>HQ only</span>
                          : <span style={{ fontFamily: 'var(--font-family-mono)', fontWeight: 500 }}>{r.bridge.stores.length}</span>
                        : <span className="muted" style={{ fontSize: 11.5 }}>—</span>}
                    </td>
                    <td>
                      {r.bridge
                        ? <span style={{ fontFamily: 'var(--font-family-mono)', fontWeight: 500, fontSize: 12.5 }}>{r.bridge.activeTerminalsCount || 0}</span>
                        : <span className="muted" style={{ fontSize: 11.5 }}>No fleet data</span>}
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <button className="iconbtn" onClick={(e) => { e.stopPropagation(); openMerchant(r); }} title="View" aria-label="View">
...
// Expose the read-only merchant detail view so the top-level Merchants
// screen (merchants-screen.jsx) can reuse it.
window.MerchantViewReadonly = MerchantViewReadonly;
window.MerchantStoresTab    = MerchantStoresTab;
