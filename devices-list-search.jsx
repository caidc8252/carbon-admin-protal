...
(function () {
  ...
  const DEVICE_STATE = {
    active: { label: "Online", tone: "success" },
  ...
  // ─── CSV export ──────────────────────────────────────────
  function _dvCsvCell(v) {const s = v == null ? '' : String(v);return /[",\n\r]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s;}
  function _dvToday() {const d = new Date();const p = (n) => String(n).padStart(2, '0');return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}`;}
  function exportDevicesCsv(rows, filename) {
    const header = ['Serial number', 'Model', 'OS', 'Firmware', 'Merchant', 'Store', 'Last seen', 'State'];
    const lines = [header.map(_dvCsvCell).join(',')];
    rows.forEach((d) => {
      const m = window.findMerchantById?.(d.merchantId);
      const s = m?.stores?.find((x) => x.id === d.storeId);
      lines.push([d.sn, d.model, d.os || '', d.firmware || '', m?.name || '', s?.name || '', d.lastSeenAt || '', d.state || ''].map(_dvCsvCell).join(','));
    });
  ...
  function renderCell(key, d) {
    ...
    switch (key) {
      ...
      case "osFirmware":
        return (
          <div>
            <div style={{ fontSize: 12 }}>{d.os || "—"}</div>
            <div className="mono" style={{ fontSize: 10, color: "var(--fg3)" }}>{d.firmware || "—"}</div>
          </div>);
      ...
      case "merchantStore":{
          ...
          return (
            ...
            <div style={{ fontSize: 12, color: "var(--fg1)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 200 }}>{m.name}</div>
            <div style={{ fontSize: 10, color: "var(--fg3)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 200 }}>{s?.name || "—"}</div>
          </div>);
      ...
      case "lastSeen":
        return (
          <div style={{ minWidth: 0 }}>
            <div className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{fmtLastSeen(d.lastSeenAt)}</div>
          </div>);

      case "posture":
        if (!flagged) return <span style={{ fontSize: 12, color: "var(--fg3)" }}>—</span>;
        return (
      ...
      case "warnings":{
          ...
          if (w === 0) return <span style={{ fontSize: 12, color: "var(--fg3)" }}>—</span>;
      ...
      case "ssid":
        ...
        <span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{d.network.wifi.ssid}</span> :
        <span style={{ color: "var(--fg3)" }}>—</span>;
      case "ip":{
          const ip = ipFor(d);
          return ip ?
          <span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{ip}</span> :
          <span style={{ color: "var(--fg3)" }}>—</span>;
        }
      case "signal":{
          const s = signalFor(d);
          ...
          return <span className="mono" style={{ fontSize: 12, color: tone }}>{s} dBm</span>;
        }
      case "battery":{
          ...
          return (
            ...
            <span className="mono" style={{ fontSize: 12, color: tone, fontWeight: 500 }}>{lvl}%</span>
          </div>);

        }
      case "batteryHealth":
        if (!d.battery) return <span style={{ color: "var(--fg3)" }}>—</span>;
        return <span style={{ fontSize: 12, color: "var(--fg2)", textTransform: "capitalize" }}>{d.battery.health}</span>;
      case "storage":{
          ...
          return (
            ...
            <span className="mono" style={{ fontSize: 12, color: tone }}>{pct}%</span>
          </div>);
      ...
      case "imei":
        return (
          <div style={{ minWidth: 0 }}>
            <div className="mono" style={{ fontSize: 10, color: "var(--fg2)" }}>{d.imei || <span style={{ color: "var(--fg4)" }}>no IMEI</span>}</div>
      ...
      case "appsCount":{
          ...
          return (
            ...
            <span className="mono num" style={{ fontSize: 14, color: "var(--fg1)", fontWeight: 500 }}>{c}</span>
  ...
  // ─── Merchant search picker（显式搜索 + 最近使用） ─────────
  //  商户量大 → 不一次性渲染整张列表，也不再"边打字边远程查询"。
  //  交互改为：
  //    · 输入框打字只更新草稿，列表不动、不发请求；
  //    · 回车 或 点框内 Search 按钮 才真正发起一次查询；
  //    · 空输入时展示「最近使用」（最多 5 个，选中即记录，
  //      持久化到 localStorage），没有记录时给引导文案；
  //    · 命中结果按相关度排序、截断到 MERCHANT_RESULT_CAP。
  const MERCHANT_RESULT_CAP = 20;
  const RECENT_MERCHANTS_KEY = "carbon.devices.recentMerchants";
  const RECENT_MERCHANTS_MAX = 5;

  function loadRecentMerchants() {
    try {
      const raw = JSON.parse(localStorage.getItem(RECENT_MERCHANTS_KEY) || "[]");
      return Array.isArray(raw) ? raw.filter((r) => r && r.id) : [];
    } catch (e) {return [];}
  }
  function saveRecentMerchants(list) {
    try {localStorage.setItem(RECENT_MERCHANTS_KEY, JSON.stringify(list));} catch (e) {/* ignore */}
  }
  // 选中即记录：去重置顶、截断到 5、回写 localStorage，返回新列表
  function pushRecentMerchant(prev, m) {
    const next = [{ id: m.id, name: m.name }, ...prev.filter((r) => r.id !== m.id)].slice(0, RECENT_MERCHANTS_MAX);
    saveRecentMerchants(next);
    return next;
  }

  //  模拟后端查询：实际项目里换成 fetch(`/api/merchants?q=`)。
  //  这里对本地 window.MERCHANTS 做模糊匹配，并人为加一点延迟，
  //  以体现 loading 态与"返回数据"的处理路径。
  function queryMerchants(term) {
    return new Promise((resolve) => {
      ...
      // 相关度：名称前缀命中优先，其次按名称字母序
      hits.sort((a, b) => {
        const ap = a.name.toLowerCase().startsWith(n) ? 0 : 1;
        const bp = b.name.toLowerCase().startsWith(n) ? 0 : 1;
        return ap - bp || a.name.localeCompare(b.name);
      });
      setTimeout(() => resolve({ term: n, total: hits.length, rows: hits.slice(0, MERCHANT_RESULT_CAP) }), 220);
    });
  }

  function MerchantPicker({ value, onChange }) {
    const [open, setOpen] = useState(false);
    const [q, setQ] = useState(""); // 草稿输入（不触发查询）
    const [submitted, setSubmitted] = useState(""); // 已执行查询的词
    const [loading, setLoading] = useState(false);
    const [result, setResult] = useState(null); // { term, total, rows } | null
    const [recents, setRecents] = useState(loadRecentMerchants);
    const ref = useRef(null);
    const reqRef = useRef(0); // 防止过期请求覆盖较新的结果
    ...
    // 输入被改动但还没重新搜索 → 结果视为过期
    const stale = !isEmpty && term.toLowerCase() !== submitted;

    // 显式触发查询（回车 / 点 Search 按钮）
    const runSearch = useCallback(() => {
      const t = q.trim();
      if (!t) {setSubmitted("");setResult(null);setLoading(false);return;}
      setSubmitted(t.toLowerCase());
      setLoading(true);
      const myReq = ++reqRef.current;
      queryMerchants(t).then((res) => {
        if (reqRef.current !== myReq) return; // 已有更新请求，丢弃
        setResult(res);
        setLoading(false);
      });
    }, [q]);
    ...
    const selectMerchant = (m) => {
      setRecents((prev) => pushRecentMerchant(prev, m));
      onChange(m.id);
      setOpen(false);
    };
    const clearText = () => {setQ("");setSubmitted("");setResult(null);setLoading(false);};
    const removeRecent = (id) => setRecents((prev) => {const next = prev.filter((r) => r.id !== id);saveRecentMerchants(next);return next;});
    const clearRecents = () => {saveRecentMerchants([]);setRecents([]);};

    const Row = ({ m }) => {
      const isOn = value === m.id;
      return (
        <div onClick={() => selectMerchant(m)}
        style={{
        ...
        onMouseLeave={(e) => {if (!isOn) e.currentTarget.style.background = "transparent";}}>
          ...
          <span style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--fg1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.name}</span>
        </div>);

    };

    const showResults = !isEmpty && !stale && (loading || result);
    ...
    return (
      <div ref={ref} style={{ position: "relative", flex: 1, minWidth: 180 }}>
        ...
        <button onClick={() => setOpen((v) => !v)} style={triggerStyle}>
          ...
          {selected &&
          <span onMouseDown={(e) => {e.preventDefault();e.stopPropagation();onChange(null);clearText();}} title="Clear"
        ...
        <div style={popoverStyle}>
            {/* 输入 + Search 按钮（显式搜索，不跟随输入） */}
            <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--border-1)", display: "flex", gap: 6 }}>
              <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, background: "var(--bg1)", border: "1px solid var(--border-1)", borderRadius: 10, padding: "5px 8px" }}>
              ...
              style={{ flex: 1, minWidth: 0, border: 0, background: "transparent", outline: "none", fontSize: 12 }} />
                {loading &&
              <span style={{
                width: 13, height: 13, borderRadius: "50%", flex: "none",
                border: "2px solid var(--border-2)", borderTopColor: "var(--color-primary-700)",
                animation: "mp-spin 0.6s linear infinite"
              }} />
              }
                {!loading && q.length > 0 &&
              <span onClick={clearText} title="Clear"
              style={{ display: "grid", placeItems: "center", width: 15, height: 15, borderRadius: 4, color: "var(--fg3)", cursor: "pointer", flex: "none" }}>
                    <window.Ico name="x" size={10} />
                  </span>
              }
              </div>
              <button onClick={runSearch} disabled={isEmpty}
            style={{
              display: "inline-flex", alignItems: "center", gap: 5, height: 30, padding: "0 12px",
              fontSize: 12, fontWeight: 600, border: 0, borderRadius: 10, flex: "none",
              cursor: isEmpty ? "not-allowed" : "pointer",
              background: isEmpty ? "var(--bg3)" : "var(--color-primary-700)",
              color: isEmpty ? "var(--fg4)" : "#fff"
            }}>
                <window.Ico name="search" size={11} />
                Search
              </button>
            </div>
            ...
            <>
                  <div style={{ display: "flex", alignItems: "center", padding: "8px 12px 4px" }}>
                    <span style={{ flex: 1, fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--fg3)" }}>Recent</span>
                    <span onClick={clearRecents} style={{ fontSize: 10, color: "var(--fg3)", cursor: "pointer" }}>Clear</span>
                  </div>
                  {recents.map((r) => {
                const m = merchants.find((x) => x.id === r.id) || r;
                const isOn = value === r.id;
                return (
                  <div key={r.id} onClick={() => selectMerchant(m)}
                  style={{
                    padding: "8px 12px", display: "flex", alignItems: "center", gap: 10,
                    cursor: "pointer", background: isOn ? "var(--color-primary-50)" : "transparent",
                    borderLeft: isOn ? "2px solid var(--color-primary-500)" : "2px solid transparent"
                  ...
                  onMouseEnter={(e) => {if (!isOn) e.currentTarget.style.background = "var(--bg-hover)";}}
                  onMouseLeave={(e) => {if (!isOn) e.currentTarget.style.background = "transparent";}}>
                      <window.Ico name="clock" size={12} style={{ color: "var(--fg3)", flex: "none" }} />
                      <span style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--fg1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.name}</span>
                      <span onClick={(e) => {e.stopPropagation();removeRecent(r.id);}} title="Remove"
            ...
            <div style={{ padding: "26px 16px", textAlign: "center" }}>
                  <window.Ico name="search" size={18} style={{ color: "var(--fg4)" }} />
                  <div style={{ marginTop: 8, fontSize: 12, color: "var(--fg2)", fontWeight: 500 }}>
                    Find a merchant
                  </div>
                  <div style={{ marginTop: 3, fontSize: 12, color: "var(--fg3)" }}>
                    Type a merchant name and press Enter to search
                  </div>
                </div>
            }
              ...
              {!isEmpty && stale &&
            <div style={{ padding: "26px 16px", textAlign: "center" }}>
                  <div style={{ fontSize: 12, color: "var(--fg2)", fontWeight: 500 }}>
                    Press <span style={kbdStyle}>Enter</span> to search “{term}”
                  </div>
                  <div style={{ marginTop: 3, fontSize: 12, color: "var(--fg3)" }}>or click the Search button</div>
                </div>
            }

              {/* ③ 已搜索：loading / 无命中 / 结果 */}
              {showResults && loading && !result &&
            <div style={{ padding: "26px 16px", textAlign: "center", fontSize: 12, color: "var(--fg3)" }}>
                  Searching “{submitted}”…
                </div>
            }
              {showResults && !loading && result && result.rows.length === 0 &&
            <div style={{ padding: "26px 16px", textAlign: "center" }}>
                  <div style={{ fontSize: 12, color: "var(--fg2)", fontWeight: 500 }}>No merchants match “{submitted}”</div>
                  <div style={{ marginTop: 3, fontSize: 12, color: "var(--fg3)" }}>Check spelling or try a shorter term</div>
                </div>
            }
              {showResults && result && result.rows.map((m) =>
            <Row key={m.id} m={m} />
            )}
            </div>

            {/* 页脚 */}
            <div style={{ padding: "6px 10px", fontSize: 10, color: "var(--fg3)", borderTop: "1px solid var(--border-1)", background: "var(--bg1)" }}>
              {isEmpty ?
            "Single-select · clearing the merchant also clears the store" :
            stale ?
            "Search isn’t live — submit to query" :
            loading ?
            "Querying merchants…" :
            result && result.total > MERCHANT_RESULT_CAP ?
            `Showing top ${MERCHANT_RESULT_CAP} of ${result.total.toLocaleString()} matches · refine your search` :
            result ?
            `${result.total} match${result.total === 1 ? "" : "es"} · single-select · clears store on change` :
            ""}
            </div>
          </div>
        }
    ...
    return (
      <div ref={ref} style={{ position: "relative", flex: 1, minWidth: 180 }}>
        ...
        style={{
          ...
          <span style={{ flex: 1, minWidth: 0, textAlign: "left", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: selected ? "var(--fg1)" : "var(--fg3)" }} data-comment-anchor="6a52b83d29-span-595-11">
        ...
        <div style={popoverStyle}>
            <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--border-1)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--bg1)", border: "1px solid var(--border-1)", borderRadius: 10, padding: "5px 8px" }}>
                <window.Ico name="search" size={11} style={{ color: "var(--fg3)" }} />
                <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search stores"
              style={{ flex: 1, border: 0, background: "transparent", outline: "none", fontSize: 12 }} />
              </div>
              <div style={{ marginTop: 6, fontSize: 10, color: "var(--fg3)", fontWeight: 500, letterSpacing: "0.04em", textTransform: "uppercase" }}>
            ...
            <div style={{ padding: "16px 12px", textAlign: "center", fontSize: 12, color: "var(--fg3)" }}>
              ...
              return (
                ...
                onMouseLeave={(e) => {if (!isOn) e.currentTarget.style.background = "transparent";}}>
                    ...
                    <span style={{ flex: 1, fontSize: 12, color: "var(--fg1)" }}>{s.name}</span>
                  </div>);
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 5, gridColumn: span ? `span ${span}` : undefined }}>
      <label style={{ fontSize: 10, fontWeight: 600, color: "var(--fg3)", letterSpacing: "0.06em", textTransform: "uppercase" }}>{label}</label>
      {children}
    </div>;
  ...
  <div style={{ display: "inline-flex", padding: 2, background: "var(--bg3)", borderRadius: 10, border: "1px solid var(--border-1)" }}>
      {options.map((o) => {
      const on = value === o.value;
      return (
        <button key={o.value} type="button" onClick={() => onChange(on ? null : o.value)} style={{
          padding: "4px 10px", borderRadius: 4, border: 0, background: on ? "var(--bg2)" : "transparent",
  ...
  function AdvancedSheet({ open, draft, setDraft, onClose, onApply, estimatedCount }) {
    ...
    return (
      <>
      <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "var(--color-bg-overlay)", zIndex: 8000, animation: "fadeIn 120ms ease" }} />
      <section style={{
        ...
        <window.PanelHeader
          ...
          onClose={onClose} closeTitle="Close (Esc)" />
        ...
        <div style={{ padding: "20px 24px", flex: 1, overflowY: "auto", minHeight: 0 }}>
          ...
          <div style={advGrid}>
            ...
            <Field label="OS">
              <select value={draft.os} onChange={(e) => upd("os", e.target.value)} style={selectStyle}>
            ...
            <Field label="Firmware · operator">
              <div style={{ display: "flex", gap: 6 }}>
                <select value={draft.firmwareOp} onChange={(e) => upd("firmwareOp", e.target.value)} style={{ ...selectStyle, width: 64 }}>
          ...
          <div style={advGrid}>
            ...
            <Field label="Activated">
              <select value={draft.activated} onChange={(e) => upd("activated", e.target.value)} style={selectStyle}>
          ...
          <div style={advGrid}>
            ...
            <Field label="Battery health">
              <select value={draft.batteryHealth} onChange={(e) => upd("batteryHealth", e.target.value)} style={selectStyle}>
        ...
        <div style={{
          ...
          <button type="button" onClick={() => setDraft(emptyAdvanced())}
          style={{
    ...
    return (
      <div ref={ref} style={{
        ...
        <div style={{ padding: "12px 14px 8px", borderBottom: "1px solid var(--border-1)" }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: "var(--fg1)" }}>Saved views</div>
            ...
            return (
              ...
              style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 600, color: "var(--fg1)" }}>{v.name}</span>
                  {v.shared &&
                  <span style={{
        ...
        <div style={{
          ...
          <button onClick={onSaveCurrent} style={{
            display: "inline-flex", alignItems: "center", gap: 6,
            padding: "5px 9px", border: 0, borderRadius: 10,
          ...
          <button style={{
            padding: "5px 9px", border: 0, borderRadius: 10,
            background: "transparent", color: "var(--fg3)", fontSize: 12, cursor: "pointer"
          }}>Manage views</button>
        </div>
      </div>);
        ...
        <aside style={{
          ...
          <div style={{ padding: "16px 20px 14px", borderBottom: "1px solid var(--border-1)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: "var(--fg3)", marginTop: 2 }}>
                  {selectedCount} of 22 columns selected · SN & Status are fixed
                </div>
              </div>
              <button onClick={onClose} style={{
                width: 26, height: 26, borderRadius: 10, border: "1px solid var(--border-2)",
            ...
            <div key={g.group} style={{ marginBottom: 10 }}>
                ...
                return (
                  ...
                  onClick={() => !isLocked && toggle(c.key)}>
                      ...
                      <span style={{ flex: 1, fontSize: 12, color: isOn ? "var(--fg1)" : "var(--fg2)" }}>
                        {c.label}
                        {isLocked &&
                      <span style={{
          ...
          <div style={{
            ...
            <button onClick={() => onApply(draft, customName)} style={{
              ...primaryBtnStyle, height: 32, padding: "0 14px", fontSize: 12
            }}>
              Apply
            </button>
          </div>
  ...
  function matchesQuick(d, q) {
    if (q.sn) {
      const term = q.sn.trim().toLowerCase();
      if (term && !d.sn.toLowerCase().includes(term)) return false;
    }
  ...
  function ActiveChips({ quick, advanced, onRemove, onClearAll }) {
    ...
    return (
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center", padding: "0 0 6px" }}>
        <span style={{ fontSize: 10, color: "var(--fg3)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>
          Active filters
        </span>
        {chips.map((c) =>
        ...
        <button onClick={onClearAll} className="sb__clear-all">
          <window.Ico name="x" size={11} />
          Clear all
        </button>
      </div>);
  ...
  function DevicesListScreen({ navigate }) {
    ...
    const cardHeadRef = useRef(null);
    const [offsets, setOffsets] = useState({ toolbar: 0, tabs: 0, desc: 0, cardHead: 0 });
    useLayoutEffect(() => {
      ...
      const measure = () => {
        setOffsets({
          ...
          cardHead: cardHeadRef.current?.offsetHeight || 0
        });
      };
      ...
      if (cardHeadRef.current) ro.observe(cardHeadRef.current);
      measure();
      return () => ro.disconnect();
    }, []);
    const tabsTop = offsets.toolbar;
    const descTop = offsets.toolbar + offsets.tabs;
    const cardHeadTop = offsets.toolbar + offsets.tabs + offsets.desc;
    const theadTop = offsets.toolbar + offsets.tabs + offsets.desc + offsets.cardHead;
      ...
      <div>
        ...
        <div style={{ background: "var(--color-bg-1)" }}>
          ...
          <div ref={toolbarRef} style={{
            ...
            {/* Outer row: search controls on the LEFT (natural width, not
                 stretched), Columns button anchored to the FAR-RIGHT edge of
                 the toolbar (justify-content: space-between). */}
            <div style={{ padding: "0 var(--space-6) 4px" }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              {/* SN input — fixed width so the cluster doesn't stretch; the
                     remaining row width belongs to the right-aligned Columns btn. */}
              <div style={{
                  display: "flex", alignItems: "center", gap: 8, width: 320,
                  padding: "0 10px", height: 36, borderRadius: 10, background: "var(--bg2)",
                ...
                <input value={pendingQuick.sn}
                  ...
                  style={{ flex: 1, border: 0, background: "transparent", outline: "none", fontSize: 14, color: "var(--fg1)" }} />
              ...
              {/* Search — SECONDARY (page primary is reserved; one primary CTA per surface) */}
              <button onClick={doSearch} style={{ ...secondaryBtnStyle, padding: "0 14px", fontSize: 14, color: "var(--fg1)", borderColor: "var(--color-border-default)" }}>
                <window.Ico name="search" size={13} />
                Search
              </button>

              {/* Advanced (toggle) — unified with Tickets */}
              <button onClick={() => setAdvancedOpen((v) => !v)}
                aria-expanded={advancedOpen}
                style={{
                  display: "inline-flex", alignItems: "center", gap: 7, position: "relative",
                  height: 36, padding: "0 12px", borderRadius: 10,
                  border: "1px solid " + (advancedOpen ? "var(--color-primary-500)" : "var(--color-border-default)"),
                  background: advancedOpen ? "var(--color-primary-50)" : "var(--bg2)",
                  color: advancedOpen ? "var(--color-primary-700)" : "var(--fg1)",
                ...
                <window.Ico name="chevD" size={11} style={{ color: advancedOpen ? "var(--color-primary-700)" : "var(--fg3)", transform: advancedOpen ? "rotate(180deg)" : "none", transition: "transform .15s ease" }} />
              </button>

              {/* Saved views — removed */}
            </div>
          </div>{/* /toolbar search row */}
          ...
          <div style={{ padding: "var(--space-3) var(--space-6) var(--space-6)" }}>
            <div style={{
              ...
              {/* Standard list-card head — count (left) + Export (right), sticky */}
              <div ref={cardHeadRef} style={{
                position: "sticky", top: cardHeadTop, zIndex: 19,
                ...
                <span style={{ fontSize: 12, color: "var(--fg2)" }}>
                  <strong className="mono num" style={{ fontWeight: 500, color: "var(--fg1)" }}>{filtered.length.toLocaleString()}</strong>
                  {" "}{filtered.length === 1 ? "device" : "devices"}
                  {filtered.length !== all.length ? <span style={{ color: "var(--fg3)" }}> matching filters</span> : null}
                </span>
                <div style={{ flex: 1 }} />
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <button onClick={openManageColumns}
                  data-comment-anchor="4f3695ff85-button-1152-17"
                  ...
                  <window.ListExportMenu
                    ...
                    onExport={(scope, rows) => {
                      ...
                      `devices_view_${_dvToday()}.csv` :
                      `devices_results_${_dvToday()}.csv`;
                      exportDevicesCsv(rows, fname);
                      if (toast) toast({
                        kind: "success",
                        title: "Export downloaded",
                        desc: `${rows.length.toLocaleString()} device${rows.length === 1 ? "" : "s"} · ${fname}`
                      });
                    }} />
                  }
                </div>
              </div>
              {/* Preset tabs removed — fixed to the default Overview column set */}
              ...
              <div>
                <style>{`
                  ...
                  .devices-table tbody tr:last-child { border-bottom: none !important; }
                `}</style>
                <table className="devices-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                  <thead>
              ...
              {/* Pagination bar — standard shared component */}
              <window.Pagination
                ...
                pageSize={pageSize}
                unit="devices"
                setPage={setPage} setPageSize={setPageSize} divider />
            </div>
          </div>
        </div>
  ...
  // Canonical select — mirrors the bundle's dsSelect (appearance:none + chevron
  // bg) so every <select> matches the list-bar / form selects. CLAUDE.md: Select
  // 统一用 dsSelect.
  const selectStyle = {
    appearance: "none", WebkitAppearance: "none", MozAppearance: "none",
    boxSizing: "border-box", height: 36, lineHeight: "34px",
    padding: "0 30px 0 10px", fontFamily: "inherit", fontSize: 14,
    background: "var(--bg2) url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2398a' stroke-width='2'><polyline points='6 9 12 15 18 9'/></svg>\") no-repeat right 10px center / 12px",
    border: "1px solid var(--border-2)", borderRadius: 10,
    color: "var(--fg1)", cursor: "pointer", outline: "none"
  };
  const primaryBtnStyle = {
    display: "inline-flex", alignItems: "center", gap: 6,
    height: 36, padding: "0 14px", fontSize: 14, fontWeight: 600,
    border: 0, borderRadius: 10, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0,
  ...
  const chipOffStyle = {
    ...
    fontSize: 12, fontWeight: 500, lineHeight: 1.5, cursor: "pointer",
    whiteSpace: "nowrap"
  };
  const advGrid = {
    display: "grid",
