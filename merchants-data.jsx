...
(function seedMerchants() {
  ...
  // ─── Bridge from MERCHANT-contract customer (Entity) → legacy MERCHANTS row ─
  // Phase-1 of the Merchants-into-Customers merge: the new normalized model
  // (data-tables.jsx → SEED_ENTITIES) treats merchants as Entity rows holding
  // a MERCHANT EntityContract. The legacy MERCHANTS data here stays as the
  // "operations-side facts" (stores / terminals / apps) and gets bridged to
  // the corresponding customer entity via this map. The customer-detail page
  // calls findMerchantByCustomerId(c.id) when rendering the Stores & Apps
  // tabs for a MERCHANT-contract customer.
  //
  // Pairings are by flavor, not by exact identity — each new MERCHANT-contract
  // customer (c-007/008/010/013/014) is attached to a legacy merchant whose
  // store footprint vaguely matches. We override the legacy isoId so the
  // ISO-side downstream-Merchants list also resolves correctly.
  const CUSTOMER_BY_MERCHANT = {
    'm-bistro':   'c-007', // Bluefin Market   (auth ISO c-001 Northwind) — 3-store F&B
    'm-petal':    'c-008', // Pinegate Apparel (auth ISO c-002 Helios)    — single retail
    'm-meridian': 'c-010', // Sandstone Bistro (auth ISO c-003 Brightleaf)— multi hospitality
    'm-coffee':   'c-013', // Sunbreak Cafe    (auth ISO c-001 Northwind) — multi coffee
    'm-summit':   'c-014', // Stonefield Auto  (auth ISO c-003 Brightleaf)— multi retail
  };
  // For bridged rows we also need to override the isoId so that ISO
  // customer-detail's Merchants tab (which now queries by authorizing
  // EntityContract) shows them under the correct ISO.
  const AUTH_ISO_BY_MERCHANT = {
    'm-bistro':   'c-001',
    'm-petal':    'c-002',
    'm-meridian': 'c-003',
    'm-coffee':   'c-001',
  ...
  // Minimal metadata for every appId referenced in APPS_BY_ISO. Read by
  // customer-merchants.jsx's MerchantAppsTabRO via window.APPS — the table
  // uses only `name`, `package`, and `iconHue`. (The full SEED_APPS catalog
  // and its publisher/version data have been removed along with the Apps
  // menu, so this is the smallest shape that keeps the assignments table
  // populated.)
  const APP_META = [
    { id: "app-pos-pro",       name: "POS Pro",         package: "io.toms.pospro",       iconHue: "#5B7CFA" },
    { id: "app-stockroom",     name: "Stockroom",       package: "io.toms.stockroom",    iconHue: "#10B981" },
    { id: "app-smart-receipt", name: "Smart Receipt",   package: "io.toms.smartreceipt", iconHue: "#F59E0B" },
    { id: "app-loyalty-plus",  name: "Loyalty+",        package: "io.toms.loyaltyplus",  iconHue: "#EC4899" },
    { id: "app-curbside",      name: "Curbside",        package: "io.toms.curbside",     iconHue: "#06B6D4" },
    { id: "app-insights",      name: "Insights",        package: "io.toms.insights",     iconHue: "#8B5CF6" },
    { id: "app-catalog-sync",  name: "Catalog Sync",    package: "io.toms.catalogsync",  iconHue: "#0EA5E9" },
    { id: "app-timeclock",     name: "Timeclock",       package: "io.toms.timeclock",    iconHue: "#EF4444" },
  ];
  // Merge into window.APPS without clobbering anything an upstream module may
  ...
  MERCHANTS.forEach(m => {
    // Bridged rows: override ISO + attach customerId back-ref. Non-bridged
    // rows fall back to the legacy ISO_BY_MERCHANT mapping.
    const bridgedCid = CUSTOMER_BY_MERCHANT[m.id] || null;
    const iso = AUTH_ISO_BY_MERCHANT[m.id] || ISO_BY_MERCHANT[m.id] || m.isoId || null;
    m.customerId = bridgedCid;
    ...
    // Reference timestamp for lastSeen — fixed per seed pass so reloads
    // stay deterministic. Each active terminal is back-dated by a hashed
    // number of minutes (0…~3 days) so the timeline looks realistic.
    const _now = Date.now();
    const _pad = (n) => String(n).padStart(2, "0");
    const _fmtTs = (d) => `${d.getFullYear()}-${_pad(d.getMonth() + 1)}-${_pad(d.getDate())} ${_pad(d.getHours())}:${_pad(d.getMinutes())}:${_pad(d.getSeconds())}`;
    m.stores = (m.stores || []).map((s, i) => {
      ...
      const terminals = Array.from({ length: count }, (_, j) => {
        ...
        // Back-date by a deterministic number of seconds (0..3 days).
        const offsetSec = ((hash(s.id) * 7 + j * 53) % (60 * 60 * 24 * 3));
        const ts = _fmtTs(new Date(_now - offsetSec * 1000));
        return {
  ...
  // ─── customerId → legacy MERCHANT row ──
  // Used by Customer Detail's Stores & Terminals + Apps tabs when the
  // customer holds a MERCHANT contract. See CUSTOMER_BY_MERCHANT above.
  const merchantByCustomerId = new Map(
    MERCHANTS.filter((m) => m.customerId).map((m) => [m.customerId, m])
  );
  window.findMerchantByCustomerId = (cid) => merchantByCustomerId.get(cid) || null;
})();
