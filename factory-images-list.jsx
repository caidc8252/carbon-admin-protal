...
const FactoryImageList = ({ images, onOpen, onNew }) => {
  ...
  return (
    <div className="page">
      ...
      <window.StatGrid cols={2} style={{ marginBottom: 8 }}>
        <window.StatCard label="Factory images" value={kpis.total.toLocaleString()} description="Published pre-install specs" />
        <window.StatCard label="Bound ISOs" value={kpis.totalIsos.toLocaleString()} description="Unique ISO customers" />
      </window.StatGrid>
            ...
            {pageRows.length === 0 ? (
              ...
              return (
                <tr key={fi.id} onClick={() => onOpen(fi.id)}>
                  <td>
                    <div style={{ fontWeight: 500, fontSize: 14, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 360 }}>
                      {fi.name}
                    </div>
                  </td>
                  <td>
                  ...
                  <td>
                    <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 14, fontWeight: 500 }}>{isoCount}</span>
                    <span className="muted" style={{ fontSize: 12, marginLeft: 4 }}>{isoCount === 1 ? 'ISO' : 'ISOs'}</span>
                  </td>
                  <td>
                    <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 14, fontWeight: 500 }}>{pkgCount}</span>
                    <div className="muted" style={{ fontSize: 12, fontFamily: 'var(--font-family-mono)' }}>{window.fmtBytes(window.factoryImageTotalSize(fi))}</div>
                  </td>
                  <td>
                    <div style={{ fontSize: 12 }}>{relTime(fi.updatedAt || fi.createdAt)}</div>
                    <div className="muted" style={{ fontSize: 12, fontFamily: 'var(--font-family-mono)' }}>{fi.updatedBy || fi.createdBy}</div>
                  </td>
