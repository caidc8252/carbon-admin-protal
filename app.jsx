...
const CURRENT_USER = { name: 'Jordan Diaz', email: 'admin@toms', username: 'jordan.diaz', country: 'US', avatar: null, avatarColor: null, initials: 'JD', org: 'TOMS · Carbon · Production', role: 'Administrator', memberSince: 'January 14, 2024', lastSignIn: '2026-06-12 09:14 PT' };
...
const App = () => {
  ...
  // Live identity shown in the chrome + edited on the My profile page.
  // initials are always derived from the name (never stored).
  const [profileUser, setProfileUser] = useState(CURRENT_USER);
  const meInitials = (profileUser.name || '').trim().split(/\s+/).slice(0, 2).map((w) => w[0] || '').join('').toUpperCase() || '?';
    ...
    <div className="app" data-density={t.density}>
      <aside className="side">
        ...
        <button
          ...
          {profileUser.avatar
            ? <div className="side__avatar" style={{ padding: 0, overflow: 'hidden' }}><img src={profileUser.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>
            : <div className="side__avatar" style={profileUser.avatarColor ? { background: profileUser.avatarColor, color: '#fff' } : undefined}>{meInitials}</div>}
          <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
            <div style={{ color: 'var(--color-text-primary)', fontWeight: 500, fontSize: 12.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{profileUser.name}</div>
            <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{profileUser.email}</div>
          </div>
      ...
      <main className="main">
        <header className="topbar">
          <div className="crumbs">
            ...
            <>
                <a onClick={() => setMenuOpen(true)}>{profileUser.name}</a>
        ...
        <div className="content">
          ...
          {route.name === 'profile' && <ProfilePage user={profileUser} onSave={(u) => setProfileUser((p) => ({ ...p, ...u }))} onSignOut={() => setSignOutOpen(true)} />}
