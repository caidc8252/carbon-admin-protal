...
const AppPublishHost = () => {
  ...
  useEffect(() => {
    ...
    // The App Publish bundle reads fleet globals (TICKETS / DeviceMonitoringTab
    // / logLinesFor) — make sure that chunk is loading too.
    ...
    window.__AP_BUNDLE_PROMISE__ = (async () => {
      ...
      const out = window.Babel.transform(src, {
        ...
        // compact output skips Babel's expensive pretty-printing (which it
        // disables anyway past 500KB, logging a deopt note). Faster codegen
        // + no console noise; we never read this generated source by hand.
        compact: true,
        comments: false,
      }).code;
...
const App = () => {
  ...
  // Re-render once the lazy fleet chunk (tickets/workbench/devices) lands so
  // breadcrumbs, Cmd+K device search and ticket lookups computed during
  // render pick up the freshly-injected globals.
  const [, bumpFleet] = useState(0);
  useEffect(() => {
    const h = () => bumpFleet((n) => n + 1);
    window.addEventListener('toms:fleet-ready', h);
    return () => window.removeEventListener('toms:fleet-ready', h);
  }, []);
  ...
  // Categories (Sales > Catalog > Categories) — the attribute-defining tree.
  const updateCategory = (next) => setCategories((cs) => cs.some((c) => c.id === next.id) ? cs.map((c) => c.id === next.id ? next : c) : [...cs, next]);
  ...
  const goCategories    = () => setRoute({ name: 'categories' });
  const goNewCategory   = (parentId) => setRoute({ name: 'category-new', parentId: parentId || null });
  const goEditCategory  = (id) => setRoute({ name: 'category-edit', id });
  ...
  const inCatalogSection  = route.name === 'catalog' || route.name === 'catalog-new' || route.name === 'catalog-edit' || route.name === 'categories';
  ...
  if (route.name === 'workbench') {
    return (
      <FleetGate full>{() => {
        const wbTicket = window.findTicketById?.(route.id);
        if (!wbTicket || !window.WorkbenchScreen) {
          return (
        ...
        return (
          <window.WorkbenchScreen
            ...
            onCloseTab={() => goTicket(wbTicket.id)}
          />
        );
      }}</FleetGate>
    );
    ...
    <div className="app" data-density={t.density}>
      <aside className="side">
        ...
        <nav className="side__nav">
          ...
          <SidebarItem icon="cart" label="Catalog" active={inProductsSection} onClick={goProducts} />
          <SidebarItem icon="tag" label="Products" active={inCatalogSection} onClick={goCatalog} />
        </nav>
        ...
        <nav className="side__nav">
          ...
          <SidebarItem icon="smartphone" label="Devices" active={inFleetSection} onClick={goDevicesList} />
        </nav>

        <div className="side__sectionlabel">Support</div>
        <nav className="side__nav">
          <SidebarItem icon="alert" label="Tickets" active={inTicketsSection} onClick={goTickets} />
        </nav>
        ...
        <nav className="side__nav">
          ...
          {/* Customer Role Definitions menu hidden by request — route/command-palette/functionality retained */}
        ...
        <button
          ...
          <div className="side__avatar" style={userView.avatarColor ? { background: userView.avatarColor } : undefined}>
            {userView.avatar ? <img src={userView.avatar} alt="" /> : userView.initials}
          </div>
      ...
      <main className="main">
          ...
          <div className="crumbs">
            ...
            <>
                ...
                {route.name === 'products' ? (
                  ...
                  <>
                    <a onClick={goProducts}>Catalog</a>
                    <span className="crumbs__sep">/</span>
            ...
            <>
                ...
                {route.name === 'catalog' ? (
                  ...
                  <>
                    <a onClick={goCatalog}>Products</a>
                    <span className="crumbs__sep">/</span>
                    <span className="crumbs__current">Categories</span>
                  </>
                ) : (
                  <>
                    <a onClick={goCatalog}>Products</a>
        ...
        <div className="content">
          ...
          {route.name === 'admin-users' && <AdminUsers users={users} setUsers={setUsers} currentUserId={CURRENT_USER.id}/>}
          {route.name === 'orders' &&
          <OrderList
            orders={orders}
            onOpen={goOrder}
            ...
            backLabel={
              route.from === 'products' ? 'Back to Catalog' :
              route.from === 'product-detail' ? `Back to ${products.find((p) => p.id === route.fromId)?.name || 'Product'}` :
              undefined
            } />
          }
          {route.name === 'order-new' &&
          <OrderWizard customers={customers} products={products} onCancel={goOrders} onComplete={(o) => {addOrder(o);goOrder(o.id);}} />
          }
          ...
          {route.name === 'tickets' &&
          <FleetGate>{() => <div className="tickets-host"><window.TicketsListScreen navigate={ticketsNavigate} /></div>}</FleetGate>
          }
          {route.name === 'ticket' &&
          <FleetGate>{() => {
            const tk = window.findTicketById?.(route.id);
            if (!tk) {
              return <div className="page"><div className="empty">Ticket not found. <a onClick={goTickets} style={{ color: 'var(--color-primary-500)', cursor: 'pointer' }}>Back to tickets</a></div></div>;
            }
            return <div className="tickets-host"><window.TicketDetailScreen ticket={tk} navigate={ticketsNavigate} /></div>;
          }}</FleetGate>
          }
          {route.name === 'ticket-new' &&
          <FleetGate>{() => <div className="tickets-host"><window.NewTicketScreen navigate={ticketsNavigate} presetSn={route.deviceSn} /></div>}</FleetGate>
          }
          {route.name === 'models' &&
          <DeviceModelList
            models={models}
            ...
            deleteBlockReason={(m) => {
              ...
              const OPEN_ORDER_STATUSES = ['PENDING_PAYMENT', 'PENDING_SHIP'];
              const prodRefs = products.filter((p) => p.deviceModelId === m.id).length;
              const modelProductIds = new Set(products.filter((p) => p.deviceModelId === m.id).map((p) => p.id));
              const orderRefs = orders.filter((o) =>
                OPEN_ORDER_STATUSES.includes(o.status) && (o.items || []).some((i) => modelProductIds.has(i.productId) || i.modelId === m.id)
              ).length;
          ...
          {route.name === 'catalog' &&
          <ProductList products={products} categories={categories} onOpen={goEditProduct} onNew={goNewProduct} onManageCategories={goCategories}
            onDelete={(p) => {
          ...
          {route.name === 'catalog-new' &&
          <ProductForm categories={categories} products={products} onCancel={goCatalog} onSave={(p) => { addProduct(p); goCatalog(); }} />
          }
          {route.name === 'catalog-edit' && (() => {
            ...
            return <ProductForm initial={currentProduct} categories={categories} products={products} onCancel={goCatalog} onSave={(p) => { updateProduct(p); goCatalog(); }}
              onDelete={(prod) => {
          ...
          {route.name === 'categories' &&
          <window.CategoryManager categories={categories} products={products} models={models} onBack={goCatalog}
            onSave={(c, affectedProducts) => {
              updateCategory(c);
              (affectedProducts || []).forEach((p) => updateProduct(p));
            }}
            onDelete={(c) => { removeCategory(c.id); toast({ kind: 'success', title: `${c.name} deleted` }); }} />
          }
          {route.name === 'devices-list' &&
            <FleetGate>{() => <div className="devices-host"><window.DevicesListScreen navigate={fleetNavigate} /></div>}</FleetGate>
          }
          {route.name === 'device-detail' &&
          <FleetGate>{() => {
      ...
      {signedOut && (
        <div className="signedout">
          <div className="signedout__card">
            ...
            <p className="signedout__sub">Your session ended at {window.fmtTime(new Date())}. Sign back in to continue.</p>
