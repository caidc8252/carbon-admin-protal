...
const ContractAssignBoard = ({
  ...
  React.useEffect(() => {
    ...
    if (!cur.effectiveFrom) {
      ...
      setTermField('effectiveTo', window.addDaysDate(today, 180));
    }
  ...
  const renderPickRow = (kind) => {
    ...
    return (
      ...
      >
        <div className={`pick-card__icon pick-card__icon--${meta.iconClass}`} style={{ width: 32, height: 32, borderRadius: 6 }}>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="cab-railrow__sub" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span className={`tds-badge tds-badge--${statusTone}`} style={{ padding: '1px 6px', fontSize: 10 }}>
              <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'currentColor' }}/>{statusWord}
            </span>
            {hasErr && (
              <span style={{ color: 'var(--color-error-700)', fontSize: 12, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                <Icon name="alert" size={10}/>{Object.keys(kindErrs).length} error{Object.keys(kindErrs).length === 1 ? '' : 's'}
              </span>
            )}
          </div>
          ...
          <>
            <div className="cab__mainhead">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
                <div className={`pick-card__icon pick-card__icon--${focusedMeta.iconClass}`} style={{ width: 30, height: 30, borderRadius: 6, flex: 'none' }}>
              ...
              {!isEditMode && !isManageMode && !focusedSelected && (
                <span style={{ fontSize: 12, color: 'var(--color-warning-700)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            ...
            <div className="cab__form">
              ...
              {focusedSelected && focusedIsPilot && (
                <div className="ca-pilot-note" style={{
              ...
              {terms && setTerms && focusedSelected && (() => {
                ...
                return (
                  <div className="cab-termrow">
                    ...
                    <Field label="Effective to date" hint={pilotLockEffTo ? 'Auto-set to +180 days. Use Extend / Convert to change.' : 'Leave blank for no end date.'}>
...
.cab-railrow__title { font-size: 14px; font-weight: 500; color: var(--color-text-primary); line-height: 1.2; }
.cab-railrow__sub   { font-size: 12px; color: var(--color-text-secondary); margin-top: 2px; line-height: 1.3; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
