    ...
    {(title || action) && (
      <header style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {title && <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-primary)', letterSpacing: '-0.005em' }}>{title}</div>}
          {hint && <div style={{ marginTop: 3, fontSize: 12, color: 'var(--color-text-tertiary)', lineHeight: 1.5 }}>{hint}</div>}
        </div>
...
const ShimDatePicker = ({ size = 'md', invalid, disabled, style, value, onChange,
  ...
  useEffect(() => {
    ...
    const onKey = (e) => { if (e.key === 'Escape') setSafe(false, e); };
    const setSafe = (v, e) => { e && e.stopPropagation(); setOpen(v); };
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative', ...style }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, zIndex: 'var(--z-popover, 1100)',
                      background: 'var(--color-bg-2)', border: '1px solid var(--color-border-subtle)',
                      borderRadius: 'var(--card-radius-md, 12px)', boxShadow: 'var(--shadow-3)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2 }}>
            {_DOW.map((w, i) => (
              <div key={'w' + i} style={{ textAlign: 'center', fontSize: 10, fontWeight: 600,
      ...
      <div style={{
        ...
        {(title || subtitle) && (
          <header style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {title && <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-text-primary)' }}>{title}</div>}
              {subtitle && <div style={{ marginTop: 4, fontSize: 12, color: 'var(--color-text-tertiary)' }}>{subtitle}</div>}
            </div>
...
const PageHeader = ({ title, subtitle, actions, tabs }) => (
  <header style={{
    padding: tabs ? 'var(--space-4) var(--space-6) 0' : 'var(--space-4) var(--space-6)',
...
const AppIcon = ({ app, size = 36 }) => {
  ...
  return (
    <div style={{
      ...
      color: 'var(--avatar-fg)', display: 'grid', placeItems: 'center',
