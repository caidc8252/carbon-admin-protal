          ...
          <React.Fragment key={p.key}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
              <div style={{
                ...
                background: (done || active) ? 'oklch(55% 0.15 262)' : partial ? 'var(--color-info-50)' : 'var(--color-bg-3)',
                color: (done || active) ? '#fff' : partial ? 'var(--color-info-700)' : 'var(--color-text-tertiary)',
                border: `1px solid ${(done || active) ? 'oklch(55% 0.15 262)' : partial ? 'oklch(60% 0.14 230 / 0.4)' : 'var(--color-border-default)'}`,
                boxShadow: active ? '0 0 0 3px oklch(55% 0.15 262 / 0.22)' : 'none',
              }}>
                {done ? <Icon name="check" size={16}/> : <Icon name={p.icon} size={16}/>}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.15 }}>
                <small style={{ fontSize: 10, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 500 }}>Step {i + 1}</small>
                <strong style={{ fontSize: 14, fontWeight: active || partial ? 600 : 500, color: active || done || partial ? 'var(--color-text-primary)' : 'var(--color-text-secondary)' }}>
            ...
            {i < PIPELINE.length - 1 && (
              <div style={{ flex: 1, height: 2, background: i < idx ? 'oklch(55% 0.15 262)' : 'var(--color-border-default)', margin: '0 14px' }}/>
            )}
          </React.Fragment>
        );
      })}
      ...
      {!readonly && (
        <div style={{ display: 'flex', alignItems: 'stretch', gap: 10, padding: 14, background: 'var(--color-bg-2)', border: `1px solid ${inputBorder}`, borderRadius: 12, marginBottom: 14, transition: 'border-color 160ms' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Activation code</label>
            <input
              ref={inputRef}
              value={code}
        ...
        {order.items.map(it => {
          ...
          return (
            <div key={it.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-primary)' }}>{it.modelName}</div>
              ...
              <div style={{ fontVariantNumeric: 'tabular-nums', fontSize: 12, fontWeight: 500, color: complete ? 'var(--color-success-700)' : 'var(--color-text-secondary)' }}>
                {done}/{it.qty}
              </div>
            </div>
          );
      ...
      <div className="info-card">
          ...
          {removeTarget && (
            <div>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 14px', background: 'var(--color-bg-2)', border: '1px solid var(--color-border-subtle)', borderRadius: 8, cursor: 'pointer' }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--color-text-primary)' }}>Also factory-reset the device</div>
        ...
        {allDevices.length === 0 ? (
          <div className="empty" style={{ padding: '36px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', marginBottom: 4 }}>No devices activated yet.</div>
              ...
              return (
                <div key={`${d.itemId}-${d.deviceIdx}`} style={{
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: 'var(--font-family-mono)', fontSize: 14, fontWeight: 500, color: 'var(--color-text-primary)' }}>{d.sn}</div>
                    <div style={{ fontSize: 12, color: 'var(--color-text-tertiary)', marginTop: 1, fontFamily: 'var(--font-family-mono)' }}>code {d.code}</div>
                  </div>
                  <div>
                    <span style={{ display: 'inline-block', padding: '3px 8px', borderRadius: 4, background: 'var(--color-bg-3)', border: '1px solid var(--color-border-subtle)', fontSize: 12, fontWeight: 500, color: 'var(--color-text-primary)' }}>{d.modelName}</span>
                  </div>
                  <div>
                    {readonly || d.type ? (
                      <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>{d.type || d.expectedType}</span>
                    ) : (
  ...
  return (
    <div style={{
      ...
      <input
        ...
        style={{
          padding: '7px 12px', borderRadius: 6, border: '1px solid var(--color-border-default)', background: 'var(--color-bg-2)',
          fontFamily: 'var(--font-family-mono)', fontSize: 16, letterSpacing: '0.3em', color: 'var(--color-text-primary)', outline: 'none', width: '100%',
          textAlign: 'center', fontVariantNumeric: 'tabular-nums', fontWeight: 600,
        }}/>

      {/* Resolved SN chip / status */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, minHeight: 32, fontSize: 12, color: 'var(--color-text-secondary)' }}>
  ...
  return (
    <div className="info-card" style={{ marginBottom: 14 }}>
      <button
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 14, letterSpacing: '-0.005em' }}>{item.modelName}</div>
          <div style={{ fontSize: 12, color: 'var(--color-text-tertiary)', marginTop: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontVariantNumeric: 'tabular-nums', fontSize: 12, color: allDone ? 'var(--color-success-700)' : 'var(--color-text-secondary)', fontWeight: 500, minWidth: 36 }}>
              {filled}/{total}
            </span>
          </div>
...
const OrderDetail = ({ order, onBack, onUpdate, initialTab }) => {
  const toast = useToast();
  // The "Devices & activation" flow only applies to sample-device orders.
  // Sample items carry deviceVariant === 'SAMPLE'; legacy seed orders predate
  // that field, so a line whose model resolves to a known device is treated
  // as a sample (the seed data is all sample orders).
  const hasSampleItems = order.items.some((i) =>
    i.deviceVariant === 'SAMPLE' ||
    (i.deviceVariant == null && DEVICE_MODELS.some((m) => m.id === i.modelId))
  );
  // Default tab inferred from status — caller can override via initialTab.
  const defaultTab =
    hasSampleItems && (order.status === 'Awaiting shipment' || order.status === 'Partially complete') ? 'activation' :
    'overview';
  ...
  // Non-sample orders (accessories, production hardware) don't go through the
  // SN + activation-code flow, so they can ship immediately.
  const requiresActivation = hasSampleItems;
  const canShip = !requiresActivation || allActivated;
  ...
  const markComplete = () => setCompleteOpen(true);
  const confirmComplete = () => {
    advanceStatus('Complete', 'Order marked complete · customer confirmed receipt of all items');
  ...
  return (
    <div className="page">
      ...
      <div className="det-header">
        <button type="button" className="det-back" onClick={onBack} aria-label="Back to Orders" title="Back to Orders"><Icon name="chevL" size={18}/></button>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {order.status === 'Awaiting shipment' && (
            <Btn variant="primary" icon="truck" onClick={markShipped} disabled={!canShip} title={!canShip ? `${prog.total - prog.done} device(s) still need SN + activation codes` : ''}>
              {canShip ? 'Ship order' : `Activate ${prog.total - prog.done} more`}
            </Btn>
          )}
          {order.status === 'Shipped' && (
            <Btn variant="primary" icon="check" onClick={markComplete}>Mark complete</Btn>
          )}
      ...
      {/* Activation guard banner for Awaiting shipment (sample-device orders only) */}
      {order.status === 'Awaiting shipment' && requiresActivation && (
      ...
      <div className="det-tabs">
        {[
          ...
          ...(hasSampleItems ? [{ k: 'activation', label: `Devices & activation`, badge: `${prog.done}/${prog.total}` }] : []),
        ].map(x => (
          <button key={x.k}
            ...
            <table className="tds-table">
              ...
              <tfoot>
                ...
                <tr style={{ background: 'var(--color-bg-3)' }}>
                  <td colSpan="4" style={{ textAlign: 'right', fontWeight: 600, fontSize: 16, padding: '14px 16px', borderTop: '1px solid var(--color-border-default)' }}>Total</td>
                  <td className="num" style={{ textAlign: 'right', fontWeight: 700, fontSize: 18, padding: '14px 16px', borderTop: '1px solid var(--color-border-default)', color: total === 0 ? 'var(--color-success-700)' : 'var(--color-text-primary)', letterSpacing: '-0.01em' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div className="info-card">
              ...
              <div className="info-card__body">
                ...
                <dl className="kv2">
                  <div className="kv2__row"><dt>Recipient</dt><dd>{order.shipping?.name || <span className="muted">—</span>}</dd></div>
                  <div className="kv2__row kv2__row--full"><dt>Address</dt><dd>{order.shipping?.address}</dd></div>
                  <div className="kv2__row"><dt>Method</dt><dd>{order.shipping?.method}</dd></div>
                  {order.shipping?.tracking && <div className="kv2__row"><dt>Tracking</dt><dd style={{ fontFamily: 'var(--font-family-mono)', fontSize: 12 }}>{order.shipping.tracking}</dd></div>}
                </dl>
                </div>
              </div>
            </div>
            ...
            <div className="info-card">
              ...
              <div className="info-card__body">
                <div className="kv2-wrap">
                <dl className="kv2">
                  <div className="kv2__row"><dt>Created</dt><dd>{fmtDateTime(order.createdAt)}</dd></div>
                  <div className="kv2__row"><dt>By</dt><dd style={{ fontFamily: 'var(--font-family-mono)', fontSize: 12 }}>{order.createdBy}</dd></div>
                  {order.notes && <div className="kv2__row kv2__row--full"><dt>Notes</dt><dd style={{ color: 'var(--color-text-secondary)' }}>{order.notes}</dd></div>}
                </dl>
                </div>
              </div>
            </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Btn variant="secondary" onClick={() => setCompleteOpen(false)}>Cancel</Btn>
            <Btn variant="primary" icon="check" onClick={confirmComplete}>Confirm complete</Btn>
          </>
        }
      >
        <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
          Please confirm that <strong style={{ color: 'var(--color-text-primary)' }}>{order.customerName}</strong> has received all items in this order. Marking it complete closes out the order and it can no longer be edited.
        </div>
        <div className="notice" style={{ marginTop: 14, background: 'var(--color-info-50)', borderColor: 'oklch(60% 0.14 230 / 0.25)', color: 'var(--color-info-700)' }}>
          <Icon name="clock" size={14}/>
          <div>If no action is taken, this order will be marked complete automatically 30 days after shipment.</div>
        </div>
      </Modal>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
          ...
          />
          <div style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>% off</div>
          <div style={{ flex: 1 }}/>
          <div className="num" style={{ fontSize: 16, fontWeight: 600, color: (Number(discDraft) || 0) === 100 ? 'var(--color-success-700)' : 'var(--color-text-primary)' }}>
