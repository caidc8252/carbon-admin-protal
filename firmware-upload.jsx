...
// Incremental OTA sub-packages — mirrors firmware-data.jsx's __incPkgs so the
// upload preview & saved version match the seed (version-range diff packages,
// e.g. "V1.0.49 → V1.0.60"), not the legacy boot.img/system.img file names.
const __fwUpPkgHash = (s) => {
  let h = 0xdeadbeef >>> 0;
  for (const ch of (s || '')) h = ((h * 33) ^ ch.charCodeAt(0)) >>> 0;
  return h >>> 0;
};
const __fwUpIncPkgs = (modelCode, version, md5fn) => {
  const h = __fwUpPkgHash((modelCode || '') + (version || ''));
  const end = 60 + (h % 30);                 // end build 60..89
  const deltas = [11, 9, 7, 4];              // descending → smaller, lighter patches
  return deltas.map((d, i) => {
    const start = end - d;
    const jitter = (__fwUpPkgHash((version || '') + ':' + i) % 100) / 100;
...
// ─── Read-only parsed field value ─────────────────────────
// These come from the filename and cannot be edited.
const ReadOnlyValue = ({ value }) => (
  ...
  >
    ...
    <Icon name="lock" size={12} style={{ color: 'var(--color-text-tertiary)', flexShrink: 0 }}/>
  </div>
);
...
const FileInfoForm = ({ os, file, parsed, fields, setField }) => {
  ...
  const mockPkgs = useMemoFU(() => {
    ...
    return __fwUpIncPkgs(fields.modelCode, fields.versionName, () => '— pending —');
  }, [meta.hasSubPackages, fields.modelCode, fields.versionName]);
  ...
  return (
    <div className="info-card">
      ...
      <div className="info-card__body">
        ...
        {/* Parsed-from-filename fields — read-only, cannot be edited */}
        <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <Field label="Version name" hint="Parsed from filename — read-only.">
            <ReadOnlyValue value={fields.versionName}/>
          </Field>
          <Field label="Model code" hint="Parsed from filename — read-only.">
            <ReadOnlyValue value={fields.modelCode}/>
          </Field>
          <Field label="Hardware ID" hint="The {hardware id} segment from the filename — identifies the device characteristic (e.g. A105G = Android 10 · 5G chip). Read-only.">
            <ReadOnlyValue value={fields.hardwareId}/>
          </Field>
          <Field label="OS version" hint="Shown in the OS column, e.g. ANDROID 10. Adjust if needed.">
            <Input value={fields.osVersion || ''} onChange={e => setField('osVersion', e.target.value)} placeholder="e.g. ANDROID 10"/>
          </Field>
          {os === 'LINUX' && (
            <>
            ...
            <div className="fu-pkglist">
              {mockPkgs.map((p, i) => (
                <div key={i} className="fu-pkglist__row">
                  <div className="fu-pkglist__name"><span style={{ fontFamily: 'var(--font-family-mono)' }}>{p.name}</span></div>
...
// ─── Wizard shell ─────────────────────────────────────────
// Sensible OS-version default shown in the editable "OS version" field.
const DEFAULT_OS_VERSION = { ANDROID: 'ANDROID 14', LINUX: 'LINUX 5.10', RTOS: 'RT-Thread 4.1' };

const FirmwareUploadWizard = ({ onCancel, onSaved, onCommit, presetOs, presetModelCode }) => {
  // When the OS is preset (the only live entry — OS is fixed by the device
  // model), step 1 (OS picker) is obsolete and must never be reachable. The
  // wizard starts at, and can't regress below, minStep.
  const minStep = presetOs ? 2 : 1;
  const [step, setStep] = useStateFU(minStep);   // 1 = OS · 2 = upload · 3 = info
  const [os, setOs]     = useStateFU(presetOs || null);
  const [file, setFile] = useStateFU(null);
  const [error, setError] = useStateFU(null);
  const [fields, setFields] = useStateFU({
    versionName: '', versionCode: '', modelCode: '', hardwareId: '', osVersion: '',
  ...
  const handleFile = (f) => {
    ...
    setFields({
      ...
      osVersion:    DEFAULT_OS_VERSION[os] || '',
  ...
  const canSave = fields.versionName.trim() &&
                  fields.modelCode.trim()   && fields.hardwareId.trim();
  ...
  const save = () => {
    ...
    const meta = window.FW_OS_META[os];
    // Mock sub-packages — incremental OTA diffs (matches seed). Backend
    // replaces with real entries + MD5s on its side.
    const mockPkgs = !meta.hasSubPackages ? [] :
      __fwUpIncPkgs(fields.modelCode, fields.versionName,
        (start, end) => __fakeMd5(fields.modelCode + fields.versionName + start + '-' + end));
    const version = {
      id: `fwv-${fields.modelCode}-${fields.hardwareId}-${fields.versionName}-${Date.now()}`.toLowerCase().replace(/[^a-z0-9-]/g, ''),
      versionName:  fields.versionName.trim(),
      versionCode:  parseInt(fields.versionCode, 10) || 1,
      hardwareId:   fields.hardwareId.trim(),
      osVersion:    (fields.osVersion || '').trim() || os,
      os,
      fileName:     file.name,
    ...
    toast({
      kind: 'success',
      title: `Uploaded · ${fields.modelCode} · ${fields.hardwareId} · ${fields.versionName}`,
      msg: 'Saved to the version inventory. Open the Deployments tab to publish it to ISOs.',
    });
  ...
  return (
    <div className="page">
      <window.TitleBar
        ...
        subtitle="Upload a new OTA firmware bundle. The system parses the filename to backfill model code, hardware ID and version."
      />
      ...
      <window.WizardFooter
        ...
        backDisabled={step <= minStep}
        onNext={() => setStep(s => s + 1)}
        nextDisabled={step === 1 ? !os : step === 2 ? !canAdvanceStep2 : false}
        onFinal={save} finalDisabled={!canSave}
        finalLabel="Upload firmware version" />
