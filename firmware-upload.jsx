...
const ReadOnlyValue = ({ value }) => (
  <div
    style={{
      ...
      fontFamily: 'var(--font-family-mono)', fontSize: 14,
    ...
    <div className="info-card">
      ...
      <div className="info-card__body">
        ...
        <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <Field label="Device flag" hint="The {device flag} segment from the filename — read-only.">
            <ReadOnlyValue value={fields.deviceFlag}/>
          </Field>
          {os === 'LINUX' && (
            <>
        ...
        <div style={{ marginTop: 14 }}>
          <Field label="Changelog" hint="Short notes about this version — shown on the version detail page.">
            <textarea
              ...
              style={{ resize: 'vertical', minHeight: 64, fontFamily: 'var(--font-family-sans)', fontSize: 14, lineHeight: 1.55, width: '100%', padding: '8px 10px', border: '1px solid var(--color-border-default)', borderRadius: 6, background: 'var(--color-bg-2)', color: 'var(--color-text-primary)' }}
            />
          </Field>
        </div>
...
const FirmwareUploadWizard = ({ onCancel, onSaved, onCommit }) => {
  ...
  const save = () => {
    ...
    toast({
      kind: 'success',
      title: `Uploaded · ${fields.modelCode} · ${fields.deviceFlag} · ${fields.versionName}`,
  ...
  return (
    <div className="page">
      ...
      <div className="fu-footer">
        ...
        {step === 3 && (
          <Btn variant="primary" disabled={!canSave} onClick={save} icon="check">Upload firmware version</Btn>
        )}
      </div>

      <style>{`
        .fu-stepper { display: flex; align-items: center; gap: 0; margin-bottom: 18px; padding: 14px 16px; background: var(--color-bg-2); border: 1px solid var(--color-border-subtle); border-radius: 8px; }
        ...
        .fu-os__hint { font-size: 12px; color: var(--color-text-tertiary); }

        .fu-pattern-banner { display: flex; gap: 8px; align-items: center; padding: 10px 14px; background: var(--color-primary-50, oklch(96% 0.02 262)); border: 1px solid oklch(45% 0.16 262 / 0.18); border-radius: 8px; color: var(--color-text-primary); font-size: 12px; margin-bottom: 14px; }
        ...
        .fu-summary__val { font-size: 14px; color: var(--color-text-primary); min-width: 0; word-break: break-all; }

        .fu-pkglist { border: 1px solid var(--color-border-subtle); border-radius: 8px; overflow: hidden; }
        .fu-pkglist__row { display: grid; grid-template-columns: 1fr 100px 1fr; gap: 12px; padding: 10px 14px; border-bottom: 1px solid var(--color-border-subtle); align-items: center; font-size: 12px; }
        ...
        .fu-footer { display: flex; gap: 10px; align-items: center; margin-top: 18px; padding: 14px 16px; background: var(--color-bg-2); border: 1px solid var(--color-border-subtle); border-radius: 8px; }

        .fu-publish { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        @media (max-width: 760px) { .fu-publish { grid-template-columns: 1fr; } }
        .fu-publish__opt { text-align: left; padding: 14px 16px; border: 1.5px solid var(--color-border-default); background: var(--color-bg-2); border-radius: 8px; cursor: pointer; transition: all 0.12s; display: flex; flex-direction: column; gap: 6px; }
