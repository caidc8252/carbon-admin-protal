...
// ─── Version chip ─────────────────────────────────────────
// Display-only — not a link to the version detail page.
const FwVerChip = ({ v }) => (
  <span
    className="fwdep-chip"
    title={`${v.versionName} · ${v.hardwareId} · ${v.fileSize}`}>
    <span className="fwdep-chip__name">{v.versionName}</span>
    <span className="fwdep-chip__hw">{v.hardwareId}</span>
  </span>
);
...
const FwVerChips = ({ versions, max = 4 }) => {
...
// ─── Deployments tab ──────────────────────────────────────
const TabFwDeployments = ({ firmware, onMutateFirmware }) => {
  const toast = useToast();
  const [page, setPage] = useStateFDep(1);
  const [pageSize, setPageSize] = useStateFDep(25);
  const toolbarRef = React.useRef(null);
  const { draft, applied, setDraft, runSearch, clearAll, clearOne, hasFilters } =
    window.useSearchBar({ q: '' }, () => setPage(1));
  ...
  const filtered = useMemoFDep(() => {
    const qn = (applied.q || '').trim().toLowerCase();
    if (!qn) return deployedIsos;
    return deployedIsos.filter((iso) => {
      if (iso.name.toLowerCase().includes(qn)) return true;
      const vs = isoToVersions[iso.id] || [];
      return vs.some(v => (v.versionName || '').toLowerCase().includes(qn));
    });
  }, [deployedIsos, isoToVersions, applied]);
  ...
  const applySet = (isoIds, versionIds) => {
    ...
    if (editCtx?.mode === 'edit' && ids.length === 1) {
      ...
      if (versionIds.length === 0) {
        toast({ kind: 'warning', title: `Unpublished · ${iso?.name || 'ISO customer'}`, msg: 'No firmware versions are published to this ISO customer anymore.' });
      } else {
        toast({
          kind: 'success',
          title: `Deployment updated · ${iso?.name || 'ISO customer'}`,
    ...
    toast({
      kind: 'success',
      title: only ? `Assigned · ${only.name}` : `Assigned to ${ids.length} ISO customers`,
      msg: `${versionIds.length} version${versionIds.length === 1 ? '' : 's'} published to ${ids.length} ISO customer${ids.length === 1 ? '' : 's'}.`,
    });
  };
  ...
  const doRemoveIso = (isoId) => {
    ...
    setRemoveCtx(null);
    toast({ kind: 'warning', title: `Unpublished · ${iso?.name || 'ISO customer'}`, msg: 'All firmware versions unpublished from this ISO customer. Already-rolled-out terminals are unaffected.' });
  };

  const hasAnyVersions = (firmware.versions || []).length > 0;

  // ── Pagination (ui-spec §3.2) ────────────────────────
  const totalPages = Math.max(1, Math.ceil(filtered.length / pageSize));
  const safePage = Math.min(page, totalPages);
  useEffectFDep(() => { if (page > totalPages) setPage(totalPages); }, [totalPages, page]);
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <h2 style={{ margin: 0, fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' }}>ISO customer deployments</h2>
          <p style={{ margin: '2px 0 0', color: 'var(--color-text-tertiary)', fontSize: 12, lineHeight: 1.5 }}>
            Each ISO customer sees the firmware versions published to it in their OTA menu. Assign an ISO customer and pick which versions it can see — add or remove versions any time.
          </p>
        </div>
        <Btn variant="primary" icon="plus"
          disabled={!hasAnyVersions || undeployedIsos.length === 0}
          onClick={() => setEditCtx({ mode: 'assign' })}>Assign ISO customer</Btn>
      </div>
      ...
      {deployedIsos.length === 0 ? (
        <div style={{
          ...
          <div style={{ marginTop: 10, fontSize: 14, fontWeight: 500 }}>No ISO customer deployments yet</div>
          <p style={{ marginTop: 4, color: 'var(--color-text-tertiary)', fontSize: 12 }}>
            {hasAnyVersions
              ? 'Assign this firmware to an ISO customer and pick which versions it can see in its OTA management menu.'
              : 'Upload a firmware version first, then assign it to ISO customers here.'}
          </p>
          {hasAnyVersions && (
            <div style={{ marginTop: 14, display: 'inline-flex' }}>
              <Btn variant="primary" icon="plus" onClick={() => setEditCtx({ mode: 'assign' })}>Assign ISO customer</Btn>
            </div>
          )}
        </div>
      ) : (
        <>
          {/* Filter row */}
          <div ref={toolbarRef} style={{ marginBottom: 12 }}>
            <window.SearchBar onSearch={runSearch} sticky={false}>
              <window.SearchInput
                value={draft.q}
                onChange={(v) => setDraft(d => ({ ...d, q: v }))}
                onSearch={runSearch}
                placeholder="Search ISO customer or version…"
                width={320}/>
            </window.SearchBar>
          ...
          {/* Table — standard ListCard + Pagination (ui-spec §3) */}
          <window.ListCard toolbarRef={toolbarRef}>
            <window.ListCardHead
              count={filtered.length}
              unit={`ISO customer${filtered.length === 1 ? '' : 's'}`}
              hasFilters={hasFilters}/>
            <table className="tds-table">
              <thead>
                <tr>
                  <th style={{ width: '30%' }}>ISO customer</th>
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={4}><div className="empty">No ISO customers match your filters.</div></td></tr>
                )}
                {pageRows.map((iso) => {
                  const vs = isoToVersions[iso.id] || [];
                  return (
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: 500, letterSpacing: '-0.005em' }}>{iso.name}</div>
                            <div className="muted" style={{ fontSize: 11, marginTop: 1 }}>{iso.country || '—'}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                        <FwVerChips versions={vs}/>
                      </td>
                      <td style={{ textAlign: 'right' }}>
                        <span className="num" style={{ fontFamily: 'var(--font-family-mono)', fontSize: 13, fontWeight: 500 }}>{vs.length}</span>
                      </td>
                      <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'inline-flex', gap: 6, alignItems: 'center', justifyContent: 'flex-end' }}>
                          <button type="button" className="iconbtn" title="Edit versions" aria-label="Edit versions" onClick={() => setEditCtx({ mode: 'edit', isoId: iso.id })}>
                            <Icon name="edit" size={15}/>
                          </button>
                          <button type="button" className="iconbtn iconbtn--danger" title="Unpublish" aria-label="Unpublish" onClick={() => setRemoveCtx({ iso, versions: vs })}>
            ...
            <window.Pagination
              ...
              totalUnfiltered={deployedIsos.length}
              pageStart={pageStart}
              pageEnd={pageEnd}
              page={safePage}
      ...
      {removeCtx && (
        <Modal open onClose={() => setRemoveCtx(null)} width={480}
          ...
          footer={<>
            ...
            <Btn variant="danger" icon="ban" onClick={() => doRemoveIso(removeCtx.iso.id)}>Unpublish</Btn>
          </>}>
          <div style={{ fontSize: 13.5, lineHeight: 1.6, color: 'var(--color-text-secondary)' }}>
            ...
            This ISO customer will no longer see this firmware in its OTA management menu. Already-rolled-out terminals keep running their installed version. Release history is preserved — you can re-assign later.
          </div>
        </Modal>
      )}
      ...
      <style>{`
        ...
        .fwdep-chip__name { font-family: var(--font-family-mono); font-size: 11.5px; font-weight: 500; }
        .fwdep-chip__hw { font-family: var(--font-family-mono); font-size: 10px; font-weight: 500; padding: 1px 5px; border-radius: 999px; background: var(--color-bg-3); border: 1px solid var(--color-border-subtle); color: var(--color-text-tertiary); }
        .fwdep-chip--more {
          cursor: pointer;
    ...
    <Modal open onClose={onClose} width={isEdit ? 600 : 760}
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span>{isEdit ? 'Edit published versions' : 'Assign ISO customer'}</span>
          <span className="fwdep-vchip">
            <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 12 }}>{firmware.modelCode} firmware</span>
      ...
      footer={<>
        <div style={{ flex: 1, fontSize: 12, color: 'var(--color-text-tertiary)' }}>
          {!isEdit && (
            <><strong style={{ color: 'var(--color-text-secondary)' }}>{selectedIsos.size}</strong> ISO customer{selectedIsos.size === 1 ? '' : 's'} · </>
          )}
        ...
        <Btn variant={willClear ? 'danger' : 'primary'} icon={willClear ? 'x' : 'check'}
          ...
          {willClear ? 'Unpublish'
            ...
            : `Publish to ${selectedIsos.size} ISO customer${selectedIsos.size === 1 ? '' : 's'}`}
        </Btn>
      </>}>
      <div className="stack" style={{ gap: 12 }}>
        <div className="fwdep-help">
          ...
          <span>
            {isEdit
              ? 'Tick the versions this ISO customer can see. Unticking a version unpublishes it from this ISO customer; clearing everything unassigns the ISO customer. Terminals already running a version are unaffected.'
              : 'Pick one or more ISO customers, then tick which firmware versions they can see in their OTA menu. The same version set is published to every selected ISO customer — you can change each one later.'}
          </span>
        </div>
          ...
          {!isEdit && (
            <div style={{ border: '1px solid var(--color-border-default)', borderRadius: 8, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '8px 12px', background: 'var(--color-bg-3)', borderBottom: '1px solid var(--color-border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <span className="overline" style={{ fontSize: 10 }}>ISO customers ({eligibleIsos.length}{selectedIsos.size > 0 ? ` · ${selectedIsos.size} selected` : ''})</span>
              ...
              <div style={{ padding: '8px 10px', borderBottom: '1px solid var(--color-border-subtle)' }}>
                <Input size="sm" prefix={<Icon name="search" size={12}/>} placeholder="Search ISO customers…" value={isoQ} onChange={(e) => setIsoQ(e.target.value)}/>
              </div>
              <div style={{ maxHeight: 300, overflowY: 'auto', flex: 1 }}>
                {eligibleIsos.length === 0 ? (
                  <div style={{ padding: '28px 12px', textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: 12 }}>
                    {undeployedIsos.length === 0 ? 'Every ISO customer already has a deployment. Use Edit on a row to change versions.' : 'No ISO customers match.'}
                  </div>
                ) : pagedIsos.map((iso) => {
                  ...
                  return (
                    <label key={iso.id} className={`fwdep-vrow ${on ? 'is-on' : ''}`}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div className="muted" style={{ fontSize: 11 }}>{iso.country || '—'}</div>
                      </div>
                    </label>
                  );
                })}
                  ...
                  <label key={v.id} className={`fwdep-vrow ${on ? 'is-on' : ''} ${diff} ${isUnpub ? 'is-disabled' : ''}`}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 13, fontWeight: 500 }}>{v.versionName}</span>
                        <span style={{ fontFamily: 'var(--font-family-mono)', fontSize: 10.5, fontWeight: 500, padding: '1px 6px', borderRadius: 999, background: 'var(--color-bg-3)', border: '1px solid var(--color-border-subtle)', color: 'var(--color-text-tertiary)' }}>{v.hardwareId}</span>
                      ...
                      <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>
                        {isUnpub ? (
                          ...
                          <>
                            ...
                            {(v.publishedToIsoIds || []).length} ISO customer{(v.publishedToIsoIds || []).length === 1 ? '' : 's'}
                          </>
                        )}
                      </div>
                    </div>
      ...
      <style>{`
        ...
        .fwdep-link { border: 0; background: transparent; cursor: pointer; font-size: 11.5px; font-weight: 500; color: var(--color-text-secondary); padding: 2px 6px; border-radius: 5px; }
        ...
        .fwdep-tag { font-size: 10.5px; font-weight: 600; padding: 2px 7px; border-radius: 999px; letter-spacing: 0.02em; }
        ...
        .fwdep-pager__label { font-size: 11.5px; color: var(--color-text-tertiary); font-family: var(--font-family-mono); }
        .fwdep-pager__btn {
          display: inline-flex; align-items: center; gap: 4px;
