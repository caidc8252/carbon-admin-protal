...
//   published    — the default the moment a version is uploaded. The version
//                  is available and can be deployed to ISOs from the
//                  Deployments tab. publishedToIsoIds lists the ISOs it's
//                  currently assigned to (may be empty — available but not
//                  yet assigned to anyone).
//   unpublished  — explicitly taken down via the Versions-tab Unpublish
//                  action. Removed from every ISO and NOT selectable in the
//                  Deployments assign/edit modal until re-published.
//
// Publish audience
//   publishedToIsoIds: ['c-xxx', ...]   — the ISO customers this
//                       version is assigned to. Edits append `publishEvents`
...
// A version is 'published' (available — can be deployed to ISOs) the moment
// it's uploaded. It only becomes 'unpublished' (taken down — not deployable)
// when an admin explicitly Unpublishes it from the Versions tab.
...
const __mkVersion = ({ os, modelCode, deviceFlag, name, daysAgo, current, published, sizeMb, fileNameOverride, changelog, audience }) => {
  ...
  // Every uploaded version is published (available) on arrival; the `published`
  // flag here only controls whether the seed gives it an ISO audience.
  ...
  return {
    ...
    publishEvents: audienceIds.length
