...
//     <Pagination …/>
//   </ListCard>
function ListCard({ toolbarRef, children, className = '', style, scrollX = false }) {
  ...
  const decorated = arr.map((child, i) => {
    ...
    // Wrap a wide table in a horizontal-scroll viewport when scrollX is on.
    if (scrollX && child && child.type === 'table') {
      return <div className="list-card__scroll" key={child.key || `t${i}`}>{child}</div>;
    }
    return child;
  });
...
// ─── Hook: column visibility prefs (persisted) ───────────────
// allIds: every column id in canonical display order.
// defaultIds: the default-on set. lockedIds: always-on (can't toggle).
// storageKey: localStorage key for persistence.
function useColumnPrefs({ allIds, defaultIds, lockedIds = [], storageKey }) {
  const lockedSet = new Set(lockedIds);
  const norm = (ids) => {
    const s = new Set(ids);
    lockedIds.forEach((id) => s.add(id));
    return allIds.filter((id) => s.has(id)); // canonical order
  };
  const [visible, setVisible] = React.useState(() => {
    ...
    try {
      ...
      if (Array.isArray(raw)) {
        const valid = raw.filter((id) => allIds.includes(id));
        if (valid.length) init = valid;
      }
    } catch (e) { /* ignore */ }
    return norm(init);
  });
  const apply = (ids) => {
    const ordered = norm(ids);
    setVisible(ordered);
    try { localStorage.setItem(storageKey, JSON.stringify(ordered)); } catch (e) { /* ignore */ }
  };
  const isOn = (id) => visible.includes(id);
  const toggle = (id) => {
    if (lockedSet.has(id)) return;
    apply(isOn(id) ? visible.filter((x) => x !== id) : [...visible, id]);
  };
  const reset = () => apply(defaultIds);
  const selectAll = () => apply(allIds);
  return { visible, isOn, toggle, reset, selectAll, count: visible.length };
}

// ─── ColumnsMenu — eye + "Columns" button with a grouped popover ──
// groups: [{ name, model, items: [{ id, label, locked }] }]. Pair with
// useColumnPrefs for state. Toggling updates the table live (display
// preference, not a search filter).
function ColumnsMenu({ groups, isOn, toggle, reset, selectAll, count, label = 'Columns' }) {
  const [open, setOpen] = React.useState(false);
  const ref = React.useRef(null);
  React.useEffect(() => {
    if (!open) return undefined;
  ...
  return (
    <div ref={ref} className="cols-menu" style={{ position: 'relative' }}>
      <button type="button" className={`cols-menu__btn${open ? ' is-open' : ''}`}
        onClick={() => setOpen((o) => !o)} title="Choose which columns to show">
        {EYE}
        <span className="cols-menu__label">{label}</span>
        <span className="cols-menu__badge">{count}</span>
      </button>
      {open ? (
        <div className="cols-menu__pop" role="menu">
          <div className="cols-menu__head">
            <span className="cols-menu__title">Columns <span>· {count} shown</span></span>
            <span className="cols-menu__link" onClick={selectAll}>Select all</span>
            <span className="cols-menu__link" onClick={reset}>Reset</span>
          </div>
          <div className="cols-menu__body">
            {groups.map((g) => (
              <div key={g.name || 'grp'} className="cols-menu__grp">
                {g.name ? (
                <div className="cols-menu__grp-h">
                  <span className="cols-menu__grp-name">{g.name}</span>
                  {g.model ? <span className="cols-menu__grp-model">{g.model}</span> : null}
                </div>
                ) : null}
                {g.items.map((it) => {
                  const on = isOn(it.id);
                  return (
                    <div key={it.id}
                      className={`cols-menu__opt${on ? ' on' : ''}${it.locked ? ' locked' : ''}`}
                      onClick={() => !it.locked && toggle(it.id)}>
                      <span className="cols-menu__box">{on ? TICK : null}</span>
                      <span className="cols-menu__opt-label">{it.label}</span>
                      {it.locked ? <span className="cols-menu__lock">LOCKED</span> : null}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="cols-menu__foot">Saved to this browser</div>
        </div>
      ) : null}
    </div>
  );
...
/* Search button — ui-spec §4.3 FIX: SECONDARY (not a 2nd primary CTA).
   bg-2 surface + 1px border-default + shadow-1, primary text colour.
   The New X button stays the lone filled primary CTA on the page. */
.sb__primary {
  display: inline-flex;
  align-items: center;
  gap: 6px;
...
.sb__primary:hover { background: var(--color-bg-hover); border-color: var(--color-border-strong, var(--color-border-default)); }
.sb__primary:active { background: var(--color-bg-active, var(--color-bg-hover)); }
.sb__primary:focus-visible {
  outline: none;
  box-shadow: 0 0 0 3px color-mix(in oklab, var(--color-primary-500) 25%, transparent);
}
.sb__primary > svg { color: var(--color-text-tertiary); }
...
/* Horizontal-scroll viewport for wide tables (ListCard scrollX).
   Columns size to their content; the table scrolls inside the card
   while the head strip + pagination stay put. */
.list-card__scroll { overflow-x: auto; overflow-y: visible; }
.list-card__scroll > table.tds-table { width: max-content; min-width: 100%; }
.list-card__scroll > table.tds-table thead th,
.list-card__scroll > table.tds-table tbody td { white-space: nowrap; }
/* Inside a horizontal-scroll viewport the page-relative sticky top no longer
   applies (the wrapper is its own scroll context) — pin the header to the
   wrapper top so it tracks columns horizontally without drifting vertically.
   Extra .list-card prefix raises specificity over the generic thead rule. */
.list-card .list-card__scroll > table.tds-table thead th { position: sticky; top: 0; }
...
.cols-menu__btn {
  ...
  color: var(--color-text-secondary); transition: color .12s, background .12s;
}
.cols-menu__btn:hover { color: var(--color-primary-700); background: color-mix(in oklab, var(--color-primary-500) 8%, transparent); }
.cols-menu__btn.is-open { color: var(--color-primary-700);
  background: color-mix(in oklab, var(--color-primary-500) 10%, transparent); }
.cols-menu__badge { font-size: 10px; font-weight: 600; padding: 0 5px; height: 15px; min-width: 15px;
  display: inline-grid; place-items: center; border-radius: 999px;
  background: var(--color-primary-700); color: #fff; }
.cols-menu__pop {
  position: absolute; top: calc(100% + 6px); right: 0; width: 320px; z-index: 60;
  background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 12px;
  box-shadow: 0 18px 48px -10px oklch(0% 0 0 / 0.22); overflow: hidden;
}
.cols-menu__head { display: flex; align-items: center; gap: 8px; padding: 11px 14px;
  border-bottom: 1px solid var(--color-border-subtle); }
.cols-menu__title { font-size: 13px; font-weight: 600; flex: 1; }
.cols-menu__title span { color: var(--color-text-tertiary); font-weight: 400; }
.cols-menu__link { font-size: 11.5px; color: var(--color-primary-700); cursor: pointer; }
.cols-menu__link:hover { text-decoration: underline; }
.cols-menu__body { max-height: 440px; overflow-y: auto; padding: 6px 0; }
.cols-menu__grp { padding: 8px 12px 2px; }
.cols-menu__grp-h { display: flex; align-items: baseline; gap: 8px; margin-bottom: 4px; }
.cols-menu__grp-name { font-size: 10px; font-weight: 700; letter-spacing: 0.07em; text-transform: uppercase; color: var(--color-text-tertiary); }
.cols-menu__grp-model { font-size: 9.5px; color: var(--color-text-tertiary); opacity: 0.7; font-family: var(--font-family-mono); }
.cols-menu__opt { display: flex; align-items: center; gap: 10px; padding: 7px 8px; border-radius: 7px; cursor: pointer; }
.cols-menu__opt:hover { background: var(--color-bg-3); }
.cols-menu__opt.locked { cursor: not-allowed; }
.cols-menu__box { width: 16px; height: 16px; border-radius: 5px; border: 1.5px solid var(--color-border-strong);
  display: grid; place-items: center; flex: none; color: #fff; }
.cols-menu__opt.on .cols-menu__box { background: var(--color-primary-500); border-color: var(--color-primary-500); }
.cols-menu__opt.locked .cols-menu__box { background: var(--color-text-tertiary); border-color: var(--color-text-tertiary); }
.cols-menu__opt-label { font-size: 12.5px; color: var(--color-text-primary); flex: 1; }
.cols-menu__opt.locked .cols-menu__opt-label { color: var(--color-text-secondary); }
.cols-menu__lock { font-size: 9.5px; color: var(--color-text-tertiary); letter-spacing: 0.04em; }
.cols-menu__foot { padding: 9px 14px; border-top: 1px solid var(--color-border-subtle);
  background: var(--color-bg-1); font-size: 11px; color: var(--color-text-tertiary); }
`;
