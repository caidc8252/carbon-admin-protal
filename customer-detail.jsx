...
const StatusChip = ({ status, effectiveTo, tz }) => {
  ...
  // PILOT contracts get the same EXPIRED derivation. tz = partner timezone.
...
const CompanyInfoView = ({ customer, maskOn, revealed, onToggleReveal }) => {
  ...
  return (
    <div className="kv2-wrap">
      <dl className="kv2">
        <div className="kv2__row"><dt>Name</dt>          <dd>{customer.name}</dd></div>
        <div className="kv2__row"><dt>License</dt>       <dd>{customer.license || notProvided}</dd></div>
        <div className="kv2__row"><dt>Country</dt>       <dd>{customer.country ? countryName(customer.country) : notProvided}</dd></div>
        <div className="kv2__row"><dt>Timezone</dt>      <dd>{customer.timezone ? <code style={{ fontSize: 12, fontFamily: 'var(--font-family-mono)' }}>{customer.timezone}</code> : notProvided}</dd></div>
        <div className="kv2__row"><dt>Contact name</dt>  <dd>{customer.contactName || notProvided}</dd></div>
        <div className="kv2__row"><dt>Phone</dt>         <dd>
          {phone ?
          <>
        ...
        <div className="kv2__row"><dt>Email</dt>         <dd>
          {customer.email ?
          <>
        ...
        <div className="kv2__row"><dt>Registered</dt>    <dd>{fmtDateTime(customer.registeredAt)}</dd></div>
        <div className="kv2__row kv2__row--full"><dt>Address</dt> <dd style={{ whiteSpace: 'pre-line' }}>{customer.address || notProvided}</dd></div>
        <div className="kv2__row kv2__row--full"><dt>Remark</dt>  <dd style={{ whiteSpace: 'pre-line', color: 'var(--color-text-secondary)' }}>{customer.remark || notProvided}</dd></div>
      </dl>
    </div>);
};
...
!!(f?.name?.trim() && f?.country && f?.timezone) &&
  ...
  return (
    <div className="stack" style={{ gap: 16 }}>
      ...
      <div className="form-grid form-grid--2" style={{ gap: 16 }}>
        ...
        <Field label="Timezone" required error={!form.timezone ? 'Timezone is required' : null}>
...
const ContractDetailView = ({
  ...
  // ── Save handlers ────────────────────────────────────────────
  const saveTerm = () => {
    // Stored as a calendar DATE ('YYYY-MM-DD'). Validity is judged against
    // this date in the partner's timezone (see effectiveStatus).
    const nextIso = termValue || null;
  ...
  const SectionHead = ({ title, action }) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-secondary)', textTransform: 'uppercase', letterSpacing: 0.5 }}>{title}</div>
      {action}
    </div>
  );
  ...
  return (
    <div className="stack" style={{ gap: 18 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <div className={`pick-card__icon pick-card__icon--${c.kind.toLowerCase()}`} style={{ width: 44, height: 44, borderRadius: 8, flexShrink: 0 }}>
          <Icon name={iconForKind(c.kind)} size={20}/>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            {c.kind} Contract
            <StatusChip status={c.status} effectiveTo={c.effectiveTo} tz={customer.timezone}/>
          </div>
          <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginBottom: 4 }}>{CONTRACT_INFO[c.kind]?.desc}</div>
          <code style={{ fontSize: 12, color: 'var(--color-text-tertiary)' }}>{c.id || `CT-${c.kind.toUpperCase()}-${String(customer.id || '0000').padStart(4, '0')}`}</code>
        </div>
      </div>
      ...
      {banner && (
        <div style={{ ...bannerStyle, display: 'flex', gap: 10, padding: '12px 14px', borderRadius: 8, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, lineHeight: 1.5, marginTop: 2, color: 'inherit', opacity: 0.85 }}>{banner.msg}</div>
          </div>
        </div>
      )}
      ...
      <div>
        ...
        {entEdit ? (
          ...
          (() => {
            ...
            if (schema.length === 0) {
              return (
                <div style={{ padding: '20px 12px', textAlign: 'center', background: 'var(--color-bg-3)', borderRadius: 8, fontSize: 12, color: 'var(--color-text-secondary)' }}>
                    ...
                    display = arr.length === 0
                      ...
                      : <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 4 }}>
                          {arr.map((id, i) => {
                            ...
                            return <span key={i} className="tds-badge tds-badge--neutral" style={{ padding: '2px 7px', fontSize: 12 }} title={id}>{m ? (m.name || id) : id}</span>;
                          })}
                        </span>;
      ...
      {timeline.length > 0 && (
        <div>
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', fontSize: 14, color: 'var(--color-text-secondary)' }}>
            {timeline.slice().reverse().slice(0, 8).map((ev, idx) => {
              ...
              return (
                <li key={idx} style={{ padding: '8px 0', borderBottom: idx === Math.min(timeline.length, 8) - 1 ? 'none' : '1px solid var(--color-border-subtle)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  {hasTransition && (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, flexShrink: 0, marginTop: 1 }}>
                      {ev.statusFrom
...
const TimelineMarker = ({ x, color, label, date, solid }) => (
  <div style={{ position: 'absolute', left: `${x}%`, top: 0, transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', pointerEvents: 'none' }}>
    ...
    <div style={{ fontSize: 10, color: 'var(--color-text-tertiary)', fontFamily: 'var(--font-family-mono)' }}>{fmtDate(date)}</div>
  </div>
);
  ...
  return (
    ...
    >
      <div className="stack">
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-secondary)', marginBottom: 6 }}>Duration</div>
            ...
            {_PILOT_EXTEND_OPTS.map((opt) => (
              <button
                ...
                style={{
                  ...
                  font: '500 14px var(--font-family-sans)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr', gap: 12, alignItems: 'center', padding: 12, background: 'var(--color-bg-3)', borderRadius: 8 }}>
          <div>
            <div style={{ fontSize: 10, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: 0.08 }}>Current</div>
            <div style={{ fontWeight: 600, marginTop: 4 }}>{curTo ? fmtDate(curTo.toISOString()) : '—'}</div>
          </div>
          <Icon name="arrowR" size={14}/>
          <div>
            <div style={{ fontSize: 10, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: 0.08 }}>After</div>
...
const ConvertPilotModal = ({ target, onClose, onConfirm }) => {
  ...
  const submit = () => {
    ...
    onConfirm({
      entitlements,
      effectiveFrom: effFrom || window.todayDate(),
  ...
  return (
    <Modal
      ...
      {step === 0 && (
        <div className="stack">
          <div style={{ display: 'flex', gap: 12, padding: '14px 16px', background: 'var(--color-warning-50)', border: '1px solid oklch(75% 0.13 80 / 0.3)', borderRadius: 8, color: 'var(--color-warning-700)' }}>
            <Icon name="alert" size={18}/>
            <div>
              <div style={{ fontWeight: 600, fontSize: 14 }}>This is irreversible</div>
              <ul style={{ margin: '6px 0 0 18px', padding: 0, fontSize: 12, lineHeight: 1.5, color: 'oklch(36% 0.07 70)' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, border: '1px solid var(--color-border-default)', borderRadius: 8, fontSize: 14, background: ack ? 'var(--color-primary-50)' : 'transparent', cursor: 'pointer' }}>
      ...
      {step === 1 && (
        <div className="stack">
          {feeFields.length > 0 ? (
            ...
            <div style={{ padding: '14px 16px', background: 'var(--color-bg-3)', borderRadius: 8, fontSize: 14, color: 'var(--color-text-secondary)' }}>
              No fee-bearing clauses on {target.kind} contracts — just confirm the effective dates below.
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <Field label="Effective from date" hint="Defaults to today.">
              <Input type="date" value={effFrom} onChange={(e) => setEffFrom(e.target.value)}/>
            </Field>
            <Field label="Effective to date (optional)" hint="Leave blank for no end date.">
...
const TabContracts = ({ customer, onAdd, onRemove, onStatusChange, onUpdateContract }) => {
  ...
  const onToggleMgmtPick = (kind) => {
    ...
    setMgmtPicked((p) => {
      ...
      setMgmtTerms((t) => ({ ...t, [kind]: { effectiveFrom: window.todayDate(customer.timezone), effectiveTo: null } }));
      return [...p, kind];
    });
    setMgmtFocused(kind);
  };
        ...
        <div>
          ...
          liveContracts.map((c, i) => {
            ...
            return (
              ...
              >
                <div className={`pick-card__icon pick-card__icon--${c.kind.toLowerCase()}`} style={{ width: 40, height: 40, borderRadius: 8 }}>
                ...
                <div className="crow__actions">
                  ...
                  {(window.effectiveStatus(c, customer.timezone) === 'PILOT'
                    || (window.effectiveStatus(c, customer.timezone) === 'EXPIRED' && String(c.status).toUpperCase() === 'PILOT')) && (
                    <>
            ...
            <div>
              {historyContracts.map((c, i) => (
                <div key={i} className="crow" style={{ opacity: 0.7 }}>
                  <div className={`pick-card__icon pick-card__icon--${c.kind.toLowerCase()}`} style={{ width: 40, height: 40, borderRadius: 8, filter: 'grayscale(0.6)' }}>
                    <Icon name={iconForKind(c.kind)} size={18} />
                  </div>
                  <div className="crow__main">
      ...
      <ConvertPilotModal
        ...
        onConfirm={({ entitlements, effectiveFrom, effectiveTo }) => {
          ...
          onUpdateContract(
            ...
            {
              status: 'ACTIVE',
              effectiveFrom: effectiveFrom || window.todayDate(customer.timezone),
            ...
            audit,
            {
              ...
              eventInfo: {
                ...
                newEffectiveFrom: effectiveFrom || window.todayDate(customer.timezone),
              },
            }
          );
    ...
    <Modal open={open} onClose={onClose} title="Admin invite link" width={580}
      ...
      {freshlyMinted &&
        <div style={{
          ...
          borderRadius: 8, fontSize: 12, color: 'var(--color-success-700)'
        }}>
      ...
      <div className="stack" style={{ gap: 16, marginBottom: 18 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 6, color: 'var(--color-text-secondary)' }}>
            Invite link
          </div>
        ...
        <div className="qr-block">
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 4 }}>Or scan the QR code</div>
...
const SendInviteEmailDialog = ({ open, onClose, onSend, invite, customer }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Send activation email" width={480}
      ...
      <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', lineHeight: 1.6, marginBottom: 14 }}>
  ...
  return (
    <Modal open={open} onClose={onClose} title="Regenerate activation link?" width={480}
      ...
      <div style={{
        ...
        borderRadius: 8, fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.55
      }}>
...
const OperatorMenu = ({ op, pendingEmailReq, pendingPromoteReq, me, onAction, onWithdraw }) => {
  ...
  // ─── Pending-aware menu items ─────────────────────────
  // For "Change email" and "Promote to Admin", show the request status when one
  // is pending. Only the submitter can withdraw; other admins see a disabled
  // row with a "Pending" badge. The full approve/reject flow lives on the
  // Approvals page; this menu is purely for submitting + cancelling.
  const renderEmailItem = () => {
    ...
    return (
      <button className="opmenu__item" disabled
        title={`Submitted by ${window.awPersonaName ? window.awPersonaName(pendingEmailReq.submittedBy) : pendingEmailReq.submittedBy} — only the submitter can cancel`}>
        <Icon name="mail" size={14} />{OP_ACTIONS['edit-email'].label}
        <span className="opmenu__pendmark">Pending</span>
      </button>);
  };
  const renderPromoteItem = () => {
    if (!pendingPromoteReq) {
      return (
    ...
    return (
      <button className="opmenu__item" disabled
        title={`Submitted by ${window.awPersonaName ? window.awPersonaName(pendingPromoteReq.submittedBy) : pendingPromoteReq.submittedBy} — only the submitter can cancel`}>
        <Icon name="shield" size={14} />{OP_ACTIONS.promote.label}
        <span className="opmenu__pendmark">Pending</span>
      </button>);
  };
        ...
        <tbody>
          ...
          <tr key={row.key} onClick={() => onOpenOrder(row.order.id)} style={{ cursor: 'pointer' }}>
              <td style={{ whiteSpace: 'nowrap' }}>
                {row.dev.sn ?
              <div style={{ fontFamily: 'var(--font-family-mono)', fontSize: 14, fontWeight: 500 }}>{row.dev.sn}</div> :
              <div className="muted" style={{ fontSize: 12 }}>— not assigned —</div>}
              </td>
              <td>
                <div style={{ fontSize: 14, fontWeight: 500 }}>{row.item.modelName}</div>
                <div className="cust-meta">{row.dev.type || row.item.type}</div>
              </td>
              <td style={{ whiteSpace: 'nowrap' }}>
                {row.dev.code ?
              <code style={{ fontFamily: 'var(--font-family-mono)', fontSize: 14, letterSpacing: 1 }}>{row.dev.code}</code> :
              ...
              <td style={{ whiteSpace: 'nowrap' }}>
                <div style={{ fontFamily: 'var(--font-family-mono)', fontSize: 12, color: 'var(--color-primary-600)' }}>{row.order.number}</div>
...
const OrdersView = ({ orders, customer, onOpenOrder, onNewOrder }) => {
  ...
  <span className="tds-table__sort" onClick={() => {
    ...
    {setSortBy(key);setSortDir('asc');}
      ...
      {label}
      <span style={{ opacity: sortBy === key ? 1 : 0.3, fontSize: 10 }}>{sortBy === key && sortDir === 'asc' ? '▲' : '▼'}</span>
    </span>;
          ...
          pageRows.map((o) => {
            ...
            return (
              <tr key={o.id} onClick={() => onOpenOrder(o.id)} style={{ cursor: 'pointer' }}>
                <td style={{ whiteSpace: 'nowrap' }}>
                  <div style={{ fontFamily: 'var(--font-family-mono)', fontSize: 14, fontWeight: 500 }}>{o.number}</div>
                </td>
                <td>
                  {o.items.slice(0, 2).map((i, idx) =>
                  <div key={idx} style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>
                ...
                <td style={{ whiteSpace: 'nowrap' }}>
                  <div style={{ fontSize: 14 }}>{fmtDate(o.createdAt)}</div>
          ...
          {visibleSystem.map((r) => {
            ...
            return (
              <div key={r.id} className={`crow cust-role-row ${isDenied ? 'is-denied' : ''}`}>
                <div className="pick-card__icon" style={{ width: 40, height: 40, borderRadius: 8, background: 'var(--color-bg-3)', color: 'var(--color-text-secondary)' }}>
      ...
      <div className="info-card">
        ...
        <div>
          ...
          <div key={r.id} className="crow">
              <div className="pick-card__icon" style={{ width: 40, height: 40, borderRadius: 8, background: 'var(--color-primary-50)', color: 'var(--color-primary-700)' }}>
          ...
          return (
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
                  <span className="muted" style={{ fontSize: 12 }}>Effective on this customer:</span>
...
const TransferAdminModal = ({ open, onClose, operators, onTransfer }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Transfer Admin role" width={560}
      ...
      {admin &&
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>{admin.email}</div>
          </div>
      ...
      {candidates.length === 0 ?
        <div style={{ padding: '16px 14px', background: 'var(--color-bg-3)', border: '1px solid var(--color-border-subtle)', borderRadius: 8, fontSize: 14, lineHeight: 1.55, color: 'var(--color-text-secondary)' }}>
        ...
        <>
            ...
            {candidates.map(({ op, idx }) => {
              ...
              return (
                <button
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 500 }}>{op.name || <span className="muted" style={{ fontStyle: 'italic', fontWeight: 400 }}>Not yet provided</span>}</div>
                    <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>{op.email}</div>
                  </div>
          ...
          {target &&
            <div style={{ marginTop: 14, padding: '10px 12px', background: 'var(--color-warning-50, var(--color-bg-3))', border: '1px solid var(--color-warning-200, var(--color-border-subtle))', borderRadius: 8, fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.5 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
...
const TabOperators = ({ customer, onUpdate, maskOn, setMaskOn }) => {
  ...
  // ─── Approval-flow modals (Change email + Promote require second-admin review) ───
  const [emailSubmit, setEmailSubmit] = useState(null);   // { op, idx }
  const [promoteSubmit, setPromoteSubmit] = useState(null); // { op, idx }
  const [withdrawTarget, setWithdrawTarget] = useState(null); // approval req object
  // Subscribe to the global AW store so this screen re-renders when requests
  // change (e.g. an admin on the Approvals page approves/rejects, or another
  // tab submits). Reads via window.AW.state.
  const awStoreSnap = (window.useApprovalStore || (() => ({})))();
  const aw = window.AW;
  const me = awStoreSnap.persona || aw?.state.persona;
  const pendingFor = (opId, type) =>
    aw ? aw.pendingFor(customer.id, opId, type) : null;
  ...
  const handleOpAction = (op, idx, action) => {
    // Promote and Change email now go through the approval workflow.
    if (action === 'promote')    { setPromoteSubmit({ op, idx }); return; }
    if (action === 'edit-email') { setEmailSubmit({ op, idx }); return; }
  ...
  return (
        ...
        customer.operators.length === 0 ?
          <div className="empty" style={{ padding: '40px 24px', textAlign: 'center' }}>
            ...
            <>
                ...
                <div style={{ fontSize: 12, color: 'var(--color-text-tertiary)', maxWidth: 420, marginInline: 'auto', lineHeight: 1.5 }}>
                ...
                return (
                  <tr key={i} style={{
                    ...
                    <td style={isLocked ? { opacity: 0.78 } : undefined}>
                      ...
                      {pendEmail && window.PendingChip && (
                        <div style={{ marginTop: 4 }}>
                          <window.PendingChip req={pendEmail} me={me} label="Email change pending review" onWithdraw={(r) => setWithdrawTarget(r)} />
                        </div>)}
                    </td>
                    <td style={isLocked ? { opacity: 0.78 } : undefined}>
                      {isAdmin ?
                      ...
                      {pendPromote && window.PendingChip && (
                        <div style={{ marginTop: 4 }}>
                          <window.PendingChip req={pendPromote} me={me} label="Promote pending review" onWithdraw={(r) => setWithdrawTarget(r)} />
                        </div>)}
                    </td>
                    <td style={isLocked ? { opacity: 0.78 } : undefined}>
                      <span className="num" style={{ fontSize: 14 }}>{op.lastLogin ? relTime(op.lastLogin) : <span className="muted">Never</span>}</span>
                    </td>
    ...
    {/* ─── Approval-workflow modals (Change email + Promote require review) ─── */}
    {emailSubmit && window.SubmitEmailModal &&
      <window.SubmitEmailModal
        open
        op={emailSubmit.op}
        idx={emailSubmit.idx}
        customer={customer}
        me={me}
        onClose={() => setEmailSubmit(null)}
        onSubmitted={() => {
          setEmailSubmit(null);
          toast({ kind: 'success', title: 'Request submitted for review', msg: 'Another Carbon admin will review and apply this change.' });
        }} />}
    {promoteSubmit && window.SubmitPromoteModal &&
      <window.SubmitPromoteModal
        open
        op={promoteSubmit.op}
        idx={promoteSubmit.idx}
        customer={customer}
        me={me}
        onClose={() => setPromoteSubmit(null)}
        onSubmitted={() => {
          setPromoteSubmit(null);
          toast({ kind: 'success', title: 'Request submitted for review', msg: 'Another Carbon admin will review and apply this change.' });
        }} />}
    <Modal open={!!withdrawTarget} onClose={() => setWithdrawTarget(null)} title="Withdraw request?" width={420}
      footer={<>
        ...
        <Btn variant="danger" icon="x" onClick={() => {
          if (withdrawTarget && window.AW) window.AW.withdraw(withdrawTarget.id, me);
          setWithdrawTarget(null);
          toast({ kind: 'info', title: 'Request withdrawn' });
        }}>Withdraw</Btn>
      </>}>
      <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55 }}>
        This will close your request without applying any changes. You can re-submit later if needed.
      </p>
    </Modal>
    </>);
  ...
  return (
    <div className="page">
      ...
      <div className="det-header">
        <button type="button" className="det-back" onClick={onBack} aria-label="Back to customers" title="Back to customers"><Icon name="chevL" size={18} /></button>
