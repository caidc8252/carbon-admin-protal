...
(function buildFactoryImageSeed() {
  ...
  const seeds = [
    // ── 1 — Active baseline, mixed models, mostly system apps ──
    {
      ...
      isoCustomerIds: [isos[0]].filter(Boolean),
      payload: {
        byModel: {
          'N950': {
            firmwareRefs:  [__fiPickFw('N950', 'A12', 'V2.3.1'), __fiPickFw('N950', 'A10', 'V2.3.1')].filter(Boolean),
            systemAppRefs: [{ appId: 'inventory', versionId: 'v12' }, { appId: 'catalog', versionId: 'v09' }, { appId: 'timeclock', versionId: 'v15' }],
    ...
    // ── 2 — Active, single model, large fleet ──
    {
      ...
      isoCustomerIds: [isos[2]].filter(Boolean),
      payload: {
        byModel: {
          'S30': {
    ...
    // ── 4 — Multi-model image ──
    {
      ...
      isoCustomerIds: [isos[1]].filter(Boolean),
      payload: {
        byModel: {
          'N950': {
    ...
    // ── 5 — Archived ──
    {
      ...
      isoCustomerIds: [isos[4]].filter(Boolean),
      payload: {
        byModel: {
          'S60': {
...
// ── Resolution: "system apps" are bound from the App Publish catalog ──
// The App Publish menu (formerly the "System Apps" menu) owns this catalog.
// It is exposed by the App Publish bundle at window.AP.APPS. Centralized here
// so a future move/rename of that menu is a one-line change.
const factoryImageSystemAppCatalog = () => (window.AP && window.AP.APPS) || [];

// Resolve a {appId, versionId} ref against the App Publish catalog.
