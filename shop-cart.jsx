/* global React, Btn, Icon, Badge, ModelTile, DEVICE_MODELS, useToast,
   CATEGORY_BADGE, resolveVariantLabel */
// ─── Shop Cart ───────────────────────────────────────────────
...
// (Legacy fulfillment grouping removed — the new data model has no
//  sample/production attribute on products; sample vs product lives on the
//  ORDER as order_type, chosen at checkout.)

// ─── CartTrigger (top-bar button) ─────────────────────────────
const CartTrigger = ({ totalQty, subtotal, onOpen }) => (
  <button type="button" className="tds-btn tds-btn--secondary tds-btn--md cart-trigger" onClick={onOpen} title="Open cart" aria-label="Open cart">
...
const CartVariantPopup = ({ line, product, variant, anchorRect, onClose, onSave, onAddNew, otherLinesInCart = [] }) => {
  ...
  // Sample/production is an ORDER attribute now (order_type at checkout) —
  // products carry no integration mode, so this path is inert.
  const isSample = false;
  const isIntAxis = false;
  const standaloneIntModes = [];
              ...
              {activeVariants.map((v) => {
                ...
                return (
                  <button key={v.id} type="button"
                    className={`cart-pop__chip cart-pop__chip--variant ${active ? 'is-active' : ''}`}
          ...
          <section className="cart-pop__section">
            ...
            <div className="cart-pop__chips">
              {activeVariants.map((v) => {
                ...
                const title = inCart ? 'Already in cart' : undefined;
                return (
                  <button key={v.id} type="button"
                    disabled={disabled}
                    title={title}
                    className={`cart-pop__chip cart-pop__chip--variant ${active ? 'is-active' : ''} ${disabled ? 'is-disabled' : ''}`}
        ...
        <div className="cart-drawer__body">
          {items.length === 0 ? (
            <div className="cart-empty">
              ...
              <div className="cart-empty__sub">Browse the Catalog to add items.</div>
            </div>
          ) : (
            <div className="cart-group">
              {items.map(({ line, product, variant }) => {
                    ...
                    const specs = product.specs || [];
                    const primaryAxis = specs[0] || null;
                    const isSampleRow = false;
                    const isIntAxis = false;
                    const activeVariants = product.variants.filter((v) => v.status === 'ACTIVE');
                    const intModesList = [];
                    // Integration mode dropped — sample/production lives on the ORDER.
                    const standaloneIntModes = [];
...
// ─── Styles ────────────────────────────────────────────────────
const cartStyles = `
.cart-trigger { gap: 8px; }
.cart-trigger__lbl { font-weight: 500; }
...
Object.assign(window, { useCart, CartTrigger, CartDrawer, resolveLine });
