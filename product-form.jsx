    ...
    return (
      <div className="prod-img-picker">
        <div className={`prod-img-picker__empty ${disabled ? 'is-disabled' : ''}`} onClick={() => canAdd && ref.current?.click()}>
          ...
          <div style={{ fontSize: 14, fontWeight: 500, marginTop: 6 }}>Click to upload images</div>
          <div className="muted" style={{ fontSize: 12, marginTop: 3 }}>PNG / JPG / WebP · Up to {max} · 1:1 recommended</div>
        </div>
        {fileInput}
      </div>
    );
...
// ─── Main form ──────────────────────────────────────────────────
const ProductForm = ({ initial, onCancel, onSave, onDelete }) => {
  ...
  const filteredModels = (allowedModels.length > 8 && modelQuery.trim())
    ? allowedModels.filter((m) =>
        `${m.name} ${m.family || ''}`.toLowerCase().includes(modelQuery.toLowerCase().trim()))
    : allowedModels;
  ...
  return (
    <div className="page">
      {wizard ? (
        ...
        <div className="page__head prod-edit-sticky">
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flex: '1 1 0%', minWidth: 0 }}>
            <button type="button" className="det-back" onClick={onCancel} aria-label="Back to Catalog" title="Back to Catalog">
              <Icon name="chevL" size={18}/>
            </button>
            <window.CompanyLogo name={p.name} size={56} />
            <div className="det-header__main">
              <h1 className="det-header__title">
                <span className="det-header__name">{p.name}</span>
                <Badge tone={PRODUCT_STATUS_TONE[eff]} dot>{PRODUCT_STATUS_LABEL[eff]}</Badge>
              </h1>
              <div className="det-header__meta">
                <span><Icon name="clock" size={13}/> Created {new Date(p.createdAt).toLocaleDateString()}</span>
                <span>Last updated {new Date(p.updatedAt).toLocaleDateString()} by {p.updatedBy}</span>
              </div>
            </div>
          </div>
      ...
      <div className="wizard-grid">
        <div className="stack" style={{ gap: 16 }}>
          ...
          {/* Specifications (optional · both types) */}
          {vis('specs') && (
            <div className="info-card">
              <div className="info-card__head">
                <div>
          ...
          <div className="info-card">
            ...
            <div className="info-card__body">
              <dl className="prod-review">
                ...
                {isDevice && <><dt>Device model</dt><dd>{linkedModel ? linkedModel.name : '—'}</dd></>}
          ...
          <div className="prod-preview">
            <div className="prod-preview__image">
              {p.baseImage ? (
                ...
                <window.CompanyLogo name={p.name || ''} size={140}/>
              )}
            </div>
            <div className="prod-preview__name">{p.name || 'Untitled product'}</div>
            {isDevice && linkedModel && (
              <div className="prod-preview__cat" style={{ fontSize: 12, color: 'var(--color-text-tertiary)' }}>
      ...
      {delistOpen && (
        <window.Modal open={true} onClose={closeDelist} width={480}
          ...
          <div style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--color-text-secondary)', marginBottom: 14 }}>
...
.pf-delist-input { width: 100%; padding: 9px 12px; border: 1px solid var(--color-border-default); border-radius: 8px; background: var(--color-bg-1); color: var(--color-text-primary); font-size: 14px; font-family: inherit; transition: border-color 120ms; }
...
.prod-banner { display: flex; align-items: center; gap: 10px; padding: 10px 14px; border-radius: 8px; font-size: 14px; margin-bottom: 18px; }
...
.prod-section-sublabel { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-tertiary); margin-bottom: 8px; }
...
.prod-cat-card { display: flex; flex-direction: column; align-items: flex-start; gap: 4px; padding: 14px 16px; background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 8px; cursor: pointer; text-align: left; font-family: inherit; transition: all var(--duration-fast); }
...
.prod-cat-card__label { font-size: 14px; font-weight: 600; color: var(--color-text-primary); }
.prod-cat-card__hint { font-size: 12px; color: var(--color-text-tertiary); line-height: 1.4; }
...
.prod-model-search input { flex: 1; border: 0; background: transparent; outline: 0; font: inherit; font-size: 14px; color: var(--color-text-primary); }
...
.prod-model-tile { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 10px 6px; background: var(--color-bg-2); border: 1.5px solid var(--color-border-default); border-radius: 8px; cursor: pointer; font-family: inherit; transition: all var(--duration-fast); }
...
.prod-model-tile__name { font-size: 14px; font-weight: 600; color: var(--color-text-primary); }
.prod-model-tile__family { font-size: 10px; color: var(--color-text-tertiary); }

.prod-chk-row { display: flex; flex-wrap: wrap; gap: 8px; }
.prod-chk { display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 999px; cursor: pointer; font-size: 12px; transition: all var(--duration-fast); }
...
.prod-variant-pick__opt { display: flex; align-items: flex-start; gap: 10px; padding: 10px 12px; background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 8px; cursor: pointer; transition: all var(--duration-fast); }
...
.prod-variant-pick__lbl { font-size: 14px; font-weight: 600; color: var(--color-text-primary); }
.prod-variant-pick__hint { font-size: 12px; color: var(--color-text-tertiary); margin-top: 2px; line-height: 1.4; }
...
.prod-spec-row__lbl { font-size: 12px; font-weight: 600; color: var(--color-text-primary); width: 110px; flex: none; }

.prod-empty { padding: 18px; text-align: center; color: var(--color-text-tertiary); border: 1.5px dashed var(--color-border-default); border-radius: 8px; font-size: 12px; background: var(--color-bg-3); margin-bottom: 10px; }

.prod-axis-editor { background: var(--color-bg-3); border: 1px solid var(--color-border-subtle); border-radius: 8px; padding: 12px 14px; margin-bottom: 10px; }
.prod-axis-editor__head { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }
.prod-axis-editor__name { flex: 1; border: 1px solid transparent; background: transparent; padding: 6px 8px; border-radius: 6px; font: inherit; font-size: 14px; font-weight: 600; color: var(--color-text-primary); outline: 0; }
...
.prod-axis-chip input { border: 0; background: transparent; outline: 0; font: inherit; font-size: 12px; color: var(--color-text-primary); padding: 3px 0; width: 110px; }
...
.prod-axis-add input { border: 1px dashed var(--color-border-default); background: transparent; border-radius: 8px; padding: 7px 12px; font: inherit; font-size: 12px; color: var(--color-text-secondary); outline: 0; width: 140px; }
...
.prod-preset-row__lbl { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-tertiary); margin-right: 4px; }
...
.prod-preset-btn__hint { font-size: 12px; color: var(--color-text-tertiary); }
...
.prod-base-price__lbl { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-tertiary); }

.prod-variant-table { width: 100%; border-collapse: collapse; }
.prod-variant-table th { text-align: left; padding: 10px 12px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-tertiary); background: var(--color-bg-3); border-bottom: 1px solid var(--color-border-default); }
...
.prod-cell-input { width: 100%; border: 1px solid var(--color-border-default); border-radius: 6px; background: var(--color-bg-2); padding: 5px 8px; font: inherit; font-size: 12px; color: var(--color-text-primary); outline: 0; }
...
.prod-cell-money > span { font-size: 12px; color: var(--color-text-tertiary); }
...
.prod-stock-toggle { display: inline-flex; align-items: center; gap: 4px; font-size: 12px; color: var(--color-text-secondary); cursor: pointer; white-space: nowrap; }
.prod-stock-toggle input[type="checkbox"] { accent-color: var(--color-primary-700); }

.prod-img-picker__empty { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; padding: 36px 12px; border: 1.5px dashed var(--color-border-default); border-radius: 8px; background: var(--color-bg-3); cursor: pointer; transition: all var(--duration-fast); color: var(--color-text-secondary); }
...
.prod-img-picker__has img { width: 110px; height: 110px; object-fit: contain; background: #fff; border: 1px solid var(--color-border-default); border-radius: 8px; padding: 6px; }
.prod-img-picker__fallback { width: 110px; height: 110px; display: grid; place-items: center; background: #fff; border: 1px dashed var(--color-border-default); border-radius: 8px; padding: 6px; }
.prod-img-picker__fallback-lbl { font-size: 12px; color: var(--color-text-tertiary); line-height: 1.5; margin-right: 6px; }
...
.prod-img-cell img { width: 110px; height: 110px; object-fit: contain; background: #fff; border: 1px solid var(--color-border-default); border-radius: 8px; padding: 6px; }
.prod-img-cell__badge { position: absolute; left: 6px; top: 6px; font: 600 10px var(--font-family-mono); letter-spacing: 0.04em; text-transform: uppercase; padding: 2px 6px; border-radius: 4px; background: var(--color-primary-700); color: #fff; }
...
.prod-img-add { width: 110px; height: 110px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; border: 1.5px dashed var(--color-border-default); border-radius: 8px; background: var(--color-bg-2); color: var(--color-text-secondary); cursor: pointer; font: 500 12px inherit; }
.prod-img-add:hover { border-color: var(--color-border-strong); color: var(--color-text-primary); }
.prod-img-hint { font-size: 12px; margin-top: 8px; }

.prod-radio-group { display: flex; flex-direction: column; gap: 8px; }
.prod-radio { display: flex; align-items: center; gap: 8px; font-size: 14px; color: var(--color-text-primary); cursor: pointer; }
.prod-radio input[type="radio"] { accent-color: var(--color-primary-700); }
.prod-dt { margin-left: 8px; padding: 5px 8px; border-radius: 6px; border: 1px solid var(--color-border-default); background: var(--color-bg-2); font: inherit; font-size: 12px; color: var(--color-text-primary); }
.prod-dt:focus { border-color: var(--color-primary-700); outline: 0; }

.prod-toggle-line { display: flex; align-items: flex-start; gap: 8px; font-size: 14px; cursor: pointer; padding: 10px 12px; background: var(--color-bg-3); border: 1px solid var(--color-border-subtle); border-radius: 8px; }
...
.prod-preview__image img { max-width: 140px; max-height: 140px; object-fit: contain; background: #fff; border-radius: 8px; padding: 6px; border: 1px solid var(--color-border-subtle); }
.prod-preview__name { text-align: center; font-weight: 600; font-size: 16px; letter-spacing: -0.01em; }
...
.prod-preview__chip { font-size: 12px; padding: 2px 8px; background: var(--color-bg-3); color: var(--color-text-secondary); border-radius: 999px; border: 1px solid var(--color-border-subtle); }
.prod-preview__variants { text-align: center; margin-top: 6px; font-size: 12px; color: var(--color-text-tertiary); }
...
.prod-edit-sticky { position: sticky; top: 0; z-index: 18; }
.prod-back { display: inline-flex; align-items: center; gap: 4px; background: transparent; border: 0; padding: 4px 8px; margin: 0 0 8px -8px; border-radius: 6px; cursor: pointer; font: 500 12px var(--font-family-sans); color: var(--color-text-secondary); }
...
.prod-review dd { font-size: 14px; color: var(--color-text-primary); }
`;
