...
function Sidebar({ route, navigate }) {
  ...
  const sections = [
    ...
    // Devices section is ISO-only — Sample tools moved to Develop.
    ...(hasISO ? [{
      title: "Devices", items: [
        { id: "devices", icon: "device", label: "Devices" },
      ],
    }] : []),
    // Develop tooling is ISV-only — sample devices + sample orders let
    // an ISV team try their builds without a real merchant fleet.
    ...(hasISV ? [{
      title: "Develop", items: [
