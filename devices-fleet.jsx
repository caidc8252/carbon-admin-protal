          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
            {[
              ...
              <div key={k.label} style={{
                ...
                <div className="overline" style={{ fontSize: 10 }}>{k.label}</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--fg3)" }}>{k.sub}</span>
                </div>
              </div>
            ))}
          </div>
                    ...
                    return (
                      <tr key={d.sn}
                        ...
                        <td style={{ padding: "12px 14px" }}>
                          <span className="mono" style={{ fontSize: 12, fontWeight: 500 }}>{d.sn}</span>
                        </td>
                        <td style={{ padding: "12px 14px" }}>
    ...
    <div style={{ height: "100%", overflow: "auto", background: "var(--color-bg-1)" }}>
      <div className="page">
        ...
        <div className="det-header">
          <button type="button" className="det-back" onClick={() => navigate({ screen: "devices" })} aria-label="Back to Devices" title="Back to Devices"><window.Ico name="chevL" size={18} /></button>
          <DeviceModelTile model={device.model} size={56} />
          <div className="det-header__main">
            <h1 className="det-header__title" style={{ fontFamily: "var(--font-family-mono)" }}>
              <span className="det-header__name">{device.sn}</span>
              ...
              {(() => {
                ...
                return (
                  <span style={{
                    display: "inline-flex", alignItems: "stretch", height: 24,
                    borderRadius: 999, overflow: "hidden",
                    border: `1px solid ${hasOpen ? "color-mix(in oklab, var(--color-warning-500) 32%, transparent)" : "var(--color-border-default)"}`,
                    whiteSpace: "nowrap",
                  }}>
                    <button type="button" onClick={() => navigate({ screen: "tickets" })}
                      title={hasOpen ? `${openTickets.length} open ticket${openTickets.length === 1 ? "" : "s"} — view all` : "No open tickets — view all"}
                      style={{
                        display: "inline-flex", alignItems: "center", gap: 5,
                        padding: "0 10px",
                        background: hasOpen ? "var(--warning-bg)" : "var(--bg2)",
                        color: hasOpen ? "var(--color-warning-700)" : "var(--fg2)",
                        border: 0, cursor: "pointer", fontSize: 12, fontWeight: hasOpen ? 500 : 400, fontFamily: "inherit",
                      }}>
                      <window.Ico name="alert" size={11} stroke={hasOpen ? 2 : 1.6} style={hasOpen ? undefined : { opacity: 0.6 }} />
                      {hasOpen ? `${openTickets.length} open` : "No open tickets"}
                    </button>
                    <button type="button" onClick={() => navigate({ screen: "newTicket", deviceSn: device.sn })}
                      title="Open a new ticket for this device"
                      style={{
                        display: "inline-flex", alignItems: "center", justifyContent: "center",
                        padding: "0 8px", background: "var(--bg2)", color: "var(--fg2)",
                        border: 0,
                        borderLeft: `1px solid ${hasOpen ? "color-mix(in oklab, var(--color-warning-500) 32%, transparent)" : "var(--color-border-default)"}`,
                        cursor: "pointer", transition: "background .12s ease, color .12s ease",
                      }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = "color-mix(in oklab, var(--color-primary-500) 10%, transparent)"; e.currentTarget.style.color = "var(--color-primary-700)"; }}
        ...
        <div className="det-tabs">
          {tabs.map(t => (
            <button key={t.id} className={`tds-tab ${tab === t.id ? "tds-tab--active" : ""}`}
    ...
    <div className="device-basic">
      ...
      <div className="device-basic__row-3">
        <window.Card title="Device">
          <KvTable rows={[
            ["Manufacturer",   <span style={{ fontSize: 14 }}>{meta.manufacturer}</span>],
            ["Part number",    <span className="mono" style={{ fontSize: 14, fontWeight: 500 }}>{meta.pn}</span>],
            ["IMEI",           device.imei ? <span className="mono" style={{ fontSize: 14 }}>{device.imei}</span> : <Dash />],
            ["Record created", <span className="mono" style={{ fontSize: 14 }}>{device.createdAt}</span>],
            ["Activated",      device.activatedAt
                ? <span className="mono" style={{ fontSize: 14 }}>{device.activatedAt}</span>
                : <span style={{ fontSize: 12, color: "var(--color-warning-700)" }}>Pending</span>],
          ]} />
        </window.Card>
          ...
          <KvTable rows={[
            ["Merchant",  merchant
                ? <a href="#" onClick={(e) => { e.preventDefault();
                    ...
                    style={{ fontSize: 14, color: "var(--accent)", textDecoration: "underline", textUnderlineOffset: 2 }}>
                    {merchant.name}
                  </a>
                : <Dash />],
            ["Store",     store
            ...
            ["Address",   store?.address
                ? <span style={{ fontSize: 12, color: "var(--fg2)", lineHeight: 1.45 }}>{store.address}</span>
                : <Dash />],
            ["Last seen", <span className="mono" style={{ fontSize: 14 }}>{lastSeenTimestamp(device.lastSeenAt) || device.lastSeenAt}</span>],
          ]} />
        </window.Card>
      ...
      <div className="device-basic__row-2">
        <window.Card title="System">
          <KvTable rows={[
            ["OS version",     <span className="mono" style={{ fontSize: 14 }}>{device.os}</span>],
            ["Firmware",       <span className="mono" style={{ fontSize: 14 }}>{device.firmware}</span>],
            ["Last boot",      meta.lastBoot
                ? <span className="mono" style={{ fontSize: 14 }}>{meta.lastBoot}</span>
                : <span style={{ fontSize: 12, color: "var(--fg3)" }}>Never booted</span>],
            ["Timezone",       <span className="mono" style={{ fontSize: 14 }}>{device.settings.timezone}</span>],
            ["Language",       <span className="mono" style={{ fontSize: 14 }}>{device.settings.language}</span>],
        ...
        <window.Card title="Hardware posture"
          ...
          <KvTable rows={[
            ["MAC address",    device.macAddress ? <span className="mono" style={{ fontSize: 14 }}>{device.macAddress}</span> : <Dash />],
            ["Config code",    <span className="mono" style={{ fontSize: 14 }}>{meta.cfg}</span>],
            ["Hardware ID",    <span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{meta.hwid}</span>],
            ["Root status",
          ...
          {warnings.length > 0 && (
            <div style={{
              ...
              <div className="overline" style={{
                fontSize: 10, color: "var(--color-warning-700)", marginBottom: 6,
              }}>Warnings</div>
...
function HeaderStats({ device }) {
  ...
  const subStyle = { fontSize: 10, color: "var(--fg3)", whiteSpace: "nowrap" };
      ...
      <div style={cell}>
        ...
        <span style={{ ...valueStyle,
          ...
          {device.storage
            ...
            : "—"}
          {storagePct != null && <span style={{ fontSize: 10, color: "var(--fg3)", fontWeight: 400, marginLeft: 4 }}>GB</span>}
        </span>
...
function LocationMap({ coords, address, state, lastSeen }) {
  ...
  const Tile = ({ label, value, sub, mono = true }) => (
    <div style={{
      ...
      {sub && <span style={{ fontSize: 12, color: "var(--fg3)",
    ...
    <window.Card title="SIM data usage"
      ...
      {!open && (
        <div style={{
          ...
          padding: "2px 0", fontSize: 12, color: "var(--fg3)",
        }}>
          <span>
      ...
      <div style={{ display: "grid",
        ...
        <div>
          ...
          <div style={{ marginTop: 4, display: "flex", alignItems: "baseline", gap: 4 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--fg3)" }}>of {fmtMb(capMb)}</span>
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 14, color: "var(--fg1)" }}>
            {daysToCap >= 30
              ? <>Won't reach cap</>
              : daysToCap <= 0
                ? <span style={{ color: "var(--color-error-700)", fontWeight: 500 }}>Cap reached</span>
...
function StatTile({ label, value, sub, tone, mono = true, valueFont }) {
  ...
  return (
    <div style={{
      ...
      {sub && <div style={{
        marginTop: 2, fontSize: 12, color: "var(--fg3)",
        whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
      }}>{sub}</div>}
    </div>
  );
    ...
    <div style={{
      ...
      <div style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: 4 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 14, fontWeight: 500, color: "var(--fg1)" }}>{label}</span>
          {primary && on && (
            <span style={{
        ...
        {on && badges.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10,
            fontSize: 12, color: "var(--fg2)", lineHeight: 1.4 }}>
            {badges.map((b, i) => (
              <span key={i}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
        {ip && (
          <span style={{
            ...
            <span className="overline" style={{ fontSize: 10, letterSpacing: "0.06em" }}>IP</span>
...
function ModuleRow({ label, icon, on }) {
  return (
    <div style={{
      ...
      <span style={{ flex: 1, fontSize: 14, color: "var(--fg1)" }}>{label}</span>
      <span style={{
...
// Tight key-value table — stacked cells (label ON TOP of value) flowing into
// as many columns as the card width allows. Mirrors the customer overview's
// .kv2 treatment: top-bottom pairs, denser, no stranded value column.
function KvTable({ rows }) {
  return (
    <div className="kv2-wrap"><dl className="kv2">
      {rows.map(([k, v], i) => (
        <div key={i} className="kv2__row"><dt>{k}</dt><dd>{v}</dd></div>
      ))}
    </dl></div>
  );
}
...
function PushAppPicker({ open, device, onClose, onConfirm }) {
  ...
  return (
    <window.Modal open={open} onClose={onClose} width={680}
      padding={0}
      title={<>Push an app to <span className="mono" style={{ fontSize: 14, fontWeight: 500 }}>{device.sn}</span></>}
            ...
            <button type="button" key={app.id}
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8, minWidth: 0 }}>
                  <span style={{ fontSize: 14, fontWeight: 500, color: "var(--fg1)",
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {app.name}
                  </span>
                  {already && (
      ...
      {selectedApp && (
        <div style={{
          ...
          <div>
              ...
              {[
                ...
                return (
                  <button type="button" key={opt.id}
                    ...
                    <span style={{ fontSize: 10, color: on ? "var(--color-primary-700)" : "var(--fg3)" }}>
                      {opt.hint}
                    </span>
                  </button>
                );
            ...
            {isAlreadyInstalled && (
              <div style={{ marginTop: 6, fontSize: 12, color: "var(--color-warning-700)",
      ...
      <window.Card title="Firmware"
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div className="mono" style={{ fontSize: 12, color: "var(--fg3)", marginTop: 4 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 12, color: firmware?.behind ? "var(--color-warning-700)" : "var(--fg3)", marginTop: 4 }}>
            ...
            <window.Button primary size="sm" icon="download"
              onClick={() => setPushConfirm({
                ...
                body: <>
                  ...
                  <ul style={{ margin: "8px 0 0", paddingLeft: 18, fontSize: 12, color: "var(--fg2)", lineHeight: 1.6 }}>
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
            {delta.missing.map(a => (
              <li key={a.id} style={{
                ...
                <span className="mono" style={{ fontSize: 12, color: "var(--color-warning-700)" }}>{a.version}</span>
                <span style={{ marginLeft: "auto", fontSize: 12, color: "var(--color-warning-700)" }}>Not installed</span>
              </li>
            ))}
          </ul>
        </window.Card>
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
            {delta.outdated.map(a => (
              <li key={a.id} style={{
                ...
                <AppLogo seed={a.id} name={a.name} />
                <span style={{ fontSize: 12, fontWeight: 500 }}>{a.name}</span>
                <span style={{ fontSize: 12, color: "var(--fg3)" }}>installed</span>
                <span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{a.installedVersion}</span>
                <window.Ico name="arrowR" size={11} style={{ color: "var(--fg3)" }} />
                <span className="mono" style={{ fontSize: 12, color: "var(--color-warning-700)", fontWeight: 500 }}>{a.version}</span>
                <span style={{ marginLeft: "auto", fontSize: 12, color: "var(--color-warning-700)" }}>Needs update</span>
              </li>
            ))}
          </ul>
        </window.Card>
            ...
            <tbody>
              {device.apps.map((a, i) => (
                <tr key={a.id} style={{ borderBottom: i < device.apps.length - 1 ? "1px solid var(--color-border-subtle)" : "none" }}>
                  ...
                  <td style={{ padding: "10px 12px", fontSize: 12, color: "var(--fg2)" }}>
                  ...
                  <td style={{ padding: "10px 12px" }}>
                    {a.system
                      ...
                      : <span style={{ fontSize: 12, color: "var(--fg3)" }}>App pool</span>}
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              {pushHistory.map((h, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: i < pushHistory.length - 1 ? "1px solid var(--color-border-subtle)" : "none" }}>
                    <td style={{ padding: "10px 12px" }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6,
                        fontSize: 12, color: "var(--fg2)", textTransform: "capitalize" }}>
                    ...
                    <td style={{ padding: "10px 12px" }}>
                      ...
                      {h.error && (
                        <div style={{ marginTop: 2, fontSize: 10, color: "var(--color-error-700)" }}>{h.error}</div>
                      )}
                    </td>
                    <td style={{ padding: "10px 12px", fontSize: 12, color: "var(--fg2)" }}>
                      <span className="mono">{h.completedAt || "—"}</span>
                    </td>
                    <td style={{ padding: "10px 12px", fontSize: 12, color: "var(--fg2)" }}>{h.by}</td>
                  </tr>
                );
              })}
            </tbody>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, color: "var(--fg2)", display: "flex",
            ...
            <span style={{ fontSize: 12, color: "var(--fg3)" }}>· {t.collectedAt.relative}</span>
          </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 600,
              ...
              <span className="mono" style={{ fontSize: 12, fontWeight: 400,
        ...
        {t.network.apnConfig && (
          <div style={{
            ...
            <button type="button" onClick={() => setApnOpen(!apnOpen)}
              ...
              <span style={{ marginLeft: 4, fontSize: 12, color: "var(--fg3)", fontWeight: 400 }}>
              ...
              <div style={{
                ...
                {Object.entries(t.network.apnConfig).map(([k, v]) => (
                  <React.Fragment key={k}>
                    <span className="overline" style={{ fontSize: 10, paddingTop: 1 }}>{k}</span>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {t.location.savedWifi.map((w, i) => (
                <span key={i} style={{
                  ...
                  {w.ssid}
                  <span className="mono" style={{ fontSize: 10, color: "var(--fg3)" }}>{w.level} dBm</span>
                </span>
              ))}
            </div>
          </div>
      ...
      <window.Card title="System settings"
        ...
        <div style={{ display: "grid",
          ...
          <KvTable rows={[
            ["Language",     <span className="mono" style={{ fontSize: 14 }}>{t.settings.language}</span>],
            ["Timezone",     <span className="mono" style={{ fontSize: 14 }}>{t.settings.timezone}</span>],
            ...
            ["Screen timeout",
              <span style={{ display: "inline-flex", alignItems: "baseline", gap: 4 }}>
                ...
                <span style={{ fontSize: 12, color: "var(--fg3)" }}>seconds ({t.settings.screenTimeoutMs}ms)</span>
              </span>],
          ]} />
            ...
            <div style={{
              ...
              {Object.entries(t.state.sysParams).map(([k, v]) => (
                <React.Fragment key={k}>
                  ...
                  <span className="mono" style={{ fontSize: 12, color: "var(--fg1)", fontWeight: 500,
              ...
              <tbody>
                {[...memData].sort((a, b) => b.memMB - a.memMB).map((p, i) => (
                  <tr key={p.pid} style={{ borderBottom: i < memData.length - 1 ? "1px solid var(--color-border-subtle)" : "none" }}>
                    ...
                    <td style={{ padding: "9px 12px" }}><span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{p.pid}</span></td>
              ...
              <li key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start",
                ...
                <div style={{ flex: 1, minWidth: 0, paddingTop: 2 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 12, fontWeight: 500, color: "var(--fg1)" }}>{meta.label}</span>
                    <span className="mono" style={{ fontSize: 12, color: "var(--fg3)" }}>{e.at}</span>
                  </div>
      ...
      {tiles.map((tile, i) => {
        ...
        return (
          <div key={tile.key} style={{
            ...
            <div className="overline" style={{ fontSize: 10, color: "var(--fg3)",
              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {tile.label}
            </div>
            <div style={{
              display: "flex", alignItems: "center", gap: 6,
              fontSize: 14, fontWeight: 600, color, lineHeight: 1.2,
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 12, color: "var(--fg3)" }}>· not pushed yet</span>
        </span>
        <div style={{ flex: 1 }} />
        ...
        {items.map((ch) => {
          ...
          return (
            <span key={ch.id} style={{
              ...
              fontSize: 12, color: "var(--fg1)", maxWidth: "100%",
            }}>
...
function EditableModuleRow({ label, icon, on, onEdit, pending }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ flex: 1, fontSize: 14, color: "var(--fg1)", fontWeight: 500 }}>
        {label}
      </span>
      <span style={{
...
function EditableFlagRow({ label, positive, positiveLabel, negativeLabel, tone, onEdit, readOnly, pending }) {
  ...
  return (
    <div style={{
      ...
      {readOnly ? (
        <span style={{ fontSize: 10, color: "var(--fg3)", letterSpacing: "0.04em" }}>READ-ONLY</span>
      ) : (
        <button type="button" onClick={onEdit}
          title={isStaged ? `Edit staged · ${label}` : `Change · ${label}`}
          style={{
...
function EditableToggleRow({ label, on, onLabel, offLabel, onEdit, locked, pending }) {
  ...
  return (
    <div style={{
      ...
      {locked ? (
        <span style={{ fontSize: 10, color: "var(--fg3)", letterSpacing: "0.04em" }}>LOCKED</span>
      ) : (
        <button type="button" onClick={onEdit}
          title={isStaged ? `Edit staged · ${label}` : `Change · ${label}`}
          style={{
  ...
  return (
    <div style={{
      ...
      <span style={{ display: "inline-flex", alignItems: "center", gap: 8,
        flexWrap: "wrap", minWidth: 0 }}>
        <span className={mono ? "mono" : ""} style={{ fontSize: 14,
          color: "var(--fg1)",
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          textDecoration: isStaged ? "line-through" : "none",
          opacity: isStaged ? 0.55 : 1,
      ...
      {locked ? (
        <span style={{ fontSize: 10, color: "var(--fg3)", letterSpacing: "0.04em" }}>AUTO</span>
      ) : (
        <button type="button" onClick={onEdit}
          title={isStaged ? `Edit staged · ${label}` : `Change · ${label}`}
          style={{
    ...
    <>
      <window.Card title="Apps & Firmware"
        ...
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "var(--fg1)" }}>Firmware</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3,
              fontSize: 12, color: "var(--fg2)", flexWrap: "wrap" }}>
            ...
            {firmware?.target?.notes && (
              <div style={{ marginTop: 4, fontSize: 12, color: "var(--fg3)", lineHeight: 1.45 }}>
                  ...
                  {installedRows.map(row => {
                    ...
                    return (
                      <div key={row.id} style={{
                        ...
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 12, fontWeight: 500, color: "var(--fg1)",
                            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {row.name}
                            {row.category === "system" && (
                              <span style={{
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <window.Card title={`Event log · ${events.length}`}
        ...
        {!evtOpen && (() => {
          ...
          if (events.length === 0) {
            return (
              <div style={{ padding: "2px 0", fontSize: 12, color: "var(--fg3)" }}>
                ...
                {events.map((e, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: i < events.length - 1 ? "1px solid var(--color-border-subtle)" : "none" }}>
                      ...
                      <td style={{ padding: "10px 12px", fontSize: 12, fontWeight: 500 }}>{e.policy}</td>
                      <td style={{ padding: "10px 12px", color: "var(--fg2)" }}>
                        <span className="mono" style={{ fontSize: 12 }}>{e.at}</span>
                      </td>
                      <td style={{ padding: "10px 12px" }}>
...
function PolicyCard({ kind, label, icon, description, assignment, options, onChange }) {
  ...
  return (
    <div style={{
      ...
      {/* Description */}
      <div style={{ fontSize: 12, color: "var(--fg3)", lineHeight: 1.55 }}>{description}</div>

      {/* Policy select */}
      <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <span className="overline" style={{ fontSize: 10 }}>Active policy</span>
      ...
      <button onClick={() => window.showToast?.("Manage policies — coming in Phase 3.6", "info")}
        style={{
          marginTop: 2, fontSize: 12, color: "var(--color-text-tertiary)",
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 6 }}>
        <span className="overline" style={{ fontSize: 10 }}>{label}</span>
        <span className="mono num" style={{ fontSize: 12, color: "var(--fg3)" }}>{pct}%</span>
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 }}>
        <span className="mono num" style={{ fontSize: 20, fontWeight: 500,
          color: tone === "danger" ? "var(--color-error-700)"
              : tone === "warning" ? "var(--color-warning-700)"
              : "var(--fg1)" }}>{value}</span>
        <span style={{ fontSize: 12, color: "var(--fg3)" }}>{unit}{subValue ? ` · ${subValue}` : ""}</span>
      </div>
...
function KvCompact({ label, value }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: "var(--fg1)", display: "inline-flex", alignItems: "center", gap: 6 }}>{value}</div>
    </div>
  );
}
    ...
    <div style={{
      ...
      {rows.map(([k, v], i) => (
        <React.Fragment key={i}>
          ...
          <span style={{ fontSize: 14, color: "var(--color-text-primary)", minWidth: 0,
...
function Flag({ positive, positiveLabel, negativeLabel, tone }) {
  if (positive) {
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--color-success-700)" }}>
...
function SecurityBanner({ hardware }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: "var(--color-warning-700)" }}>
...
function NetRow({ label, enabled, detail }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
      ...
      <span style={{
        fontSize: 14, color: enabled ? "var(--color-text-primary)" : "var(--fg3)",
        minWidth: 0, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6,
...
function YesNo({ on, warnWhenOff }) {
  if (on) {
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--color-success-700)" }}>
