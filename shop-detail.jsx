...
const ProductDetail = ({ productId, products, onBack, onAddToCart, cartQty = 0, cartSubtotal = 0, onOpenCart, onOpenOrders, onCheckout, onOrderNow }) => {
  ...
  // then variant images for other variants (preview), then ObjectTile fallback.
  // Only REAL raster photos count; system-generated SVG placeholders fall back to
  // the duotone ObjectTile (same treatment as the Goods grid) so the PDP never
  // shows the striped placeholder graphic.
  const isRealPhoto = (s) => s && !/^data:image\/svg\+xml/.test(s);
  const gallery = [];
  if (variant && isRealPhoto(variant.image)) gallery.push({ src: variant.image, label: 'variant', kind: 'img' });
  if (isRealPhoto(product.baseImage)) gallery.push({ src: product.baseImage, label: 'base', kind: 'img' });
  (product.extraImages || []).forEach((src) => {
    if (isRealPhoto(src)) gallery.push({ src, label: 'extra', kind: 'img' });
  });
  product.variants.forEach((v) => {
    if (isRealPhoto(v.image) && v.image !== variant?.image) gallery.push({ src: v.image, label: 'alt', kind: 'img' });
  });
  if (gallery.length === 0) {
    gallery.push({ kind: 'object' });
  }
  const mainImg = gallery[Math.min(selectedImage, gallery.length - 1)] || gallery[0];

  // Hero treatment for the object (no-photo) fallback: a same-hue flat wash
  // ground + a larger floating ObjectTile, so the gallery reads premium, not empty.
  const objCat = window.productObjectCat(product);
  const heroHue = objCat === 'device' ? window.modelHue(model?.name || product.name)
    : objCat === 'service' ? 70 : objCat === 'accessory' ? 230 : 250;
  const isObjectHero = mainImg.kind === 'object';
  ...
  return (
    <div className="page page--wide">
      ...
      <div className="pdp-back">
        <button type="button" className="det-back" onClick={onBack} aria-label="Back to Products" title="Back to Products">
      ...
      <div className="pdp-main">
        ...
        <div className="pdp-gallery">
          <div className="pdp-gallery__main" data-hero={isObjectHero ? 'object' : undefined}
            style={isObjectHero ? { '--pdp-hue': heroHue } : undefined}>
            {mainImg.kind === 'img' && <img src={mainImg.src} alt={product.name}/>}
            {mainImg.kind === 'object' && (
              <svg className="pdp-hero-glyph" viewBox="0 0 24 24" width="252" height="252"
                fill="currentColor" aria-hidden="true">
                {window.OBJ_GLYPH[objCat === 'device' ? 'device' : (window.OBJ_PRESET[objCat]?.glyph || 'accessory')]}
              </svg>
            )}
          </div>
          {gallery.length > 1 && (
            <div className="pdp-gallery__thumbs">
              {gallery.map((g, i) => (
                <button key={i} type="button"
                  ...
                  {g.kind === 'object' && (
                    window.productObjectCat(product) === 'device'
                      ? <window.ObjectTile glyph="device" hue={window.modelHue(model?.name || product.name)} size={40}/>
                      : <window.ObjectTile cat={window.productObjectCat(product)} size={40}/>
                  )}
                </button>
              ))}
            </div>
...
// ─── Styles ────────────────────────────────────────────────────
const pdpStyles = `
.pdp-back { position: sticky; top: 0; z-index: 30; background: var(--color-bg-2); border-bottom: 1px solid var(--color-border-subtle); margin: calc(-1 * var(--space-6)) calc(-1 * var(--space-6)) var(--space-6); padding: var(--space-4) var(--space-6); display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.pdp-back__btn { display: inline-flex; align-items: center; gap: 4px; background: transparent; border: 0; color: var(--color-text-secondary); font-family: inherit; font-size: 14px; cursor: pointer; padding: 4px 0; }
.pdp-back__btn:hover { color: var(--color-text-primary); }
.pdp-back__btn.is-title { color: var(--color-text-primary); font-weight: 600; font-size: 16px; max-width: 60%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.pdp-gallery__main { flex: 1; min-height: 420px; background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 16px; padding: 48px; display: grid; place-items: center; overflow: hidden; box-shadow: var(--shadow-1); }
[data-theme="dark"] .pdp-gallery__main { background: var(--color-bg-3); }
/* No-photo hero — one big confident glyph on a flat same-hue wash ground (no gradient, no nested plate), Stripe/Linear style. */
.pdp-gallery__main[data-hero="object"] { background: oklch(97.5% 0.018 var(--pdp-hue, 250)); padding: 32px; }
[data-theme="dark"] .pdp-gallery__main[data-hero="object"] { background: oklch(20% 0.022 var(--pdp-hue, 250)); }
.pdp-hero-glyph { width: 72%; height: auto; max-width: 320px; color: oklch(56% 0.13 var(--pdp-hue, 250)); filter: drop-shadow(0 4px 14px oklch(50% 0.10 var(--pdp-hue, 250) / 0.20)); }
[data-theme="dark"] .pdp-hero-glyph { color: oklch(74% 0.12 var(--pdp-hue, 250)); filter: none; }
...
.pdp-thumb { width: 56px; height: 56px; border: 1.5px solid var(--color-border-default); background: var(--color-bg-2); border-radius: 12px; display: grid; place-items: center; padding: 4px; cursor: pointer; overflow: hidden; transition: all var(--duration-fast); }
[data-theme="dark"] .pdp-thumb { background: var(--color-bg-3); }
...
.pdp-buy-box { background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 12px; padding: 20px; box-shadow: var(--shadow-1); display: flex; flex-direction: column; gap: 16px; margin-top: 6px; }
...
.pdp-price__hint { font-size: 12px; color: var(--color-text-tertiary); }
.pdp-price__edit { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border: 1.5px solid var(--color-primary-700); border-radius: 8px; background: var(--color-bg-1); width: fit-content; }
...
.pdp-price__reset { font-size: 12px; background: transparent; border: 0; color: var(--color-primary-700); cursor: pointer; padding: 4px 8px; font-family: inherit; }
.pdp-price__reset:hover { text-decoration: underline; }

.pdp-variant-section { display: flex; flex-direction: column; gap: 8px; }
.pdp-variant-label { font-size: 14px; color: var(--color-text-secondary); }
...
.pdp-variant-opt { display: flex; flex-direction: column; align-items: flex-start; gap: 2px; padding: 10px 14px; background: var(--color-bg-1); border: 1.5px solid var(--color-border-default); border-radius: 8px; cursor: pointer; font-family: inherit; transition: all var(--duration-fast); min-width: 120px; text-align: left; }
...
.pdp-variant-opt__lbl { font-size: 14px; font-weight: 600; color: var(--color-text-primary); }
.pdp-variant-opt__price { font-size: 12px; color: var(--color-text-secondary); }
.pdp-variant-opt__tag { font-size: 10px; color: var(--color-warning-700); margin-top: 2px; }
...
.pdp-qty { display: inline-flex; align-items: center; background: var(--color-bg-1); border: 1.5px solid var(--color-border-default); border-radius: 8px; padding: 2px; }
...
.pdp-qty__input { width: 56px; height: 36px; text-align: center; border: 0; background: transparent; font-size: 16px; font-weight: 500; color: var(--color-text-primary); font-variant-numeric: tabular-nums; outline: 0; font-family: inherit; }
.pdp-qty__input::-webkit-inner-spin-button, .pdp-qty__input::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; }
.pdp-line-total { font-size: 14px; color: var(--color-text-secondary); }
.pdp-line-total strong { font-size: 16px; font-weight: 600; color: var(--color-text-primary); margin-left: 6px; }

.pdp-fulfill { padding: 12px 14px; background: var(--color-bg-3); border-radius: 8px; border: 1px solid var(--color-border-subtle); }
.pdp-fulfill__lbl { display: flex; align-items: center; gap: 6px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--color-text-tertiary); margin-bottom: 4px; }
.pdp-fulfill__txt { font-size: 12px; color: var(--color-text-secondary); line-height: 1.5; }

.pdp-desc { background: var(--color-bg-2); border: 1px solid var(--color-border-default); border-radius: 12px; box-shadow: var(--shadow-1); padding: 22px 24px; }
.pdp-desc__title { margin: 0 0 12px; font-size: 14px; font-weight: 600; color: var(--color-text-primary); }
.pdp-prose p { margin: 0; line-height: 1.65; font-size: 14px; color: var(--color-text-primary); }
