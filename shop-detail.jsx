/* global React, Btn, Icon, Badge, ModelTile, DEVICE_MODELS, useToast,
   productEffectiveStatus, CATEGORY_BADGE,
   resolveVariantLabel, CartTrigger */
const { useState, useMemo, useEffect, useRef } = React;

// ─── Product Detail Page (PDP) ─────────────────────────────
// Two-column layout (simpler than Amazon's 3-col):
//   Left  (45%): image gallery
//   Right (55%): title, info, variant picker, qty, Add to cart
//   Below: tabs (Description / Specifications / Options)
//
...
const ProductDetail = ({ productId, products, onBack, onAddToCart, cartQty = 0, cartSubtotal = 0, onOpenCart, onOpenOrders, onCheckout, onOrderNow }) => {
  ...
  // (kept as a no-op anchor — the legacy effect synced an "Integration mode"
  //  axis with selectedIntegration; both are gone.)
  useEffect(() => {
    if (!product) return;
    void product;
    /* legacy block removed — variant is whatever the user picks */
  }, [product]);
  ...
  const effectivePrice = variant?.price || 0;
  ...
  const canAdd = isListed && variant && variant.status === 'ACTIVE' && qty > 0;
  ...
  const handleOrder = () => {
    if (!canAdd) return;
    if (onOrderNow) {
      onOrderNow(product, variant, qty);
    } else if (onCheckout) {
      onAddToCart(product, variant, qty);
      onCheckout();
    }
      ...
      {!isListed && (
        <div className="prod-banner prod-banner--warn" style={{ marginBottom: 16 }}>
          ...
          <span>
            ...
            <em>Products</em> to manage its status.
          </span>
        </div>
      )}
        ...
        <div className="pdp-info">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 2 }}>
            {(() => {
              ...
              const path = (window.categoryPath ? window.categoryPath(cats, product.categoryId) : []).slice(-2);
              if (!path.length) return null;
              return (
                <span className="pdp-cat-tag" title={window.categoryPathLabel ? window.categoryPathLabel(cats, product.categoryId) : ''}>
                  <Icon name="folder" size={12}/>
                  {path.map((c, i) => (
                    <React.Fragment key={c.id}>
                      {i > 0 && <span className="pdp-cat-tag__sep">/</span>}
                      <span>{c.name}</span>
                    </React.Fragment>
                  ))}
                </span>
          ...
          <div className="pdp-buy-box">
            ...
            <div className="pdp-qty-row">
              <div className="pdp-qty">
                ...
                <button className="pdp-qty__btn" onClick={() => setQty((q) => q + 1)}
                  aria-label="Increase">
...
.pdp-cat-tag { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 999px; background: var(--color-bg-3); border: 1px solid var(--color-border-default); font-size: 12px; font-weight: 500; color: var(--color-text-secondary); }
.pdp-cat-tag svg { color: var(--color-text-tertiary); }
.pdp-cat-tag__sep { color: var(--color-text-tertiary); opacity: 0.6; }
...
.pdp-qty-row { display: flex; align-items: center; justify-content: flex-start; gap: 16px; }
