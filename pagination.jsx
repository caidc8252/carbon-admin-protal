...
//   pageSizeOptions 每页行数选项，默认 [25, 50, 100, 200]
//   unit            摘要里的名词，默认 "rows"（可传 "tickets" / "devices"）
//   divider         顶部是否加分隔线，默认 false（与 Devices 现状一致）
//   showRowsPerPage / showSummary / showGoTo  各区块开关，默认 true
// ─────────────────────────────────────────────────────────────
...
(function () {
  ...
  function PagerBtn({ active, disabled, onClick, title, children }) {
    return (
      <button onClick={onClick} disabled={disabled} title={title} style={{
        ...
        color: active ? "var(--color-text-on-primary, #fff)" : disabled ? "var(--fg4)" : "var(--fg2)",
  ...
  const inStyle = {
    width: 56, height: 26, fontSize: 12, padding: "0 8px",
    border: "1px solid var(--border-2)", borderRadius: 6,
    background: "var(--bg2)", color: "var(--fg1)", outline: "none",
  ...
  function Pagination({
    ...
    showRowsPerPage = true, showSummary = true, showGoTo = true
  }) {
    ...
    const goToPage = () => {
      ...
      if (!isNaN(n) && n >= 1 && n <= totalPages) { setPage(n); setGoTo(""); }
    };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <input value={goTo}
            ...
            placeholder={String(page)} className="mono" style={inStyle} />
          <button onClick={goToPage} style={goBtnStyle}>Go</button>
        </div>
        }
      </div>);
  }
