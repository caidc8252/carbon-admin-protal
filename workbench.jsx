...
function WorkbenchScreen({ ticket, navigate, onCloseTab }) {
  ...
  // Preset handed to LogPullTool when it's opened indirectly (e.g. from the
  // Trouble tab's "no log attached" empty state — operator wants to pull
  // logs that cover the ticket's fault window without re-typing the range).
  const [logPullPreset, setLogPullPreset] = useStateW(null);
  // pendingToolRef lets us request a tool *and* a tab switch atomically:
  // the tab-change effect below normally clears the active tool, but if
  // there's a pending request it picks that up instead.
  const pendingToolRef = useRefW(null);
  ...
  // Switching tabs closes any open tool drawer — unless something asked
  // for a specific tool to be opened on this tab (see openTool below).
  useEffectW(() => {
    if (pendingToolRef.current) {
      const id = pendingToolRef.current;
  ...
  // Open a tool on a specific tab. Used by surfaces that live on one tab
  // but need to deep-link the operator into a tool that lives on another
  // (e.g. Trouble's empty-log CTA → Monitoring's "Pull logs" drawer).
  const openTool = (tab, toolId, preset = null) => {
    if (preset) setLogPullPreset(preset);
    if (activeTab !== tab) {
  ...
  return (
    <div style={{
      display: "grid", height: "100%", width: "100%", minHeight: 0, minWidth: 0,
      overflow: "hidden",
          ...
          <div style={{ height: "100%",
            ...
            <div style={{ flex: 1, minHeight: 0,
              ...
              {activeTab === "info"    && <TroubleTab    ticket={ticket} device={device} logAction={logAction}
                onPullLogs={(preset) => openTool("monitor", "logpull", preset)} />}
          ...
          {tool && (
            <WBToolDrawer tool={tool} onClose={() => setActiveTool(null)}>
              {tool.id === "logpull"  && <LogPullTool      ticket={ticket} device={device} logAction={logAction}
                initialPreset={logPullPreset}
                onPresetConsumed={() => setLogPullPreset(null)} />}
  ...
  return (
    <div style={{
      ...
      <button onClick={onToggleSidebar} style={{
        ...
        color: "var(--fg2)", fontSize: 12, fontWeight: 500,
        cursor: "pointer", fontFamily: "inherit",
...
function WBActionBar({ tools, activeTool, onSelect }) {
  return (
    <div style={{
      ...
      <span className="overline" style={{ fontSize: 10, color: "var(--fg3)", marginRight: 4,
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ marginTop: 1, fontSize: 12, color: "var(--fg3)" }}>{tool.blurb}</div>
        </div>
  ...
  return (
    <div style={{
      ...
      <span className="mono" style={{ fontSize: 12, fontWeight: 600,
        color: "var(--color-primary-700)", letterSpacing: "0.02em", whiteSpace: "nowrap" }}>{ticket.id}</span>
      ...
      <span style={{
        ...
        <span style={{ width: 6, height: 6, borderRadius: "50%",
          background: st.tone === "success" ? "var(--color-success-500)"
                    : st.tone === "warning" ? "var(--color-warning-500)"
                    : st.tone === "danger"  ? "var(--color-error-500)"
                    :                          "var(--color-info-500)" }} />
        {st.label}
      </span>
...
// Light-mode action chip styled for the workbench top bar. Three
...
function WBTopBarAction({ icon, children, onClick, primary, danger }) {
  const bg = danger  ? "var(--color-error-50)"
          : primary ? "var(--color-primary-700)"
                    : "var(--color-bg-2)";
  const bgHover = danger  ? "color-mix(in oklab, var(--color-error-50) 60%, var(--color-error-100))"
                : primary ? "var(--color-primary-800)"
                          : "var(--color-bg-hover)";
  const border = danger  ? "var(--color-error-200)"
              : primary ? "var(--color-primary-700)"
                        : "var(--color-border-default)";
  const color = danger  ? "var(--color-error-700)"
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border-1)" }}>
        <div className="overline" style={{ fontSize: 10, marginBottom: 8 }}>Issue</div>
        ...
        <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
          ...
          <span style={{
            ...
            fontSize: 10, fontWeight: 500, color: "var(--fg2)",
          }}>{sv.label}</span>
        </div>
        <div style={{
        ...
        <div style={{
          ...
          gridTemplateColumns: "70px minmax(0, 1fr)", rowGap: 4, fontSize: 12,
        }}>
      ...
      <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--border-1)" }}>
        <button type="button" onClick={openComments}
          ...
          <span style={{ display: "flex", flexDirection: "column", gap: 1, flex: 1, minWidth: 0 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: "var(--fg1)" }}>
              Comments
              {totalComments > 0 && (
                <span className="mono num" style={{
                  marginLeft: 6, fontSize: 12, color: "var(--fg3)", fontWeight: 400,
                }}>{totalComments}</span>
              )}
            </span>
            <span style={{ fontSize: 10, color: "var(--fg3)" }}>
                  ...
                  return (
                    <li key={i} style={{ display: "flex", gap: 8 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0,
                        ...
                        <div style={{ display: "flex", alignItems: "baseline",
                          ...
                          <span style={{ fontSize: 12, fontWeight: 600,
                            ...
                            {c.actor}{isMine && <span style={{ marginLeft: 4,
                              fontSize: 10, color: "var(--fg3)", fontWeight: 400 }}>(you)</span>}
                            {isNew && !isMine && (
                              <span style={{ marginLeft: 6,
                                fontSize: 10, color: "var(--color-error-700)",
                        ...
                        <div style={{ fontSize: 12, color: "var(--fg1)", lineHeight: 1.5,
      ...
      {items.map((e, i) => {
        ...
        return (
          <li key={i} style={{ display: "flex", gap: 8 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, color: "var(--fg1)", lineHeight: 1.4,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 260, fontSize: 12, color: "var(--fg3)", lineHeight: 1.55 }}>
          {mode === "auto"
      ...
      {state === "idle" && (
        <div style={{
          ...
          borderRadius: "var(--radius-md)",
          fontSize: 12, color: "var(--fg3)", lineHeight: 1.6,
        }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span className="overline" style={{ fontSize: 10 }}>{title}</span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span className="overline" style={{ fontSize: 10 }}>Manual checks</span>
        <span style={{ fontSize: 10, color: "var(--fg3)" }}>
      ...
      <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
        {rows.map((r, i) => (
          <li key={r.id} style={{
            ...
            <span style={{ fontSize: 12, fontWeight: 500, color: "var(--fg1)" }}>{r.label}</span>
...
function DiagValue({ value }) {
  if (typeof value === "string") {
    return <span style={{ fontSize: 12, color: "var(--fg1)" }}>{value}</span>;
  }
...
function TroubleTab({ ticket, device, logAction, onPullLogs }) {
  const logs = useMemoW(
    ...
    <div ref={scrollRef} style={{
      ...
      <div style={{ background: "oklch(12% 0.01 250)",
        ...
        {logs.length > 0
          ...
          : <TroubleNoLogState ticket={ticket} onPullLogs={onPullLogs} />
        }
      </div>
...
// Empty state for the Trouble log viewer when no log file is attached.
// Surfaces a CTA to pull logs from the device, pre-scoped to the ticket's
// fault window when known — operator should never be stuck staring at a
// dead-end "no logs" message.
function TroubleNoLogState({ ticket, onPullLogs }) {
  const fw = ticket.faultWindow;
  // Choose a sensible default window for the LogPullTool. If the ticket
  // carries an explicit fault window we widen by one bucket on each side
  // to make sure the surrounding context comes in too. Otherwise default
  // to "last 1 hour" — covers the vast majority of fresh tickets.
  const preset = fw
    ? { window: pickWindowBucket(fw), source: "faultWindow",
        start: fw.start, end: fw.end }
    : { window: "60min", source: "createdAt", start: ticket.createdAt };

  return (
    <div style={{
      padding: "48px 20px 56px", display: "flex", flexDirection: "column",
      ...
      <div style={{
        ...
        <window.Ico name="doc" size={16} stroke={1.8} />
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 4,
        maxWidth: 360 }}>
        <div style={{ color: "oklch(82% 0.01 250)", fontSize: 14, fontWeight: 500 }}>
          No log attached to this ticket.
        </div>
        <div style={{ color: "oklch(60% 0.02 240)", fontSize: 12, lineHeight: 1.55 }}>
          {fw
            ? <>Pull logs from the device for the fault window below — the file is uploaded to the cloud and attached here when ready.</>
            : <>Pull logs from the device around the time the ticket was reported. Typical extraction takes 5–15 s.</>}
        </div>
      </div>
      ...
      {fw && (
        <div style={{
          ...
          fontSize: 12, color: "oklch(75% 0.02 240)",
        }}>
          <window.Ico name="clock" size={11} stroke={1.8}
            style={{ color: "oklch(60% 0.02 240)" }} />
          <span>{fw.start}</span>
          <span style={{ color: "oklch(50% 0.02 240)" }}>→</span>
          <span>{fw.end}</span>
        </div>
      )}

      <button onClick={() => onPullLogs?.(preset)}
        style={{
          marginTop: 4,
          display: "inline-flex", alignItems: "center", gap: 7,
          padding: "9px 16px",
          ...
          color: "white",
          border: "1px solid color-mix(in oklab, var(--color-primary-500) 60%, black)",
          borderRadius: "var(--radius-md)",
          fontSize: 12, fontWeight: 500,
          cursor: "pointer", fontFamily: "inherit",
        }}>
        <window.Ico name="download" size={12} stroke={2} />
        {fw ? "Pull logs for fault window" : "Pull logs from device"}
      </button>

      <div style={{ fontSize: 10, color: "oklch(50% 0.02 240)",
        marginTop: 2 }}>
        Opens the <b style={{ color: "oklch(65% 0.02 240)", fontWeight: 500 }}>Pull logs</b> tool
        in the Monitoring tab{fw ? ", pre-scoped to this window." : "."}
      </div>
    </div>
  );
}

// Map a faultWindow {start,end} pair to the closest LogPullTool window
// bucket. Falls back to "60min" if we can't parse — the operator can
// always widen it manually inside the tool.
function pickWindowBucket(fw) {
  if (!fw || !fw.start || !fw.end) return "60min";
  const a = Date.parse(fw.start);
  const b = Date.parse(fw.end);
  if (!isFinite(a) || !isFinite(b) || b <= a) return "60min";
  const mins = (b - a) / 60000;
  if (mins <= 25) return "30min";
  if (mins <= 55) return "60min";
  if (mins <= 5 * 60) return "6h";
  return "24h";
}
          ...
          <ol style={{ margin: 0, paddingLeft: 0, listStyle: "none",
            ...
            {kb.recommendedSteps.map((s, i) => (
              <li key={i} style={{
                ...
                fontSize: 12, color: "var(--fg1)", lineHeight: 1.5,
                textWrap: "pretty",
              }}>
                <span style={{
                  flexShrink: 0,
...
function ContextChip({ label, value, warn, warnHint }) {
  ...
  return (
    <span style={{
      ...
      <span style={{ fontSize: 10, color: warn ? "var(--color-warning-700)" : "var(--fg3)",
        letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</span>
      <span className="mono" style={{ fontWeight: 500 }}>{value}</span>
      {warn && warnHint && (
        <span style={{ fontSize: 10, opacity: 0.85 }} title={warnHint}>· {warnHint}</span>
      )}
    </span>
  );
}
  ...
  return (
    <div style={{
      ...
      <span style={{ fontStyle: "normal", color: "var(--color-primary-700)",
        fontWeight: 600, marginRight: 6, fontSize: 10,
        letterSpacing: "0.04em", textTransform: "uppercase" }}>
        Merchant note
      </span>
      "{note}"
    ...
    <div style={{ display: "grid",
      ...
      <WBSectionCard title="System">
        <WBKvTable rows={[
          ...
          ["Firmware", <span className="mono" style={{ fontSize: 12 }}>{snap.firmware}</span>],
        ]} />
      </WBSectionCard>
    </div>
  );
    ...
    <button type="button" onClick={onClick} title={`Open ${attachment.name}`}
      ...
      <div style={{
        ...
        {isLog ? (
          <div style={{
            ...
            <span style={{
              fontFamily: "var(--font-family-mono)", fontSize: 8,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 1,
        ...
        <span style={{
          ...
          fontFamily: "var(--font-family-mono)",
          fontSize: 10, color: "var(--fg1)", fontWeight: 500,
        ...
        <div style={{
          ...
          <button onClick={onToggleFocus}
            ...
            style={{
              ...
              fontSize: 10, fontWeight: focus ? 600 : 500, cursor: "pointer",
              fontFamily: "var(--font-family-sans)",
            }}>
        ...
        <div ref={bodyRef}
          ...
          {visible.length === 0 && (
            <div style={{ padding: "14px", textAlign: "center",
              fontSize: 12, color: "oklch(55% 0.02 240)" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ marginLeft: "auto", display: "flex", gap: 12,
            fontFamily: "var(--font-family-mono)", color: "var(--fg3)", fontSize: 12 }}>
        ...
        <div style={{
          ...
          {sessionState === "ended"
            ? <div style={{ textAlign: "center", color: "var(--fg3)" }}>
                ...
                <div style={{ marginTop: 8, fontSize: 14 }}>Session ended</div>
                <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <WBSectionCard title="Input flow">
          ...
          <div style={{
            ...
            borderRadius: "var(--radius-sm)",
            fontSize: 10, color: "var(--color-warning-700)",
          }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop: 14, textAlign: "center" }}>
        ...
        <div style={{ marginTop: 4, fontSize: 12, color: "rgba(255,255,255,.7)", lineHeight: 1.5 }}>
...
function LogPullTool({ ticket, logAction, initialPreset, onPresetConsumed }) {
  ...
  // Form state — seeded from `initialPreset` when the tool was opened via
  // a deep link (Trouble tab's no-log empty state). Otherwise plain defaults.
  const [window24, setWindow24] = useStateW(
    () => initialPreset?.window || "60min"); // 30min / 60min / 6h / 24h
  const [modules, setModules] = useStateW({ emv: true, network: true, app: true, system: false });
  // Local copy of the preset banner so the operator can dismiss it once
  // they've seen it. We notify the parent on first commit so the same
  // preset isn't re-applied if the drawer is re-opened later.
  const [presetBanner, setPresetBanner] = useStateW(initialPreset || null);
  useEffectW(() => {
    if (initialPreset) onPresetConsumed?.();
      ...
      <WBSectionCard title="Pull new logs" hint="Scope a fresh extraction. The device packages logs server-side and uploads.">
        {presetBanner && (
          <div style={{
            ...
            padding: "9px 12px",
            background: "color-mix(in oklab, var(--color-primary-500) 8%, var(--bg2))",
            border: "1px solid color-mix(in oklab, var(--color-primary-500) 30%, transparent)",
            borderRadius: "var(--radius-sm)",
            marginBottom: 12,
          }}>
            <window.Ico name="bolt" size={12} stroke={1.8}
              style={{ color: "var(--color-primary-600, var(--color-primary-500))",
                marginTop: 2, flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0, fontSize: 12,
              color: "var(--fg2)", lineHeight: 1.5 }}>
              <div style={{ color: "var(--fg1)", fontWeight: 500 }}>
                Pre-scoped to the ticket's {presetBanner.source === "faultWindow" ? "fault window" : "report time"}.
              </div>
              <div style={{ marginTop: 2, fontFamily: "var(--font-family-mono)",
                fontSize: 10, color: "var(--fg3)" }}>
                {presetBanner.source === "faultWindow"
                  ? <>{presetBanner.start} → {presetBanner.end} · default {WINDOW_LABEL[presetBanner.window] || presetBanner.window}</>
                  : <>around {presetBanner.start} · default {WINDOW_LABEL[presetBanner.window] || presetBanner.window}</>}
              </div>
            </div>
            <button onClick={() => setPresetBanner(null)}
              aria-label="Dismiss"
              style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
          <div>
            <div className="overline" style={{ fontSize: 10, marginBottom: 6 }}>Time window</div>
          ...
          <div>
            <div className="overline" style={{ fontSize: 10, marginBottom: 6 }}>Modules</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {history.map(h => (
              <button key={h.id} onClick={() => setOpenFile(h)}
                ...
                <span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{h.size}</span>
                <span className="mono num" style={{ fontSize: 10, color: "var(--fg3)" }}>{h.lines.toLocaleString()} lines</span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6,
          fontSize: 10, color: paused ? "oklch(78% 0.16 75)" : "oklch(70% 0.20 152)",
            ...
            <span ref={resPopRef} style={{ position: "relative" }}>
              ...
              {resOpen && (
                <div role="listbox" style={{
                  ...
                  background: "#fff",
                  border: "1px solid var(--color-border-default, #e2e8f0)",
                  borderRadius: 8, boxShadow: "0 16px 32px -12px rgba(15,23,42,0.18)",
                  zIndex: 8,
                }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <WBSectionCard title="Pull a file or directory"
        ...
        <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 6 }}>
          <span style={{ fontSize: 10, color: "var(--fg3)", alignSelf: "center" }}>Quick presets:</span>
          {presets.map((p, i) => (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {files.map((f, i) => (
              <div key={i} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div className="mono" style={{ fontSize: 12, fontWeight: 500, color: "var(--fg1)" }}>{f.name}</div>
                  <div className="mono" style={{ marginTop: 2, fontSize: 10, color: "var(--fg3)" }}>{f.path}</div>
                </div>
                <span className="mono" style={{ fontSize: 12, color: "var(--fg2)" }}>{f.size}</span>
                <span className="mono" style={{ fontSize: 10, color: "var(--fg3)" }}>{f.pulledAt}</span>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <WBSectionCard title="Device restart"
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 240, fontSize: 12, color: "var(--fg2)", lineHeight: 1.55 }}>
            <p style={{ margin: 0 }}>
            ...
            <p style={{ margin: "6px 0 0", fontSize: 12, color: "var(--fg3)" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--color-error-700)", lineHeight: 1.55 }}>
        ...
        <WBSectionCard title="Reset in progress">
          {stage === "wiping" ? (
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 4px" }}>
              ...
              <span style={{ fontSize: 14, color: "var(--color-error-700)", fontWeight: 500 }}>
...
function FactoryCheckbox({ checked, onChange, label }) {
  return (
    <label style={{
      ...
      <span style={{ fontSize: 12, color: "var(--fg1)", lineHeight: 1.5 }}>{label}</span>
    </label>
  );
}
