...
function WBTopBar({ ticket, device, merchant, connection, role, onClose,
  ...
  return (
    <div style={{
      ...
      <button onClick={onClose} title="Back" aria-label="Back" style={{
        flex: "none",
        display: "grid", placeItems: "center",
        width: 36, height: 36,
        borderRadius: 6,
...
function WBRightSidebar({ ticket, device, merchant, onOpenTicket, onOpenComment, commentTrigger }) {
  ...
  const comments = [...(ticket.comments || [])].sort((a, b) => new Date(b.at) - new Date(a.at));
  const [cVisible, setCVisible] = React.useState(10);
  const [cLoadingMore, setCLoadingMore] = React.useState(false);
  const cLoadMore = () => {
    if (cLoadingMore) return;
    setCLoadingMore(true);
    setTimeout(() => { setCVisible((v) => v + 10); setCLoadingMore(false); }, 1000);
  };
  ...
  return (
    <div style={{
        ...
        <div style={{
          ...
          {ticket.faultWindow && <>
            ...
            <span className="mono" style={{ color: "var(--fg2)", lineHeight: 1.35,
              ...
              <span>{window.fmtDateTimeS(ticket.faultWindow.start)}</span>
              <span style={{ color: "var(--fg3)" }}>
                <span style={{ marginRight: 4 }}>→</span>{window.fmtDateTimeS(ticket.faultWindow.end)}
              </span>
            </span>
          </>}
        </div>
          ...
          <div style={{ marginTop: 10 }}>
            ...
            {totalComments > 0 && (
              <ol style={{ margin: 0, padding: 0, listStyle: "none",
                ...
                {comments.slice(0, cVisible).map((c, i) => {
            ...
            {comments.length > cVisible && (
              <div style={{ marginTop: 8, display: "flex", justifyContent: "center" }}>
                <window.Button size="sm" onClick={cLoadMore} loading={cLoadingMore} disabled={cLoadingMore}>
                  {cLoadingMore ? "Loading…" : "Load more"}
                </window.Button>
              </div>
            )}
          </div>
      ...
      {/* Activity — collapsible; flows inline with the rest of the
          sidebar so the whole pane scrolls as one. */}
      <div style={{ display: "flex", flexDirection: "column" }}>
        <button type="button" onClick={() => setActivityOpen((v) => !v)} style={{
          width: "100%",
          padding: "10px 16px",
          borderBottom: "1px solid var(--border-1)",
          ...
          background: "var(--bg2)",
          border: 0, borderBottom: "1px solid var(--border-1)",
          cursor: "pointer", fontFamily: "inherit", textAlign: "left",
        }}>
          <span className="overline" style={{ fontSize: 9.5 }}>
            Activity <span className="mono num">{tlLen}</span>
          </span>
          <window.Ico name={activityOpen ? "chevd" : "chevr"} size={12}
            style={{ color: "var(--fg3)", flexShrink: 0 }} />
        </button>
        {activityOpen && (
...
function WBActionRecords({ timeline }) {
  ...
  return (
    ...
    <ol style={{ margin: 0, padding: 0, listStyle: "none",
      ...
      {shown.map((e, i) => {
        // Two-tone dots: key milestones (escalation / resolution / created)
        // read as primary; routine events are muted.
        const isMilestone = e.kind === "escalation" || e.kind === "resolution" || e.kind === "created";
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4,
        maxWidth: 360 }}>
        <div style={{ color: "oklch(82% 0.01 250)", fontSize: 13.5, fontWeight: 500 }}>
          No log attached to this ticket.
        </div>
        <div style={{ color: "oklch(60% 0.02 240)", fontSize: 11.5, lineHeight: 1.55 }}>
          {fw
      ...
      {fw && (
        <div style={{
          ...
          fontSize: 11, color: "oklch(75% 0.02 240)",
        }}>
          <window.Ico name="clock" size={11} stroke={1.8}
            style={{ color: "oklch(60% 0.02 240)" }} />
          <span>{window.fmtDateTimeS(fw.start)}</span>
          <span style={{ color: "oklch(50% 0.02 240)" }}>→</span>
          <span>{window.fmtDateTimeS(fw.end)}</span>
        </div>
      )}
      ...
      <div style={{ fontSize: 10.5, color: "oklch(50% 0.02 240)",
        marginTop: 2 }}>
