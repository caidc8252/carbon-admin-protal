...
// Severity → left color-bar (Scheme A). Encodes the 4 levels with zero
// column width; rendered as a thin bar at the start of the Ticket cell.
const SEV_BAR = {
  low: "var(--border-3)",
  med: "oklch(70% 0.13 230)",
  high: "oklch(72% 0.15 75)",
  critical: "oklch(62% 0.20 25)"
};
...
},
{
  ...
  { at: "May 15, 2026 09:24:02", actor: "Maya Hassan", kind: "comment",
    note: "Pulled device telemetry — bootloader unlock signature present, not a false positive." },
  { at: "May 15, 2026 09:36:48", actor: "Maya Hassan", kind: "status",
    note: "Quarantined the device — removed from active fleet" },
  ...
  { at: "May 15, 2026 09:51:03", actor: "Maya Hassan", kind: "status",
    note: "Disabled merchant key on TID T-N750-0099-0040 as a precaution" },
  ...
  { at: "May 15, 2026 10:22:31", actor: "Hossein Naderi", kind: "status",
    note: "Picked up from the NPT queue — reviewing attestation logs" },
  { at: "May 15, 2026 10:39:17", actor: "Hossein Naderi", kind: "comment",
    note: "Bootloader unlocked at 03:14 UTC — predates any transaction in the window." },
  { at: "May 15, 2026 10:54:48", actor: "Hossein Naderi", kind: "comment",
    note: "Txn ledger cross-checked — last successful auth 02:58 UTC, nothing settled after the unlock." },
  { at: "May 15, 2026 11:20:05", actor: "Hossein Naderi", kind: "status",
    note: "Determined secure element cannot be trusted — RMA is the safer path" },
  { at: "May 15, 2026 11:38:44", actor: "Hossein Naderi", kind: "status",
    note: "Filed RMA-2026-0517 against the device for replacement" },
  { at: "May 15, 2026 12:25:33", actor: "Hossein Naderi", kind: "comment",
    note: "Replacement staged for next-day dispatch — tracking to follow once it ships." },
  { at: "May 15, 2026 12:47:19", actor: "Maya Hassan", kind: "status",
    note: "Verified quarantine flag — device is out-of-fleet and won't re-bind without approval" },
  { at: "May 15, 2026 13:05:58", actor: "Hossein Naderi", kind: "resolution",
    note: "Forensic report uploaded · root cause: physical bootloader unlock, not a remote exploit" }],

  comments: [
  { at: "May 15, 2026 09:05:12", actor: "Maya Hassan",
    body: "Auto-ticket landed from TamperAgent — integrity check 04 failed, attestation chain broken. Acknowledging and starting triage." },
  { at: "May 15, 2026 09:18:40", actor: "Maya Hassan",
    body: "Pulled the device telemetry. Bootloader unlock signature is present — this isn't a false positive, the secure key area is compromised." },
  ...
  { at: "May 15, 2026 09:51:03", actor: "Maya Hassan",
    body: "Disabled the merchant key on this TID as a precaution. No card data should be processable until we clear it." },
  ...
  { at: "May 15, 2026 10:22:31", actor: "Hossein Naderi",
    body: "Picked up from the NPT queue. Reviewing the attestation logs now — give me ~15 min." },
  { at: "May 15, 2026 10:39:17", actor: "Hossein Naderi",
    body: "Bootloader was unlocked at 03:14 UTC. The unlock predates any transaction in our window, so no live txn keys were exposed during processing." },
  { at: "May 15, 2026 10:54:48", actor: "Hossein Naderi",
    body: "Cross-checked the txn ledger for the MID — last successful auth was 02:58 UTC, before the unlock. Nothing settled after that point." },
  { at: "May 15, 2026 11:07:22", actor: "Maya Hassan",
    body: "Good — so customer funds are not impacted. I'll note that for the merchant comms. What's the path to re-provision?" },
  { at: "May 15, 2026 11:20:05", actor: "Hossein Naderi",
    body: "Re-provision requires a full key-zeroize + re-attestation. The unlocked bootloader means we cannot trust the existing secure element — RMA is the safer call." },
  { at: "May 15, 2026 11:38:44", actor: "Hossein Naderi",
    body: "Filed RMA request RMA-2026-0517 against the device. Engineering will issue a replacement with a fresh attestation chain." },
  { at: "May 15, 2026 12:02:10", actor: "Maya Hassan",
    body: "Thanks. I've let the merchant know the terminal is being swapped and that no transactions were compromised." },
  { at: "May 15, 2026 12:25:33", actor: "Hossein Naderi",
    body: "Engineering confirmed the replacement is staged for next-day dispatch. I'll attach the tracking number once it ships." },
  { at: "May 15, 2026 12:47:19", actor: "Maya Hassan",
    body: "Quarantine flag verified — device shows as out-of-fleet in the console and won't re-bind without manual approval." },
  { at: "May 15, 2026 13:05:58", actor: "Hossein Naderi",
    body: "Forensic report uploaded to the case file. Root cause: physical bootloader unlock, not a remote exploit. Recommend keeping the case open until the replacement is activated." }],
...
// Pads the queue out so the All list has ~20 rows and exercises the
// standard pager. All generated tickets sit in NPT's lanes (working /
// resolved) so the NPT "All" scope reaches 20; a share are handled by
// the current NPT user so "Mine" is populated too.
const GEN_TYPES = ["Payment Failure", "Network Issue", "Hardware", "App Crash", "Security Alert", "Other"];
const GEN_SEVS = ["low", "med", "high", "critical"];
const GEN_DEVICES = ["N950-0014-9281", "N750-0099-0040", "X800-0099-1422", "S60-0488-0021", "S90-0231-7745", "N950-0014-3322"];
const GEN_HANDLERS = ["Hossein Naderi (NPT)", "Priya Raman (NPT)", "NPT Queue"];
function generateTickets(count, startNum) {
  const out = [];
  for (let i = 0; i < count; i++) {
    const num = startNum + i;
    const id = "T-2026-" + String(num).padStart(3, "0");
    const type = GEN_TYPES[i % GEN_TYPES.length];
    const severity = GEN_SEVS[i % GEN_SEVS.length];
    const status = i % 3 === 0 ? "npt-resolved" : "npt-working";
    const nptHandler = GEN_HANDLERS[i % GEN_HANDLERS.length];
    const deviceSn = GEN_DEVICES[i % GEN_DEVICES.length];
    const source = i % 2 ? "manual" : "auto";
    const day = String(10 - i % 9).padStart(2, "0");
    const ts = `May ${day}, 2026 1${i % 9}:0${i % 6}:00`;
    out.push({
      id, source, type, severity, status, deviceSn,
      title: `${type} on ${deviceSn} — escalated for NPT review`,
      description: `${type} reported on ${deviceSn}. Escalated to the platform team for investigation. (Demo ticket #${num}.)`,
      attachments: [],
      ai: {
        summary: `Auto-suggested remediation for ${type.toLowerCase()}.`,
        steps: ["Review the device snapshot and fault window", "Apply the standard remediation runbook", "Confirm the fix with the merchant before closing"],
        confidence: 0.6 + i % 4 * 0.1,
        citedDocs: ["KB-0001 · Standard runbook"],
        generatedAt: ts
      },
      assignedTo: "Maya Hassan (ISO)",
      nptHandler,
      createdBy: source === "manual" ? "Maya Hassan (ISO)" : "Cloud (auto)",
      createdAt: ts, updatedAt: ts,
      timeline: [
      { at: ts, actor: source === "manual" ? "Maya Hassan" : "Cloud", kind: "created", note: source === "manual" ? "Logged by ISO" : "Auto-created by cloud" },
      { at: ts, actor: "Maya Hassan", kind: "escalation", note: "Escalated to NPT" },
      { at: ts, actor: nptHandler.replace(/\s*\(NPT\)$/, ""), kind: "status", note: status === "npt-resolved" ? "NPT replied with a resolution" : "NPT picked up — investigating" }],
      comments: [],
      snapshotOverride: { capturedAt: ts },
      faultWindow: { start: ts, end: ts },
      deviceReport: {
        faultCode: "demo.generated",
        faultMessage: `${type} on ${deviceSn}`,
        component: type,
        mid: "m_demo_" + num,
        tid: "T-" + deviceSn,
        appVersion: "POS-Pro 4.3.2",
        androidVersion: "13",
        networkState: "Wi-Fi · connected"
      }
    });
  }
  return out;
...
window.TICKETS = window.TICKETS || [...TICKETS_SEED, ...generateTickets(17, 9)].map((t) => ({ ...t }));
...
function TicketsListScreen({ navigate }) {
  ...
  const [scope, setScope] = useStateT(() => {
    // Default-tab rule:
    //   1. Mine has data → default to Mine
    //   2. Mine is empty → default to All
    // A previously-stored explicit choice is respected, except a stored
    // "mine" with no data falls back to All (rule 2 still wins).
    const mineHasData = all.some((t) =>
      isMine(t, me) && t.status !== "new" && t.status !== "closed");
    let stored = null;
    try { stored = localStorage.getItem(scopeKey); } catch {}
    if (stored === "all") return "all";
    if (stored === "mine") return mineHasData ? "mine" : "all";
    return mineHasData ? "mine" : "all";
  });
  useEffectT(() => {
  ...
  const [statusFilters, setStatusFilters] = useStateT(makeDefaultStatus);
  ...
  // Advanced-search Sheet open/closed. Quick bar keeps Keyword + Status;
  // Type / Severity / Source / Assignee live inside this drop-down Sheet.
  const [advOpen, setAdvOpen] = useStateT(false);

  // ── Deferred apply (搜索栏约定 R-01) ──────────────────────────
  // The controls above are the DRAFT; nothing filters until the user
  // clicks Search (or hits Enter). `applied` is the committed snapshot
  // the list + chip counts read from.
  const [applied, setApplied] = useStateT(() => ({
    q: "", statusFilters: makeDefaultStatus(),
    typeFilter: "any", sevFilter: "any", sourceFilter: "any", assigneeFilter: "any"
  }));
  const runSearch = () => setApplied({
    q, statusFilters: new Set(statusFilters),
    typeFilter, sevFilter, sourceFilter, assigneeFilter
  });
  const statusIsDefault = (set) =>
    set.size === STATUS_FILTER_BUCKETS.length - 1 && !set.has("closed");
  const statusLabel = (set) => {
    if (set.size === 0) return "None";
    if (set.size === STATUS_FILTER_BUCKETS.length) return "All";
    return STATUS_FILTER_BUCKETS.filter((b) => set.has(b.id)).map((b) => b.label).join(", ");
  };
  const hasFilters =
    !!applied.q ||
    !statusIsDefault(applied.statusFilters) ||
    applied.typeFilter !== "any" ||
    applied.sevFilter !== "any" ||
    applied.sourceFilter !== "any" ||
    (scope === "all" && applied.assigneeFilter !== "any");
  const clearAll = () => {
    setQ(""); setStatusFilters(makeDefaultStatus());
    setTypeFilter("any"); setSevFilter("any"); setSourceFilter("any"); setAssigneeFilter("any");
    setApplied({ q: "", statusFilters: makeDefaultStatus(), typeFilter: "any", sevFilter: "any", sourceFilter: "any", assigneeFilter: "any" });
  };
  // Reset one filter to its default — updates both draft + applied so the
  // chip disappears and the list updates immediately.
  const clearOne = (key) => {
    const resets = {
      q: () => { setQ(""); return ""; },
      statusFilters: () => { const d = makeDefaultStatus(); setStatusFilters(d); return d; },
      typeFilter: () => { setTypeFilter("any"); return "any"; },
      sevFilter: () => { setSevFilter("any"); return "any"; },
      sourceFilter: () => { setSourceFilter("any"); return "any"; },
      assigneeFilter: () => { setAssigneeFilter("any"); return "any"; }
    };
    const v = resets[key]();
    setApplied((a) => ({ ...a, [key]: v }));
  };
  ...
  const matchesFilters = (t) => {
    ...
    if (!applied.statusFilters.has(bucket)) return false;
    if (applied.typeFilter !== "any" && t.type !== applied.typeFilter) return false;
    if (applied.sevFilter !== "any" && t.severity !== applied.sevFilter) return false;
    if (applied.sourceFilter !== "any" && t.source !== applied.sourceFilter) return false;
    if (applied.q) {
      const n = applied.q.toLowerCase();
  ...
  [all, me, applied]);
  const allList = useMemoT(() => all.filter((t) => matchesFilters(t) && (scope === "all" ? applied.assigneeFilter === "any" ?
  true :
  applied.assigneeFilter === "__unassigned" ? !t[assigneeField] : t[assigneeField] === applied.assigneeFilter : true)),
  [all, applied, scope, assigneeField]);
  ...
  // ── Standard pagination ──────────────────────────────────
  const [pageSize, setPageSize] = useStateT(25);
  const [page, setPage] = useStateT(1);
  // Reset to page 1 whenever the result set changes (search / scope switch).
  useEffectT(() => { setPage(1); }, [applied, scope]);
  ...
  // ── Advanced-search Sheet helpers ────────────────────────
  // Which of the four advanced filters are currently APPLIED — drives the
  // badge on the Advanced button so the operator can see, even with the
  // Sheet closed, that hidden conditions are narrowing the list (R-04).
  const advAppliedCount =
    (applied.sevFilter !== "any" ? 1 : 0) +
    (applied.sourceFilter !== "any" ? 1 : 0) +
    (scope === "all" && applied.assigneeFilter !== "any" ? 1 : 0);
  // Same count over the DRAFT — used inside the Sheet so the operator sees
  // how many conditions they've staged before committing.
  const advDraftCount =
    (sevFilter !== "any" ? 1 : 0) +
    (sourceFilter !== "any" ? 1 : 0) +
    (scope === "all" && assigneeFilter !== "any" ? 1 : 0);
  // Live preview of how many rows the CURRENT DRAFT would match — read by
  // the Sheet footer (currently no count shown, but kept for reuse).
  // Commit the whole draft and close the Sheet.
  const runSearchAndClose = () => { runSearch(); setAdvOpen(false); };
  // Reset only the advanced conditions (keep keyword + status). Mirrors the
  // chip × behaviour — updates draft AND applied so it takes effect at once.
  const resetAdvanced = () => {
    setSevFilter("any"); setSourceFilter("any"); setAssigneeFilter("any");
    setApplied((a) => ({ ...a, sevFilter: "any", sourceFilter: "any", assigneeFilter: "any" }));
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%" }}>
      <window.TitleBar
        ...
        tabs={[
          { id: "all", label: "All", active: scope === "all", onClick: () => setScope("all") },
          { id: "mine", label: "Mine", active: scope === "mine", onClick: () => setScope("mine") },
        ]} />

      <div style={{ background: "var(--color-bg-1)" }}>
        {/* Condition band — scrolls with content per ui-spec §7.2 (not sticky) */}
        <div style={{
          background: "var(--color-bg-1)"
        }}>
        {/* Quick search bar — 关键字 + 状态 + Search + Advanced（其余条件收进 Sheet） */}
        <div className="sb__row" onKeyDown={(e) => { if (e.key === "Enter") runSearch(); }}
          style={{ padding: "var(--space-6) var(--space-6) 0" }}>
          <window.SearchInput value={q} onChange={setQ} onSearch={runSearch}
            placeholder="Search ID, title, device SN…" width={260}/>
          <StatusFilterDropdown buckets={STATUS_FILTER_BUCKETS}
            value={statusFilters} onChange={setStatusFilters} />
          <window.SearchSelect value={typeFilter} onChange={setTypeFilter} width={170}
            options={[{ value: "any", label: "All types" }, ...TICKET_TYPES.map((t) => ({ value: t, label: t }))]} />
          <button type="button" className="sb__primary" onClick={runSearch}>
            <window.Ico name="search" size={13} />
            Search
          </button>
          {/* Advanced toggle — opens the drop-down Sheet; badge = applied adv conditions */}
        ...
        <div onKeyDown={(e) => { if (e.key === "Enter") runSearchAndClose(); }}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--fg1)" }}>Advanced filters</span>
              <button type="button" onClick={() => setAdvOpen(false)}
                style={{ marginLeft: "auto", background: "transparent", border: 0, cursor: "pointer",
                  color: "var(--fg3)", display: "grid", placeItems: "center", padding: 4, borderRadius: 6 }}
                title="Close">
                <window.Ico name="x" size={14} />
              </button>
            </div>

            <div style={{ padding: "16px" }}>
              {/* Group A · Ticket attributes */}
              <div style={{ marginBottom: scope === "all" ? 18 : 0 }}>
                <div style={advSubStyle}>Ticket attributes</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px 18px" }}>
                  <div>
                    <label style={advLblStyle}>Severity</label>
                    <window.SearchSelect value={sevFilter} onChange={setSevFilter} width="100%"
                      options={[{ value: "any", label: "All severities" }, ...Object.keys(TICKET_SEVERITY).map((s) => ({ value: s, label: TICKET_SEVERITY[s].label }))]} />
                  </div>
                  <div>
                    <label style={advLblStyle}>Source</label>
                    <window.SearchSelect value={sourceFilter} onChange={setSourceFilter} width="100%"
                      options={[{ value: "any", label: "All sources" }, { value: "auto", label: "Auto (cloud)" }, { value: "manual", label: "Manual (ISO)" }]} />
                  </div>
                </div>
              </div>

              {/* Group B · Assignment — only meaningful in All scope */}
              {scope === "all" &&
              <div>
                <div style={advSubStyle}>{role === "NPT" ? "NPT handler" : "Assignment"}</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px 18px" }}>
                  <div>
                    <label style={advLblStyle}>{role === "NPT" ? "Handler" : "Assignee"}</label>
                    <window.SearchSelect value={assigneeFilter} onChange={setAssigneeFilter} width="100%"
                      options={[{ value: "any", label: role === "NPT" ? "All handlers" : "All assignees" }, { value: "__unassigned", label: "— Unassigned —" }, ...assigneeOptions.map((a) => ({ value: a, label: a }))]} />
                  </div>
                </div>
              </div>
              }
            ...
            <div style={{
              ...
              padding: "12px 16px", borderTop: "1px solid var(--border-1)", background: "var(--bg1)"
            }}>
              <button type="button" className="sb__primary" onClick={runSearchAndClose}>
                <window.Ico name="search" size={13} />
                Search
              </button>
              <button type="button" onClick={resetAdvanced}
                disabled={advDraftCount === 0 && advAppliedCount === 0}
                style={{
        ...
        {/* Active filters */}
        <div style={{ padding: hasFilters ? "var(--space-4) var(--space-6) 0" : 0 }}>
          <window.ActiveFilters onClearAll={clearAll} hasFilters={hasFilters}>
            {applied.q ? <window.FilterChip label={`Search: ${applied.q}`} onRemove={() => clearOne("q")} /> : null}
            {!statusIsDefault(applied.statusFilters) ? <window.FilterChip label={`Status: ${statusLabel(applied.statusFilters)}`} onRemove={() => clearOne("statusFilters")} /> : null}
            {applied.typeFilter !== "any" ? <window.FilterChip label={`Type: ${applied.typeFilter}`} onRemove={() => clearOne("typeFilter")} /> : null}
            {applied.sevFilter !== "any" ? <window.FilterChip label={`Severity: ${TICKET_SEVERITY[applied.sevFilter]?.label || applied.sevFilter}`} onRemove={() => clearOne("sevFilter")} /> : null}
            {applied.sourceFilter !== "any" ? <window.FilterChip label={`Source: ${applied.sourceFilter === "auto" ? "Auto (cloud)" : "Manual (ISO)"}`} onRemove={() => clearOne("sourceFilter")} /> : null}
            {scope === "all" && applied.assigneeFilter !== "any" ? <window.FilterChip label={`Assignee: ${applied.assigneeFilter === "__unassigned" ? "Unassigned" : applied.assigneeFilter}`} onRemove={() => clearOne("assigneeFilter")} /> : null}
          </window.ActiveFilters>
        </div>
        </div>{/* /sticky filter band */}

        {/* Table */}
        <div style={{ padding: "var(--space-4) var(--space-6) var(--space-6)" }}>
          <div style={{ background: "var(--bg2)", border: "1px solid var(--border-2)",
            borderRadius: "var(--radius-lg)", overflow: "clip", boxShadow: "var(--shadow-1)" }}>
            <div style={{ position: "sticky", top: 0, zIndex: 6, display: "flex", alignItems: "center", padding: "10px 16px", borderBottom: "1px solid var(--border-1)", background: "var(--bg2)", fontSize: 12, color: "var(--fg2)" }}>
              <span><strong className="mono num" style={{ color: "var(--fg1)", fontWeight: 500 }}>{filtered.length}</strong> {filtered.length === 1 ? "ticket" : "tickets"}{hasFilters && <span style={{ color: "var(--fg3)" }}> matching filters</span>}</span>
            </div>
            <div>
              <style>{`
                .tk-list-table td, .tk-list-table th { overflow: hidden; }
                .tk-list-table td > * { max-width: 100%; }
                .tk-list-table tbody tr:last-child { border-bottom: none !important; }
              `}</style>
              <table className="tk-list-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, tableLayout: "fixed" }}>
                <colgroup>
                  <col style={{ width: scope === "mine" ? "51%" : "37%" }} />{/* Ticket — merged primary cell */}
                  <col style={{ width: "11%" }} />{/* Type */}
                  <col style={{ width: "18%" }} />{/* Status */}
                  {scope !== "mine" && <col style={{ width: "16%" }} />}{/* Assignee */}
                ...
                <tbody>
                  {filtered.length === 0 &&
                  <tr><td colSpan={scope === "mine" ? 5 : 6} style={{ padding: "var(--space-8)", textAlign: "center", color: "var(--fg3)" }}>
                    ...
                    return (
                      ...
                      onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                        <td style={{ padding: 0, overflow: "hidden" }}>
                          <div style={{ display: "flex", gap: 11, padding: "12px 14px", minWidth: 0 }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
                              <div style={{
                                fontSize: 13, fontWeight: 600, color: "var(--fg1)", lineHeight: 1.3,
                                display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden"
                              }}>{t.title}</div>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", minWidth: 0 }}>
                                <span title={t.source === "auto" ? "Auto (cloud)" : "Manual (ISO)"} style={{
                                  width: 18, height: 18, borderRadius: 5, flexShrink: 0,
                                  display: "inline-grid", placeItems: "center",
                                  background: t.source === "auto" ? "oklch(94% 0.03 230)" : "var(--bg3)",
                                  color: t.source === "auto" ? "oklch(40% 0.16 230)" : "var(--fg2)",
                                  border: "1px solid var(--border-1)"
                                }}>
                                  <window.Ico name={t.source === "auto" ? "bolt" : "edit"} size={10} stroke={1.8} />
                                </span>
                                <span className="mono" style={{ fontSize: 11.5, fontWeight: 500, color: "var(--fg2)" }}>{t.id}</span>
                                <span className="mono" style={{ fontSize: 11, color: "var(--fg3)",
                                  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 }}>{t.deviceSn}</span>
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td style={{ padding: "12px 14px", fontSize: 11.5, color: "var(--fg2)" }}>
                          <span className="mono">{window.fmtDateTimeS(t.updatedAt)}</span>
                        </td>
                        <td style={{ padding: "12px 14px", textAlign: "right" }}>
                          <window.Icon name="chevR" size={14} style={{ color: "var(--fg3)" }} />
                        </td>
                      </tr>);
            ...
            <window.Pagination
              ...
              totalUnfiltered={scope === "mine"
                ? all.filter((t) => isMine(t, me) && t.status !== "new" && t.status !== "closed").length
                : all.length}
...
// ── Pager button styles ──────────────────────────────────
function tkPagerBtn(disabled) {
  return {
    display: "grid", placeItems: "center", width: 28, height: 28,
    borderRadius: 6, border: "1px solid var(--color-border-default)",
    background: "var(--bg2)", color: disabled ? "var(--fg4)" : "var(--fg2)",
    cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1
  };
}
function tkPagerNum(active) {
  return {
    minWidth: 28, height: 28, padding: "0 8px",
    borderRadius: 6, border: "1px solid " + (active ? "var(--color-primary-600)" : "var(--color-border-default)"),
    background: active ? "var(--color-primary-600)" : "var(--bg2)",
  ...
  return (
    ...
    style={{
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 2 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span className="mono" style={{ fontSize: 12.5, fontWeight: 500, color: "var(--fg1)" }} data-comment-anchor="2d8501e5f1-span-1524-11">
...
function NewTicketScreen({ navigate, presetSn }) {
  ...
  // Why the Create button is disabled — surfaced next to it so the
  // operator isn't left guessing. The common trap: typing an SN in the
  // picker is NOT the same as selecting a device — a candidate row must
  // be picked before `sn` is set.
  const blockers = [];
  if (!matchedDevice) blockers.push("select a device from the list");
  if (trimmedDesc.length <= 4) blockers.push("add a description");
  const blockReason = blockers.length
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <window.TitleBar
        back
        onBack={() => presetSn
          ? navigate({ screen: "deviceDetail", deviceSn: presetSn })
          : navigate({ screen: "tickets" })}
        backLabel={presetSn ? `Back to ${presetSn}` : "Back to Tickets"}
        title="New ticket"
        actions={
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {!canSubmit &&
              <span style={{
                display: "inline-flex", alignItems: "center", gap: 6,
                maxWidth: 280, fontSize: 12, lineHeight: 1.3,
                color: "var(--fg3)", textAlign: "right"
              }}>
                <window.Ico name="info" size={14} style={{ flexShrink: 0 }} />
                {blockReason}
              </span>
            }
...
function TicketDetailScreen({ ticket, navigate }) {
  ...
  // Scroll-driven header collapse: when the .content scroll container is
  // scrolled past a small threshold, the sticky header shrinks to a compact
  // two-line state (ID+chips+actions / title) and hides the device-meta line
  // and status stepper. Restores on scroll back to top.
  const headRef = React.useRef(null);
  const [collapsed, setCollapsed] = useStateT(false);
  useEffectT(() => {
    const scroller = headRef.current && headRef.current.closest(".content");
    if (!scroller) return;
    const onScroll = () => setCollapsed(scroller.scrollTop > 56);
    onScroll();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%" }}>
      {/* ─── Header ─── */}
      <div ref={headRef} style={{ position: "sticky", top: 0, zIndex: 20, background: "var(--color-bg-2)", borderBottom: "1px solid var(--color-border-subtle)" }}>
        <div style={{ padding: collapsed ? "9px 24px" : "16px 24px 0", display: "flex", alignItems: collapsed ? "center" : "flex-start", gap: 14, flexWrap: "wrap", transition: "padding .16s ease" }}>
          <button type="button" onClick={() => navigate({ screen: "tickets" })}
          className="titlebar__back" title="Back to Tickets" aria-label="Back to Tickets">
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              {collapsed && (
                <span style={{ fontSize: 13.5, fontWeight: 600, color: "var(--fg1)", letterSpacing: "-0.01em",
                  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 520,
                  borderLeft: "1px solid var(--color-border-default)", paddingLeft: 10, marginLeft: 2 }}>
                  {ticket.title}
                </span>
              )}
            </div>
            <h1 style={{ margin: "6px 0 0", fontSize: 19, fontWeight: 600, lineHeight: 1.3,
              letterSpacing: "-0.01em", maxWidth: 880, textWrap: "pretty", display: collapsed ? "none" : "block" }}>
              {ticket.title}
            </h1>
            <div style={{ marginTop: 6, fontSize: 12, color: "var(--fg3)", display: collapsed ? "none" : "block" }}>
              On{" "}
              ...
              {" · "}<span className="mono">created {window.fmtDateTimeS(ticket.createdAt)}</span>
            </div>
          </div>
          {/* Actions */}
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginLeft: "auto", alignItems: "flex-end" }}>
            ...
            <div style={{ fontSize: 10.5, color: "var(--fg3)", display: collapsed ? "none" : "flex", alignItems: "center", gap: 6 }}>
        ...
        {/* Status stepper */}
        <div style={{ padding: "16px 24px 16px", display: collapsed ? "none" : "block" }}>
    ...
    <span style={{
      ...
      <div style={{
        ...
        <span style={{ marginLeft: "auto", fontSize: 10.5 }}>
          generated <span className="mono">{window.fmtDateTimeS(ai.generatedAt)}</span>
        </span>
      </div>
    </window.Card>);
...
function DeviceSnapshotCard({ snapshot, device, onOpenDevice }) {
  ...
  return (
    ...
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
          ...
          <span className="mono" style={{ fontSize: 11, color: "var(--fg2)" }}>{window.fmtDateTimeS(snapshot.capturedAt)}</span>
          ...
          {snapshot.recentEvents.map((e, i) => {
            ...
            return (
              <li key={i} style={{
                ...
                <span className="mono" style={{ fontSize: 10.5, color: "var(--fg3)", minWidth: 56 }}>{window.fmtTimeS(e.at)}</span>
                <span style={{
...
function CommentsCard({ ticket, comment, setComment, onSubmit, role }) {
  ...
  // Newest first (the `at` strings like "May 15, 2026 11:21:02" parse cleanly).
  const sorted = React.useMemo(
    () => [...all].sort((a, b) => new Date(b.at) - new Date(a.at)),
    [all]
  );
  const [visible, setVisible] = useStateT(10);
  const [loadingMore, setLoadingMore] = useStateT(false);
  const shown = sorted.slice(0, visible);
  const hasMore = sorted.length > visible;
  const loadMore = () => {
    if (loadingMore) return;
    setLoadingMore(true);
    setTimeout(() => { setVisible((v) => v + 10); setLoadingMore(false); }, 1000);
  };
  return (
    <window.Card title={`Comments · ${all.length}`}
    hint="Internal conversation between ISO and NPT. Customer doesn't see these.">
            ...
            <li key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <span className="mono" style={{ fontSize: 10.5, color: "var(--fg3)" }}>{window.fmtDateTimeS(c.at)}</span>
                  </div>
                  <div style={{
      ...
      {hasMore && (
        <div style={{ marginTop: 12, display: "flex", justifyContent: "center" }}>
          <window.Button size="sm" onClick={loadMore} loading={loadingMore} disabled={loadingMore}
          icon={loadingMore ? undefined : "chevd"}>
  ...
  return (
    <window.Card title="Ticket info">
      ...
      <div style={{
        ...
        <Kv label="Created" value={<span className="mono" style={{ fontSize: 12 }}>{window.fmtDateTimeS(ticket.createdAt)}</span>} />
        <Kv label="Updated" value={<span className="mono" style={{ fontSize: 12 }}>{window.fmtDateTimeS(ticket.updatedAt)}</span>} />
        {ticket.faultWindow &&
...
function FaultWindowRange({ window: w }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2,
      ...
      <span>{window.fmtDateTimeS(w.start)}</span>
      <span style={{ color: "var(--fg3)" }}>
        <span style={{ marginRight: 6 }}>→</span>{window.fmtDateTimeS(w.end)}
      </span>
    </div>);
...
function TimelineCard({ timeline }) {
  ...
  return (
    <window.Card title={`Activity · ${sorted.length}`}>
      <ol style={{ margin: 0, padding: 0, listStyle: "none",
        display: "flex", flexDirection: "column", gap: 0 }}>
        {shown.map((e, i, arr) => {
          const isLast = i === arr.length - 1;
          // Two-tone dots: key milestones (created / escalation / resolution)
          // read as primary; routine events (status / comment / ai) are muted.
          const isMilestone = e.kind === "created" || e.kind === "escalation" || e.kind === "resolution";
          const dotColor = isMilestone ? "var(--color-primary-600)" : "var(--color-text-tertiary)";
          return (
            <li key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start",
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, width: 8 }}>
                <span style={{
                  ...
                  background: dotColor, marginTop: 5, flex: "none"
                }} />
                {!isLast &&
                <span style={{ width: 1, flex: 1, marginTop: 3,
              ...
              <div style={{ flex: 1, minWidth: 0, paddingTop: 0 }}>
                ...
                <div style={{ marginTop: 2, fontSize: 10.5, color: "var(--fg3)",
                  ...
                  <span className="mono">{window.fmtDateTimeS(e.at)}</span>
                  <span>· {e.actor}</span>
                </div>
              </div>
            </li>);
...
// Advanced-Sheet field label + sub-group heading styles.
const advLblStyle = {
...
const advSubStyle = {
  marginBottom: 10,
  fontSize: 10.5, fontWeight: 600, color: "var(--fg2)",
  letterSpacing: "0.06em", textTransform: "uppercase"
};
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {device && <> · {device.model}</>}
          {snapshot && <> · snapshot captured <span className="mono">{window.fmtDateTimeS(snapshot.capturedAt)}</span></>}
        </div>
      </div>
        ...
        return (
          <div style={{
            ...
            <div className="mono" style={{ color: "var(--fg3)", fontSize: 11 }}>
              {window.fmtDateTimeS(lastResolution.at)}
            </div>
