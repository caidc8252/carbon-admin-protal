...
// ─── Contract enums ────────────────────────────────────
//
// CONTRACT_TYPES — the contract kinds an entity can hold.
...
//   ISO-PILOT   — ISO trial contract. Same authorizer rules as ISO. All service
//                 fees forced to 0. "Has this partner ever piloted?" = the
//                 partner holds (or held) any *-PILOT contract. Convert-to-paid
//                 = TERMINATE the *-PILOT row + CREATE a fresh ISO row.
//   ISV         — Independent Software Vendor. Same authorizer rules as ISO.
//   ISV-PILOT   — ISV trial contract (mirror of ISO-PILOT for the ISV track).
//   MERCHANT    — Direct merchant. Authorizer MUST hold an ACTIVE ISO contract
//                 (NPT cannot directly issue MERCHANT contracts).
//
// PILOT is a CONTRACT TYPE, not a status (revised model). The status enum
// therefore no longer carries PILOT — a pilot is simply an ACTIVE contract
// whose type ends in '-PILOT'. EXPIRED remains a *derived* state produced by
// effectiveStatus() when effectiveTo has passed (not persisted here).
const CONTRACT_TYPES = ['ADMIN', 'ISO', 'ISO-PILOT', 'ISV', 'ISV-PILOT', 'MERCHANT'];

// Convenience: the pilot-flavored types + a helper to test for them.
const PILOT_CONTRACT_TYPES = ['ISO-PILOT', 'ISV-PILOT'];
const isPilotContractType = (t) => /-PILOT$/.test(String(t || '').toUpperCase());

// CONTRACT_STATUSES — the lifecycle of a single entity_contract row.
// Revised model: PILOT + the legacy PENDING placeholder are both gone.
const CONTRACT_STATUSES = ['ACTIVE', 'SUSPENDED', 'TERMINATED'];

// CONTRACT_EVENT_TYPES — the EntityContractEvent vocabulary (revised model).
// Pilot-specific verbs (BIND_PILOT / PILOT_EXTEND / PILOT_CONVERT) are gone;
// the flat verbs below cover every contract regardless of (pilot) type.
const CONTRACT_EVENT_TYPES = ['CREATE_CONTRACT', 'EDIT_CONTRACT', 'RENEW', 'SUSPEND', 'RESUME', 'TERMINATE'];
...
const SEED_ENTITIES = [
  ...
  // ─── ISV + ISO + MERCHANT (Helios — mixed portfolio) ──
  {
    ...
    email: 'elena.rossi@helios.example',
    country: 'US',
  ...
  // ─── ISO authorized by NPT (Brightleaf — delegated to from Northwind) ──
  {
    ...
    email: 'henry.tremblay@brightleaf.example',
    country: 'CA',
  ...
  // ─── ISV authorized by NPT + ISO authorized by NPT (Vanta — software-led) ──
  {
    ...
    status: 'ACTIVE',
    name: 'Vanta Software',
    email: 'lukas.meier@vanta.example',
    country: 'CH',
  ...
  // ─── ISO authorized by NPT, currently SUSPENDED (Coastline — compliance review) ──
  {
    ...
    email: 'maria.alvarez@coastline.example',
    country: 'US',
  ...
  // ─── ISV authorized by NPT (Loomis — onboarded recently) ──
  {
    ...
    email: 'eli.wagner@loomis.example',
    country: 'US',
  ...
  // ─── MERCHANT authorized by an ISO (Bluefin — under Northwind) ──
  {
    ...
    status: 'ACTIVE',
    name: 'Bluefin Market',
    email: 'akiko.tanaka@bluefin.example',
    country: 'US',
  ...
  // ─── ISO PENDING (Aurora — invite outstanding, not consumed) ──
  {
    ...
    status: 'ONBOARDING',
    name: 'Aurora Freight',
    email: 'ceo@aurora-freight.example',
    country: 'DE',
  ...
  // ─── MERCHANT PENDING + expired invite (Sandstone — under Brightleaf) ──
  {
    ...
    email: 'ops@sandstone-bistro.example',
    country: 'AU',
  ...
  // ─── ISV ACTIVE (Westport — admin already activated) ──
  {
    ...
    email: 'megan.oconnell@westport-marine.example',
    country: 'CA',
  ...
  // ─── Sub-ISO authorized by Northwind (NEW — demonstrates ISO → ISO authorization) ──
  {
    ...
    email: 'rosa.lin@cascade-ch.example',
    country: 'US',
  ...
  // ─── MERCHANT authorized by Northwind, delegated to Brightleaf (NEW) ──
  {
    ...
    email: 'theo.marsh@sunbreak.example',
    country: 'US',
  ...
  // ─── MERCHANT authorized by Brightleaf, delegated to Cascade Channels (NEW) ──
  {
    ...
    email: 'daxton.reyes@stonefield.example',
    country: 'CA',
  ...
  // the PILOT contract details + their EntityContractEvent streams.
  {
    ...
    email: 'iris.nakamura@wildflower-goods.example',
    country: 'US',
  ...
  },
  {
    ...
    email: 'drew.whitaker@trailhead-outfitters.example',
    country: 'US',
...
const SEED_ENTITY_CONTRACTS = [
  {
    id: 'ec-admin-npt',
    authorizingPartnerContractType: 'ADMIN',
  ...
  },
  {
    ...
    // Expired demo: effective-to has passed → effectiveStatus() derives
    // EXPIRED. Renewable from the contract row's kebab menu (Renew contract).
    effectiveTo: '2026-05-28T00:00:00Z',
    terminatedByEntityId: null,
    terminatedAt: null,
    status: 'ACTIVE',
    entitlements: {},
  ...
  {
    id: 'ec-007-merch',
    authorizingPartnerContractType: 'ISO',
    ...
    entitlements: {
      ...
      'PAYMENT SERVICE': {
        enable: true,
        appItems: [
          { pkgName: 'com.newland.payexplorer', price: 0 },   // per device · monthly (0 = free)
        ],
      },
      'E-RECEIPT SERVICE': {
        enable: true,
  ...
  },
  {
    ...
    entitlements: {
      ...
      'PAYMENT SERVICE': {
        enable: true,
        appItems: [
          { pkgName: 'com.newland.payexplorer', price: 0 },
        ],
      },
      'E-RECEIPT SERVICE': {
        enable: false,
  ...
  // ── PILOT samples (revised model) ──────────────────────────
  // PILOT is now a CONTRACT TYPE ('ISO-PILOT' / 'ISV-PILOT'), not a status.
  // A pilot is an ACTIVE contract whose type ends in '-PILOT', with every
  // service fee forced to 0. It expires naturally via effectiveStatus()
  // when effectiveTo passes (derived EXPIRED). "Convert to paid" =
  // TERMINATE this *-PILOT row + CREATE a fresh ISO/ISV row.
  // The data.jsx derivation layer re-projects these rows to the legacy
  // { kind:'ISO', status:'PILOT' } shape so existing pilot UI keeps working.

  // c-015 · Fresh ISO-PILOT — Day 47 of 180. All fees $0.
  {
    id: 'ec-015-iso',
    authorizingPartnerContractType: 'ADMIN',
    authorizedEntityId: 'c-015',
    authorizedContractType: 'ISO-PILOT',
    authorizingEntityId: 'e-npt',
    ...
    entitlements: {
      ...
      deviceModels: ['m-001', 'm-002'],
      FlyDesk:       { enable: true },
      GeoLocation:   { enable: false },
      GeoFencing:    { enable: false },
      'Pre-warning': { enable: true },
    },
    events: [
      // EntityContractEvent (revised vocabulary): CREATE_CONTRACT replaces
      // the old BIND_PILOT verb — the pilot nature lives in the type now.
      {
        eventType: 'CREATE_CONTRACT',
        at: '2026-04-09T09:32:00Z',
        by: 'jordan.diaz@carbon',
        eventInfo: { contractType: 'ISO-PILOT', initialEffectiveTo: '2026-10-06T09:32:00Z' },
        // Legacy fields for existing readers
        statusFrom: null, statusTo: 'ACTIVE',
        description: 'ISO pilot started · 180-day trial · all fees locked at $0',
      },
    ],
  },

  // c-016 · About-to-expire ISO-PILOT — started 173 days ago (now T-7).
  // Reminders fired at T-15 / T-7.
  {
    id: 'ec-016-iso',
    authorizingPartnerContractType: 'ADMIN',
    ...
    events: [
      {
        ...
        by: 'jordan.diaz@carbon',
        eventInfo: { contractType: 'ISO-PILOT', initialEffectiveTo: '2026-06-02T11:14:00Z' },
        statusFrom: null, statusTo: 'ACTIVE',
        description: 'ISO pilot started · 180-day trial · all fees locked at $0',
      },
      // Not yet renewed in the seed (UI shows "expiring soon" without a
      // renewal trace). Add a RENEW event here to demo the renewal history.
    ],
  },
];
...
//   mfaEnable, status (ACTIVE/LOCKED/DELETE),
//   lastLoginAt, passwordHistory (max 5), remark.
//
//   status enum (revised model):
//     ACTIVE   — normal, can sign in
//     LOCKED   — auto-locked (too many failed logins) or admin-locked; revivable
//     DELETE   — admin-deleted account (soft delete). Terminal: sign-in refused,
//                cannot be reactivated. Row is retained so audit / relationship
//                references stay attributable; the platform Users list hides it.
//   (NOTE: one legacy customer-operator seed below still carries 'DISABLED' —
//    out of scope for the current platform-Users work, pending migration.)
//
...
const SEED_USERS = [
  ...
  },
  {
    ...
    mfaEnable: false, status: 'DISABLED', lastLoginAt: '2026-04-28T22:48:00Z',
    passwordHistory: [],
    remark: 'Account deactivated by Northwind admin on 2026-05-02 — left the company. Retained for audit.',
  },
...
//
// status (revised model): string enum 'ENABLED' | 'PENDING'.
//   ENABLED — TOTP active and verified.
//   PENDING — secret minted, awaiting first successful verification.
// (The legacy int encoding 0/1/2 and the DISABLE state have been removed —
//  a user without MFA simply has no MFA INFO row.)
// =====================================================================
const SEED_MFA_INFO = [
  { id: 'mfa-1',     userId: 'u-1',     mfaType: 'TOTP', secretEncrypted: 'enc:JBSWY3DPEHPK3PXP',     failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-2',     userId: 'u-2',     mfaType: 'TOTP', secretEncrypted: 'enc:KRSXG5BAOJSWG4TFOQ',   failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-3',     userId: 'u-3',     mfaType: 'TOTP', secretEncrypted: 'enc:MFRGG43BNFUHGZLOMQ',   failTimes: 1, lastFailTimestamp: '2026-05-08T09:14:00Z', status: 'ENABLED' },
  { id: 'mfa-5',     userId: 'u-5',     mfaType: 'TOTP', secretEncrypted: 'enc:NBSWY3DPMRSXSI3F',     failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c001-1',userId: 'u-c001-1',mfaType: 'TOTP', secretEncrypted: 'enc:OBQXG43XN5ZGI2DJN5XQ', failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c001-3',userId: 'u-c001-3',mfaType: 'TOTP', secretEncrypted: 'enc:PBSXSI3VMVZWS3DJN5XS', failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c002-1',userId: 'u-c002-1',mfaType: 'TOTP', secretEncrypted: 'enc:QBQXG43XN5ZGSY3DMNUW', failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c003-1',userId: 'u-c003-1',mfaType: 'TOTP', secretEncrypted: 'enc:RBSWS43JN5XGSZTV',     failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c004-1',userId: 'u-c004-1',mfaType: 'TOTP', secretEncrypted: 'enc:SBSXG43BN5ZGOX3FNZ',   failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c006-1',userId: 'u-c006-1',mfaType: 'TOTP', secretEncrypted: 'enc:TBSXG3DTMRZWG4TF',     failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c007-1',userId: 'u-c007-1',mfaType: 'TOTP', secretEncrypted: 'enc:UBQWG33VMVZGS43F',     failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c011-1',userId: 'u-c011-1',mfaType: 'TOTP', secretEncrypted: 'enc:VBSWS43BN5XGOX3FN5XW', failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c012-1',userId: 'u-c012-1',mfaType: 'TOTP', secretEncrypted: 'enc:WBSWG4DBN5XGSZTV',     failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
  { id: 'mfa-c014-1',userId: 'u-c014-1',mfaType: 'TOTP', secretEncrypted: 'enc:XBSWG43VMVZGSY3DOJ',   failTimes: 0, lastFailTimestamp: null,                  status: 'ENABLED' },
];
...
//                            (revised model: EXPIRED removed — an expired
//                             authorization window is derived from
//                             authorizingTo, not stored as a status.
...
const SEED_ENTITY_USER_RELATIONSHIPS = [
  ...
  // ─── Jordan Diaz (u-2) — multi-Partner memberships ──────────
  // The signed-in demo account belongs to several Partners in addition
  // to the NPT Admin Console (eur-npt-2 above). Each membership powers a
  // card on the "Switch partner" page. Per §2.5a, the destination portal
  // is derived from the Partner's CONTRACT TYPE — not stored here.
  {
    // Northwind — ISO + ISV (Partner Portal). Jordan is org admin here.
    id: 'eur-u2-c001', entityId: 'c-001', userId: 'u-2',
    authorizingType: 'ADMIN', roleIds: [],
    authorizingTimestamp: '2025-09-05T11:20:00Z',
    authorizingUserId: 'u-1', authorizingUserName: 'System Admin',
    status: 'ACTIVE', authorizingFrom: '2025-09-05T11:20:00Z', authorizingTo: null,
  },
  {
    // Loomis — ISV (Partner Portal). Role-scoped (NORMAL) access.
    id: 'eur-u2-c006', entityId: 'c-006', userId: 'u-2',
    authorizingType: 'NORMAL', roleIds: ['r-operator'],
    authorizingTimestamp: '2026-01-15T09:48:00Z',
    authorizingUserId: 'u-c006-1', authorizingUserName: 'Eli Wagner',
    status: 'ACTIVE', authorizingFrom: '2026-01-15T09:48:00Z', authorizingTo: null,
  },
  {
    // Wildflower Goods — ISO-PILOT (Partner Portal). Pilot sponsor admin.
    id: 'eur-u2-c015', entityId: 'c-015', userId: 'u-2',
    authorizingType: 'ADMIN', roleIds: [],
    authorizingTimestamp: '2026-04-10T10:02:00Z',
    authorizingUserId: 'u-1', authorizingUserName: 'System Admin',
    status: 'ACTIVE', authorizingFrom: '2026-04-10T10:02:00Z', authorizingTo: null,
  },
  {
    // Bluefin Market — MERCHANT (Merchant Portal). Role-scoped access.
    id: 'eur-u2-c007', entityId: 'c-007', userId: 'u-2',
    authorizingType: 'NORMAL', roleIds: ['r-operator'],
    authorizingTimestamp: '2026-02-20T14:30:00Z',
    authorizingUserId: 'u-c001-1', authorizingUserName: 'Sarah Chen',
    status: 'ACTIVE', authorizingFrom: '2026-02-20T14:30:00Z', authorizingTo: null,
  },
  {
    // Coastline — ISO (Partner Portal), contract SUSPENDED → access LOCKED.
    id: 'eur-u2-c005', entityId: 'c-005', userId: 'u-2',
    authorizingType: 'ADMIN', roleIds: [],
    authorizingTimestamp: '2024-12-10T13:18:00Z',
    authorizingUserId: 'u-1', authorizingUserName: 'System Admin',
    status: 'LOCKED', authorizingFrom: '2024-12-10T13:18:00Z', authorizingTo: null,
  },
...
//   expiresAt (createdAt + PASSWORD_POLICY.inviteTokenDays = 7 days),
//   status (revised model: explicit enum PENDING / CONSUMED / EXPIRED),
...
Object.assign(window, {
  COUNTRIES, COUNTRY_BY_CODE,
  CONTRACT_TYPES, CONTRACT_STATUSES, CONTRACT_EVENT_TYPES,
  PILOT_CONTRACT_TYPES, isPilotContractType,
  DELEGATION_SCOPES, PASSWORD_POLICY,
  SEED_ENTITIES,
