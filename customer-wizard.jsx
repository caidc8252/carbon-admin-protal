...
const Step2 = ({ contracts, toggle, entitlements, setEntitlements, focusedKind, setFocusedKind, errorsByKind, terms, setTerms }) => (
  <div className="tds-card">
    ...
    <div className="tds-card__body" style={{ padding: 16 }} data-comment-anchor="cbaab1ba9d-div-73-5">
      ...
      />
      {contracts.length > 0 && (
        <div style={{ marginTop: 16, padding: '12px 14px', background: 'var(--color-bg-3)', borderRadius: 8, border: '1px solid var(--color-border-subtle)' }}>
...
<div className="tds-card" style={{ textAlign: 'center' }}>
    <div className="tds-card__body" style={{ padding: '40px 32px' }}>
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 999, background: 'var(--color-bg-3)', border: '1px solid var(--color-border-subtle)', fontSize: 12, fontWeight: 500, color: 'var(--color-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>
...
const CustomerWizard = ({ onCancel, onComplete }) => {
  ...
  const toggleContract = (k) => {
    setContracts((c) => {
      ...
      setTerms((t) => ({
        ...t,
        [k]: t[k] || { effectiveFrom: window.todayDate(data.timezone), effectiveTo: null },
      }));
      return [...c, k];
    });
    setFocusedKind(k);
      ...
      <div className={`wizard-grid ${step === 2 ? 'wizard-grid--wide' : ''}`}>
        ...
        <aside className="wizard-aside">
          ...
          <dl>
            ...
            <dt>Address</dt>   <dd style={{ fontSize: 12 }}>{data.address || <span className="muted">—</span>}</dd>
            ...
            <dt>Email</dt>     <dd style={{ fontSize: 12 }}>{data.email || <span className="muted">—</span>}</dd>
