...
function resolveSubscription(sub) {
  ...
  // ─── Lifecycle (subscriber view) ─────────────────────────────
  // Three-state derivation, in priority order:
  //   "publisher-unpublished" — ISV took the app OR the currently subscribed
  //     version off the public pool. ISO cannot re-publish; only the
  //     publisher can. The ISO's snapshot keeps running on terminals it is
  //     already installed on, no new pushes can land.
  //   "iso-unpublished"       — ISO chose to take the app off THEIR own
  //     App Store (sub.isoUnpublished). Independent of publisher state.
  //     ISO can re-publish at any time.
  //   "live"                  — normal state.
  const versionUnpublished = subscribedVersion?.status === "unpublished";
  let lifecycle = "live";
  if (app.status === "unpublished" || versionUnpublished) lifecycle = "publisher-unpublished";
  else if (sub.isoUnpublished) lifecycle = "iso-unpublished";
  return { ...sub, app, subscribedVersion, latestVersion, isOutdated, publisherName,
    lifecycle, versionUnpublished };
}
