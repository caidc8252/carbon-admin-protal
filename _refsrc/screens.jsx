...
function AppSubscribers({ app }) {
  ...
  const filtered = useMemoS(() => rows.filter((r) => {
    if (versionFilter === "any") {/* keep */} else
    if (versionFilter === "none") {if (r.version !== null) return false;} else
    {if (!r.version || r.version.id !== versionFilter) return false;}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <window.Card title="Subscribers" padding={0}>
        <div className="table-wrap">
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
          <thead>
            <tr>
