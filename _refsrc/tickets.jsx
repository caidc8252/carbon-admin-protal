...
const TICKET_STATUS = {
  ...
  "iso-working": { label: "Working", tone: "info", short: "ISO" },
  "npt-working": { label: "Working", tone: "warning", short: "NPT" },
...
// which team holds it (used for the All view's Assignee column and to
// tone the pill differently), but the visible label is just "Working"
// in both cases and the filter UI surfaces only one Working choice.
...
function DPCandidateRow({ device, normSnQ, active, onHover, onClick }) {
  const merchant = window.findMerchantById?.(device.merchantId);
  const store = merchant?.stores?.find((s) => s.id === device.storeId);
  const isOnline = device.state === "active";
...
function NewTicketScreen({ navigate, presetSn }) {
  ...
  // 'manual'   — user has typed in the Title field, AI suggestions disabled
  ...
  // Race-guard: only apply the latest async result. Bumped on every
  // debounce-triggered request; the resolver checks it still matches
  // before writing back into state.
  ...
  // is too short to be worth summarising.
  const TITLE_MAX = 30;
  ...
  // Strip any wrapping quotes / trailing punctuation Claude likes to add,
  ...
  useEffectT(() => {
    ...
    const timer = setTimeout(async () => {
      if (titleReqRef.current !== myReq) return;
      // No claude bridge in this build — use deterministic slice.
      ...
      try {
        const prompt =
`Generate a concise support-ticket title (≤3 0 characters, 4–7 words).
...
Description: ${trimmedDesc}`;
  ...
  const submit = () => {
    ...
    const newTicket = {
      id: `T-2026-${nextNum}`,
      source: "manual", type, severity, status: "new",
      deviceSn: sn,
      title: finalTitle,
      description: trimmedDesc,
      attachments: [
      ...
      updatedAt: now,
      timeline: [
      { at: now, actor: "Maya Hassan", kind: "created", note: "Logged by ISO" }],
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <window.PageHeader
        title="New ticket"
        subtitle="Log a customer-reported issue. AI will analyse it and propose a resolution as soon as you submit." />
          ...
          <window.Card padding={20}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              hint={
                ...
                titleLoading ?
                  <span style={{ color: "var(--color-primary-700)" }}>Carbon AI is drafting a summary…</span> :
                titleSource === "ai" ?
                  <span>AI-suggested headline — click to edit.</span> :
                titleSource === "slice" ?
                  <span>Auto-trimmed from description — click to edit.</span> :
                  <span>Auto-filled from the description — click to edit.</span>
              }>
                <TitleField
                  value={title}
                  loading={titleLoading && !titleTouched}
                  source={titleSource}
                  max={TITLE_MAX}
                  onChange={(v) => {
  ...
  const transitions = useMemoT(() => {
    ...
    if (role === "ISO") {
      ...
      if (ticket.status === "iso-working" || ticket.status === "npt-resolved") {
        ...
        out.push({ id: "close-customer", label: "Reply & close", icon: "check", primary: true,
...
function StatusStepper({ status }) {
  // 4 visual steps. Working covers both iso-working and npt-working;
...
function DescriptionCard({ ticket, onPreviewAttachment }) {
  ...
  const hasLogAttachment = hasAttachments && ticket.attachments.some((a) => a.kind === "log");
  return (
    ...
    "Logged by the ISO operator based on a customer report."}>
      ...
      {/* Fault window — a subtle one-line meta row right under the
          description. Same column as the description text so the eye
          picks it up naturally; muted color so it doesn't compete. */}
      {ticket.faultWindow &&
      <FaultWindowInline window={ticket.faultWindow} />
      }
      {hasAttachments &&
      <div style={{
...
// Compute a short human-readable duration between two date strings
// in the seed format (e.g. "May 15, 2026 14:31:30"). Returns null if
// either side fails to parse — the UI then just hides the chip.
function formatFaultWindowDuration(w) {
...
// ─── Log capture context ───────────────────────────────────
// Compact one-liner sitting just above the attachments list. Surfaces
// the fault time range right next to the log files it scopes, without
// taking up vertical real estate. If start and end share a date, the
// end side collapses to time-only so the line fits.
function LogCaptureContext({ window: w, source, hasLog }) {
  const duration = formatFaultWindowDuration(w);
  // Same-day shortening: "May 15, 2026 08:55:00 → 09:02:30".
  const sameDay = w.start && w.end &&
  w.start.split(/\s\d{2}:/)[0] === w.end.split(/\s\d{2}:/)[0];
  const endDisplay = sameDay ?
  (w.end.match(/\d{2}:\d{2}(:\d{2})?$/) || [w.end])[0] :
  w.end;
  return (
    <div style={{
      marginTop: 10,
...
// Alias kept for the old import name in DescriptionCard.
const FaultWindowInline = LogCaptureContext;
  ...
  return (
    <window.Card title="Ticket info">
      ...
      <div style={{
        ...
        {/* Fault window used to live here as a Kv row, but it's
            really log-slice metadata — moved to the LogCaptureContext
            block in DescriptionCard so it sits next to the log files
...
// ─── Standalone comment composer (used by the Workbench) ───
...
Object.assign(window, {
  ...
  EscalateDialog, AssignDialog, ReplyDialog, ReplyCloseDialog, CommentDialog,
  GRANT_LABELS, buildSnapshotFor, formatFaultWindowDuration,
