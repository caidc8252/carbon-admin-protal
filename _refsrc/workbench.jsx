...
function WBTopBar({ ticket, device, merchant, connection, role, onClose,
  ...
  return (
    <div style={{
      ...
      {/* Status pill + device chip moved into the right sidebar Issue
          card — the top bar now keeps only identity + actions. */}
      {hasActions &&
...
// ─── Fault window meta line ───────────────────────────────
// Subtle one-liner that surfaces the fault time range right under the
// identity pills in the Issue card. Same visual language as the Ticket
// Detail Description card so the two screens stay consistent. If start
// and end are on the same day, the end side collapses to time-only.
function WBFaultWindowInline({ window: w }) {
  const fmt = window.formatFaultWindowDuration;
  const duration = fmt ? fmt(w) : null;
  const sameDay = w.start && w.end &&
  w.start.split(/\s\d{2}:/)[0] === w.end.split(/\s\d{2}:/)[0];
  // Drop the year on the start side when the window is same-day — it
  // duplicates the Created row below and burns horizontal space.
  // "May 15, 2026 13:30:00" → "May 15 13:30:00"
  const startDisplay = sameDay ? w.start.replace(/,\s*\d{4}\s+/, " ") : w.start;
  ...
  return (
    <div style={{
      ...
      <span className="mono" style={{
        ...
        {startDisplay}
        <span style={{ color: "var(--fg3)", margin: "0 3px" }}>→</span>
        {endDisplay}
      </span>
      {duration &&
...
function WBRightSidebar({ ticket, device, merchant, onOpenTicket, onOpenDevice, onOpenComment, commentTrigger }) {
    ...
    <div style={{
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border-1)" }}>
        ...
        <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
          {/* Status pill — promoted from the top bar so the right-side
              Issue card carries the full identity (status + type +
              severity) of the ticket. */}
          <span style={{
            display: "inline-flex", alignItems: "center", gap: 5,
            padding: "2px 8px", borderRadius: 999,
            background: "var(--bg3)", border: "1px solid var(--border-1)",
            fontSize: 10.5, fontWeight: 500, color: "var(--fg1)",
          }}>
        ...
        {/* Fault window — subtle one-liner sitting right under the
            identity pills, before the description. This is the time
            anchor a debugger needs at-a-glance to align logs/snapshots,
            so it's lifted out of the Kv grid below into the top of the
            card. Same visual language as the Ticket Detail Description
            card. */}
        {ticket.faultWindow &&
        <WBFaultWindowInline window={ticket.faultWindow} />
        }
        <div style={{
        ...
        <div style={{
          ...
          <a href="#" onClick={(e) => { e.preventDefault(); onOpenDevice?.(); }}
            className="mono"
            title="Open device detail"
            style={{ color: "var(--accent)", textDecoration: "underline",
              textUnderlineOffset: 2, cursor: "pointer" }}>{ticket.deviceSn}</a>
          {device && <>
          ...
          {/* Fault window row removed — it now lives as an inline meta
              row at the top of the card, right under the pills. */}
        </div>
