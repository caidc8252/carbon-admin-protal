...
// ─── Reusable: collapsed-by-default Screenshots ───────────
// View pages (app overview, version detail) defer the procedural phone
// screenshots behind a click so the page loads snappy and the section
// stays compact. Manages its own open state.
function CollapsibleScreenshots({ app, version, count = 4, size = "md" }) {
  const [open, setOpen] = useStateS(false);
  if (open) {
    return (
      <div>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
          <window.Button size="sm" ghost icon="x" onClick={() => setOpen(false)}>Hide</window.Button>
        </div>
        <window.Screenshots app={app} version={version} count={count} size={size} />
      </div>);
  }
  return (
    ...
    onMouseLeave={(e) => e.currentTarget.style.background = "var(--color-bg-3)"}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11.5, color: "var(--color-text-tertiary)", marginTop: 2 }}>
          Click to load the store imagery.
        </div>
      </div>
...
function AppsListScreen({ mode = "publish", navigate, openNewApp, openPublishWizard }) {
  ...
  // App Publish and App Store each get the Merchants-style condition + list
  // area. (PoolAppsList — the legacy KPI-tile-driven browser — is retained
  // as a fallback for any other modes.)
  if (mode === "publish") {
    return <PublishAppsList
      tenant={tenant}
      navigate={navigate}
      openNewApp={openNewApp}
      openPublishWizard={openPublishWizard} />;
  }
  if (mode === "appStore" || mode === "store") {
...
// ─── Applied-filter chip (removable) — mirrors the Merchants condition area ──
function AppAppliedChip({ label, value, onRemove }) {
  return (
    <span style={{
...
// ─── Export menu item (mirrors the Merchants condition area) ────────────────
function AppExportMenuItem({ icon, title, subtitle, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
...
// ─── CSV helpers (mirror the Merchants export) ──────────────────────────────
function appCsvCell(v) {
  const s = v == null ? "" : String(v);
  return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
}
function publishAppsToCsv(items) {
  const headers = ["App", "Package", "Category", "Latest version", "Published", "Status", "Mode", "Versions"];
  const rows = items.map((it) => {
    const a = it.app;
    const latest = (a.versions || [])[0];
    const modeLabel = (window.APP_PUBLISH_MODE || {})[a.publishMode || "public"]?.label || "";
    const statusLabel = (window.APP_STATUS || {})[a.status]?.label || a.status || "";
    return [
      a.name, a.package, a.category,
      latest ? latest.name : "",
  ...
  return [headers, ...rows].map((r) => r.map(appCsvCell).join(",")).join("\r\n");
}
function appDownloadCsv(filename, csv) {
...
// ─── App Publish — Merchants-style condition + list area ────────────────────
// Mirrors MerchantsListScreen: a sticky bg2 condition panel with DRAFT inputs
// that only filter the list on Search (or chip-removal / clear-all), applied
// filters shown as removable chips below a dashed separator, and a list card
// whose header pins beneath the condition area with a paginated table + an
// Export menu (current view vs. all search results).
function PublishAppsList({ tenant, navigate, openNewApp, openPublishWizard }) {
  // Own apps only — App Publish lists what this ISV authors.
  const poolItems = useMemoS(
    () => buildPoolItems(tenant).filter((it) => it.kind === "own"),
    [tenant]);
  const cats = useMemoS(() => [...new Set(poolItems.map((it) => it.app.category))].sort(), [poolItems]);

  // ─── Draft (input) state — live as the operator types/toggles, applied to
  //     the list only on Search.
  const [q, setQ] = useStateS("");
  const [category, setCategory] = useStateS("all");
  const [status, setStatus] = useStateS("any");       // any | published | unpublished
  const [pubMode, setPubMode] = useStateS("any");      // any | public | private

  // ─── Applied state — what the table actually reads.
  const [aQ, setAQ] = useStateS("");
  const [aCategory, setACategory] = useStateS("all");
  const [aStatus, setAStatus] = useStateS("any");
  const [aPubMode, setAPubMode] = useStateS("any");

  // Sticky layout — measure the condition area so the table header can pin
  // directly beneath it.
  const condRef = useRefS(null);
  const [condH, setCondH] = useStateS(0);
  useEffectS(() => {
  ...
  const totals = useMemoS(() => ({
    ...
    published: poolItems.filter((it) => it.app.status === "published").length,
    unpublished: poolItems.filter((it) => it.app.status === "unpublished").length,
    specified: poolItems.filter((it) => (it.app.publishMode || "public") === "private").length
  }), [poolItems]);

  const filtered = useMemoS(() => poolItems.filter((it) => {
    const a = it.app;
    if (aCategory !== "all" && a.category !== aCategory) return false;
    if (aStatus !== "any" && a.status !== aStatus) return false;
    if (aPubMode !== "any" && (a.publishMode || "public") !== aPubMode) return false;
    if (aQ) {
      const needle = aQ.toLowerCase();
      if (!`${a.name} ${a.package}`.toLowerCase().includes(needle)) return false;
    }
    return true;
  }), [poolItems, aQ, aCategory, aStatus, aPubMode]);

  const pager = window.usePaginated(filtered, 10,
  `publish|${aQ}|${aCategory}|${aStatus}|${aPubMode}`);
  ...
  const clearCategory = () => {setCategory("all");setACategory("all");};
  const clearStatus = () => {setStatus("any");setAStatus("any");};
  const clearPubMode = () => {setPubMode("any");setAPubMode("any");};
  const resetAll = () => {
    setQ("");setCategory("all");setStatus("any");setPubMode("any");
    setAQ("");setACategory("all");setAStatus("any");setAPubMode("any");
  };
  ...
  const applyStatus = (val) => {setStatus(val);setAStatus(val);};
  const applyPubMode = (val) => {setPubMode(val);setAPubMode(val);};

  const hasApplied =
  !!aQ || aCategory !== "all" || aStatus !== "any" || aPubMode !== "any";

  const STATUS_LABEL = { published: "Published", unpublished: "Unpublished" };
  const MODE_LABEL = {
    public: (window.APP_PUBLISH_MODE || {}).public?.label || "All ISOs",
    private: (window.APP_PUBLISH_MODE || {}).private?.label || "Specified ISOs"
  };

  // ─── Export ────────────────────────────────────────────
  const exportBtnRef = useRefS(null);
  ...
  const exportVisible = () => {
    setExportOpen(false);
    const csv = publishAppsToCsv(pager.slice);
    const ts = new Date().toISOString().slice(0, 10);
    appDownloadCsv(`apps_page-${pager.page}_${ts}.csv`, csv);
    showExportToast(`Exported ${pager.slice.length} visible app${pager.slice.length === 1 ? "" : "s"}`, "success");
  };
  const exportAll = () => {
    setExportOpen(false);
    if (exporting) return;
    ...
    setTimeout(() => {
      const csv = publishAppsToCsv(filtered);
      const ts = new Date().toISOString().slice(0, 10);
      appDownloadCsv(`apps_query_${ts}.csv`, csv);
      setExporting(false);
      showExportToast(`Exported ${filtered.length} app${filtered.length === 1 ? "" : "s"} from search results`, "success");
    }, 1400);
  };

  const tiles = [
  { key: "total", label: "Total apps", value: totals.total, sub: "in your catalog",
    active: aStatus === "any" && aPubMode === "any" && aCategory === "all",
    onClick: () => resetAll() },
  { key: "published", label: "Published", value: totals.published, sub: "live · open for subscribers", tone: "success",
    active: aStatus === "published",
    onClick: () => applyStatus(aStatus === "published" ? "any" : "published") },
  { key: "unpublished", label: "Unpublished", value: totals.unpublished, sub: "withdrawn · re-publish anytime",
    tone: totals.unpublished > 0 ? "warning" : undefined,
    active: aStatus === "unpublished",
    onClick: () => applyStatus(aStatus === "unpublished" ? "any" : "unpublished") },
  { key: "specified", label: "Specified ISOs", value: totals.specified, sub: "hidden from public pool",
    active: aPubMode === "private",
    onClick: () => applyPubMode(aPubMode === "private" ? "any" : "private") }];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <window.PageHeader
        title="App Publish"
        subtitle="Publish your applications to the TOMS public pool. ISO companies can subscribe to anything you publish here and push it to their merchants."
        actions={
      ...
      <div style={{ flex: 1, overflow: "auto", background: "var(--bg1)" }}>
        {/* KPI strip removed — filtering lives in the condition panel below. */}
        ...
        <div ref={condRef} style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", flexWrap: "wrap" }}>
            ...
            <label className="tds-select tds-select--sm" style={{ flex: "0 0 170px", position: "relative" }}>
              <select value={category} onChange={(e) => setCategory(e.target.value)}
                style={{ appearance: "none", WebkitAppearance: "none", flex: 1, height: "100%", background: "transparent",
                  border: 0, outline: 0, fontSize: 13, color: "var(--fg1)", paddingRight: category !== "all" ? 22 : "var(--space-5)" }}>
                <option value="all">All categories</option>
                {cats.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
              {category !== "all" ?
              <button type="button" title="Reset category"
                onClick={(e) => {e.preventDefault();e.stopPropagation();clearCategory();}}
            ...
            <label className="tds-select tds-select--sm" style={{ flex: "0 0 160px", position: "relative" }}>
              <select value={status} onChange={(e) => setStatus(e.target.value)}
                style={{ appearance: "none", WebkitAppearance: "none", flex: 1, height: "100%", background: "transparent",
                  border: 0, outline: 0, fontSize: 13, color: "var(--fg1)", paddingRight: status !== "any" ? 22 : "var(--space-5)" }}>
            ...
            {/* Publish-mode select */}
            <label className="tds-select tds-select--sm" style={{ flex: "0 0 170px", position: "relative" }}>
              <select value={pubMode} onChange={(e) => setPubMode(e.target.value)}
                style={{ appearance: "none", WebkitAppearance: "none", flex: 1, height: "100%", background: "transparent",
                  border: 0, outline: 0, fontSize: 13, color: "var(--fg1)", paddingRight: pubMode !== "any" ? 22 : "var(--space-5)" }}>
                <option value="any">All modes</option>
                <option value="public">{MODE_LABEL.public}</option>
                <option value="private">{MODE_LABEL.private}</option>
              </select>
              {pubMode !== "any" ?
              <button type="button" title="Reset mode"
                onClick={(e) => {e.preventDefault();e.stopPropagation();clearPubMode();}}
                style={{ display: "grid", placeItems: "center", width: 18, height: 18, borderRadius: "var(--radius-full)", color: "var(--fg3)", marginLeft: -18, zIndex: 2 }}
                onMouseEnter={(e) => {e.currentTarget.style.background = "var(--bg3)";e.currentTarget.style.color = "var(--fg1)";}}
                onMouseLeave={(e) => {e.currentTarget.style.background = "transparent";e.currentTarget.style.color = "var(--fg3)";}}>
                  <window.Ico name="x" size={12} />
          ...
          <div style={{
            ...
            {aQ && <AppAppliedChip label="Search" value={aQ} onRemove={clearQ} />}
            {aCategory !== "all" && <AppAppliedChip label="Category" value={aCategory} onRemove={clearCategory} />}
            {aStatus !== "any" && <AppAppliedChip label="Status" value={STATUS_LABEL[aStatus] || aStatus} onRemove={clearStatus} />}
            {aPubMode !== "any" && <AppAppliedChip label="Mode" value={MODE_LABEL[aPubMode] || aPubMode} onRemove={clearPubMode} />}
          ...
          <div className="tds-card" style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <window.Ico name="package" size={14} style={{ color: "var(--fg3)" }} />
                <span style={{ fontSize: 13, fontWeight: 500, color: "var(--fg1)" }}>
                  ...
                  <span style={{ color: "var(--fg3)", marginLeft: 6, fontWeight: 400 }}>
                    ...
                    {hasApplied && <> · filtered from <span className="mono num">{poolItems.length}</span></>}
                  </span>
                </span>
              </div>
            ...
            <window.PortalDropdown
              ...
              <div style={{
                ...
                <AppExportMenuItem
                  ...
                  subtitle={`Current page · ${pager.slice.length} app${pager.slice.length === 1 ? "" : "s"}`}
                ...
                <AppExportMenuItem
                  ...
                  subtitle={hasMultiplePages ? `All matches · ${filtered.length} apps` : "Available when results span more than one page"}
                  onClick={exportAll}
                  disabled={filtered.length === 0 || !hasMultiplePages} />
              </div>
            </window.PortalDropdown>

            {/* Table — header pinned just below the condition area */}
            <table className="tds-table num" style={{ width: "100%" }}>
              <thead style={{ position: "sticky", top: condH, zIndex: 10, background: "var(--bg2)" }}>
                <tr>
                  <th style={{ minWidth: 220 }}>App</th>
                  <th>Category</th>
              ...
              <tbody>
                ...
                {pager.slice.map((it) =>
                <PoolRow key={it.key} item={it} tenant={tenant} navigate={navigate} onPull={() => {}} showPublisher={false} fromEntry="appPublish" />
                )}
              </tbody>
            </table>
...
// ─── App Store — Merchants-style condition + list area ──────────────────────
// Mirrors PublishAppsList: sticky bg2 condition panel with DRAFT inputs that
// only filter on Search (or chip-removal / clear-all), applied-filter chips,
// list card with toolbar + pinned table header, Export menu, pagination.
// Lists both subscribed apps AND own apps the tenant has published — the
// App Store is the unified library of every app the operator can install
// on merchants.
function StoreAppsList({ tenant, navigate, openNewApp, openPublishWizard }) {
  const poolItems = useMemoS(() => buildPoolItems(tenant), [tenant]);
  const cats = useMemoS(() => [...new Set(poolItems.map((it) => it.app.category))].sort(), [poolItems]);
  const isPureISV = !tenant.contracts.includes("ISO");

  // ─── Draft (input) state — live as the operator types/toggles, applied
  //     only on Search (or chip-removal / clear-all).
  const [q, setQ] = useStateS("");
  const [category, setCategory] = useStateS("all");
  const [kind, setKind] = useStateS("any");        // any | own | subscribed
  const [status, setStatus] = useStateS("any");    // any | on-latest | update | iso-unpub | pub-unpub

  // ─── Applied state — drives the table.
  const [aQ, setAQ] = useStateS("");
  const [aCategory, setACategory] = useStateS("all");
  const [aKind, setAKind] = useStateS("any");
  const [aStatus, setAStatus] = useStateS("any");
  ...
  // ─── Helper — derive a single status token per row (kind-aware).
  const statusOf = (it) => {
    if (it.kind === "own") return "self";
    const lc = it.subscription?.lifecycle || "live";
    if (lc === "publisher-unpublished") return "pub-unpub";
    if (lc === "iso-unpublished") return "iso-unpub";
    return it.subscription?.isOutdated ? "update" : "on-latest";
  };

  const totals = useMemoS(() => ({
    total: poolItems.length,
    own: poolItems.filter((it) => it.kind === "own").length,
    subscribed: poolItems.filter((it) => it.kind === "subscribed").length,
    updates: poolItems.filter((it) => it.kind === "subscribed" && it.subscription.isOutdated &&
    it.subscription.lifecycle === "live").length
  }), [poolItems]);

  const filtered = useMemoS(() => poolItems.filter((it) => {
    if (aKind === "own" && it.kind !== "own") return false;
    if (aKind === "subscribed" && it.kind !== "subscribed") return false;
    if (aCategory !== "all" && it.app.category !== aCategory) return false;
    if (aStatus !== "any" && statusOf(it) !== aStatus) return false;
    if (aQ) {
      const needle = aQ.toLowerCase();
    ...
    return true;
  }), [poolItems, aQ, aCategory, aKind, aStatus]);

  const pager = window.usePaginated(filtered, 10,
  `store|${aQ}|${aCategory}|${aKind}|${aStatus}`);
  const hasMultiplePages = filtered.length > pager.pageSize;

  const runSearch = () => {
    setAQ(q.trim());setACategory(category);setAKind(kind);setAStatus(status);
  };
  const clearQ = () => {setQ("");setAQ("");};
  const clearCategory = () => {setCategory("all");setACategory("all");};
  const clearKind = () => {setKind("any");setAKind("any");};
  const clearStatus = () => {setStatus("any");setAStatus("any");};
  const resetAll = () => {
    setQ("");setCategory("all");setKind("any");setStatus("any");
    setAQ("");setACategory("all");setAKind("any");setAStatus("any");
  };

  // KPI tiles double as quick filters.
  const applyKind = (val) => {setKind(val);setAKind(val);};
  const applyStatus = (val) => {setStatus(val);setAStatus(val);};

  const hasApplied = !!aQ || aCategory !== "all" || aKind !== "any" || aStatus !== "any";

  const KIND_LABEL = { own: "Self-uploaded", subscribed: "Subscribed" };
  const STATUS_LABEL = {
    "on-latest": "On latest",
    "update": "Update available",
    "iso-unpub": "Unpublished (by you)",
    "pub-unpub": "Unpublished by publisher",
    "self": "Self-uploaded"
  };
  ...
  const toCsv = (items) => {
    const headers = ["App", "Publisher", "Category", "Subscribed version", "Latest version", "Status"];
    const rows = items.map((it) => {
      const a = it.app;
      const publisher = (window.TENANT_NAMES || {})[it.publisherId] || "—";
      const sub = it.subscription;
      return [
        a.name, publisher, a.category,
        sub?.subscribedVersion?.name || "",
        sub?.latestVersion?.name || ((a.versions || [])[0]?.name || ""),
  ...
  const exportVisible = () => {
    ...
    const ts = new Date().toISOString().slice(0, 10);
    appDownloadCsv(`store_page-${pager.page}_${ts}.csv`, csv);
    showExportToast(`Exported ${pager.slice.length} visible app${pager.slice.length === 1 ? "" : "s"}`, "success");
  };
  const exportAll = () => {
    setExportOpen(false);
    ...
    setTimeout(() => {
      ...
      const ts = new Date().toISOString().slice(0, 10);
      appDownloadCsv(`store_query_${ts}.csv`, csv);
      setExporting(false);
      showExportToast(`Exported ${filtered.length} app${filtered.length === 1 ? "" : "s"} from search results`, "success");
    }, 1400);
  };

  // Pull / approval routing (mirrors PoolAppsList).
  ...
  const tiles = [
  { key: "total", label: "All apps", value: totals.total, sub: "everything in your store",
    active: aKind === "any" && aStatus === "any" && aCategory === "all",
    onClick: () => resetAll() },
  { key: "own", label: "Self-uploaded", value: totals.own,
    sub: "apps published by you",
    active: aKind === "own",
    onClick: () => applyKind(aKind === "own" ? "any" : "own") },
  { key: "subscribed", label: "Subscribed", value: totals.subscribed,
    sub: "from other publishers",
    active: aKind === "subscribed",
    onClick: () => applyKind(aKind === "subscribed" ? "any" : "subscribed") },
  { key: "updates", label: "Updates pending", value: totals.updates,
    ...
    active: aStatus === "update",
    onClick: () => applyStatus(aStatus === "update" ? "any" : "update") }];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <window.PageHeader
        title="App Store"
        subtitle="Browse and manage apps you've subscribed to from the TOMS public pool. Snapshots are fixed at the version you approved — new ISV releases require manual approval."
        actions={
        <window.Button primary icon="search" onClick={openBrowsePool}>Browse pool</window.Button>
        } />
      ...
      <div style={{ flex: 1, overflow: "auto", background: "var(--bg1)" }}>
        ...
        {/* ─── Condition area — bg2 panel, draft inputs + Search button,
            applied chips below dashed separator. */}
        <div ref={condRef} style={{
          position: "sticky", top: 0, zIndex: 20,
          padding: "var(--space-4) var(--space-6)",
          background: "var(--bg2)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", flexWrap: "wrap" }}>
            <div style={{ flex: "0 0 320px", minWidth: 240 }}>
              <window.Input
                value={q} onChange={(e) => setQ(e.target.value)}
                onKeyDown={(e) => {if (e.key === "Enter") runSearch();}}
                placeholder="Search apps, packages, publishers…"
                prefix={<window.Ico name="search" size={13} />}
                size="sm"
                suffix={
                <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
            ...
            <label className="tds-select tds-select--sm" style={{ flex: "0 0 170px", position: "relative" }}>
              <select value={kind} onChange={(e) => setKind(e.target.value)}
                style={{ appearance: "none", WebkitAppearance: "none", flex: 1, height: "100%", background: "transparent",
                  border: 0, outline: 0, fontSize: 13, color: "var(--fg1)", paddingRight: kind !== "any" ? 22 : "var(--space-5)" }}>
                <option value="any">All sources</option>
                {!isPureISV && <option value="own">Self-uploaded</option>}
                {isPureISV && <option value="own">Self-uploaded</option>}
                <option value="subscribed">Subscribed</option>
              </select>
              {kind !== "any" ?
              <button type="button" title="Reset source"
                onClick={(e) => {e.preventDefault();e.stopPropagation();clearKind();}}
                style={{ display: "grid", placeItems: "center", width: 18, height: 18, borderRadius: "var(--radius-full)", color: "var(--fg3)", marginLeft: -18, zIndex: 2 }}
                onMouseEnter={(e) => {e.currentTarget.style.background = "var(--bg3)";e.currentTarget.style.color = "var(--fg1)";}}
                onMouseLeave={(e) => {e.currentTarget.style.background = "transparent";e.currentTarget.style.color = "var(--fg3)";}}>
                  <window.Ico name="x" size={12} />
            ...
            {/* Status */}
            <label className="tds-select tds-select--sm" style={{ flex: "0 0 200px", position: "relative" }}>
              <select value={status} onChange={(e) => setStatus(e.target.value)}
                style={{ appearance: "none", WebkitAppearance: "none", flex: 1, height: "100%", background: "transparent",
                  border: 0, outline: 0, fontSize: 13, color: "var(--fg1)", paddingRight: status !== "any" ? 22 : "var(--space-5)" }}>
                <option value="any">All statuses</option>
                <option value="on-latest">On latest</option>
                <option value="update">Update available</option>
                <option value="iso-unpub">Unpublished (by you)</option>
                <option value="pub-unpub">Unpublished by publisher</option>
              </select>
              {status !== "any" ?
              <button type="button" title="Reset status"
                onClick={(e) => {e.preventDefault();e.stopPropagation();clearStatus();}}
          ...
          <div style={{
            ...
            {aQ && <AppAppliedChip label="Search" value={aQ} onRemove={clearQ} />}
            {aCategory !== "all" && <AppAppliedChip label="Category" value={aCategory} onRemove={clearCategory} />}
            {aKind !== "any" && <AppAppliedChip label="Source" value={KIND_LABEL[aKind] || aKind} onRemove={clearKind} />}
            {aStatus !== "any" && <AppAppliedChip label="Status" value={STATUS_LABEL[aStatus] || aStatus} onRemove={clearStatus} />}
            <button onClick={resetAll}
              style={{ marginLeft: "var(--space-1)", fontSize: 11.5, color: "var(--fg3)", padding: "2px 6px", borderRadius: "var(--radius-sm)" }}
              onMouseEnter={(e) => {e.currentTarget.style.color = "var(--fg1)";}}
        ...
        {/* ─── List area — Merchants-style card */}
        <div style={{ padding: "var(--space-5) var(--space-6)" }}>
          <div className="tds-card" style={{
            display: "flex", flexDirection: "column",
            overflow: "visible",
            ...
            {/* Table — header pinned below condition area */}
            <table className="tds-table num" style={{ width: "100%" }}>
              <thead style={{ position: "sticky", top: condH, zIndex: 10, background: "var(--bg2)" }}>
                <tr>
                  <th style={{ minWidth: 220 }}>App</th>
              ...
              <tbody>
                ...
                {pager.slice.map((it) =>
                <PoolRow key={it.key} item={it} tenant={tenant} navigate={navigate} onPull={openPullWizard} showPublisher={true} fromEntry="appStore" />
                )}
              </tbody>
            </table>
...
function PoolAppsList({ mode, tenant, navigate, openNewApp, openPublishWizard }) {
  ...
  // App Store now uses the Merchants-style StoreAppsList (above); this
  // legacy implementation is kept only for any other entry points that
  // pass a non-recognized mode. The block below preserves the original
  // behavior.
...
function PoolRow({ item, tenant, navigate, onPull, showPublisher, fromEntry }) {
  ...
  // When the upstream publisher unpublishes (app-level OR the subscribed
  // version), the row flips to "Unpublished by publisher" (warning) —
  // only the publisher can re-publish. When the ISO themselves take the
  // app off their local App Store, the row reads "Unpublished" (neutral)
  // and the ISO can re-publish from the app detail. Either way the
  // Approve-update button is suppressed because no new pushes can land.
  ...
  const lifecycle = !isOwn ? item.subscription?.lifecycle : null;
  const publisherUnpublished = lifecycle === "publisher-unpublished";
  const isoUnpublished = lifecycle === "iso-unpublished";
  if (isOwn) {
    if (isStoreView) {
  ...
  } else if (publisherUnpublished) {
    statusCell = <window.Pill tone="warning" dot>Unpublished by publisher</window.Pill>;
  } else if (isoUnpublished) {
    statusCell = <window.Pill tone="neutral" dot>Unpublished</window.Pill>;
  } else {
  ...
  return (
    <tr
      ...
      <td style={{ padding: "12px", textAlign: "right" }} onClick={(e) => e.stopPropagation()}>
        {item.kind === "subscribed" && lifecycle === "live" && item.subscription.isOutdated ?
...
function AppDetailScreen({ app, route, navigate, openPublishWizard, openEditApp }) {
  ...
  // Header pills:
  // ─── Status pills (right of title) ─────────────────────────
  // ─── Status pills (right of title) ─────────────────────────
  //   own apps   → 2 pills: lifecycle status + publish mode
  //   subscriber → 1 pill:  derived lifecycle three-state
  const subLifecycle = liveSubscription?.lifecycle || "live";
  const publisherUnpublished = !isOwnApp && subLifecycle === "publisher-unpublished";
  const isoUnpublished = !isOwnApp && subLifecycle === "iso-unpublished";
  const lifecyclePill = isOwnApp ?
  { tone: st.tone, label: st.label } :
  publisherUnpublished ?
  { tone: "warning", label: "Unpublished by publisher" } :
  isoUnpublished ?
  { tone: "neutral", label: "Unpublished" } :
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div style={{ padding: "16px 24px 0", display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 6, paddingTop: 4, flexShrink: 0 }}>
            ...
            <>
                ...
                {liveSubscription?.isOutdated && subLifecycle === "live" &&
              ...
              onClick={() => openPullWizard(liveSubscription.latestVersion)}>
                ...
                {isoUnpublished &&
              <window.Button primary icon="refresh"
              onClick={() => {
                const subs = (window.SUBSCRIBED_APPS || {})[tenant.id] || [];
                const target = subs.find((s) => s.appId === app.id);
                if (target) {
                  target.isoUnpublished = false;
                  delete target.isoUnpublishedAt;
                }
                window.showToast?.(`${app.name} re-published to your App Store`, "success");
              }}>
                    Re-publish
                  </window.Button>
              }
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "20px 24px 28px", background: "var(--color-bg-1)" }}>
        ...
        <AppSettings app={app} navigate={navigate} /> :
    ...
    <PublishToPoolModal app={app} open={republishOpen}
      ...
      onConfirm={() => {
        ...
        if (target) {
          ...
          // Cascade restore: any version that was unpublished AS A SIDE
          // EFFECT of the app-level unpublish (flagged with
          // unpublishedByAppLevel) flips back to published. Versions the
          // publisher had individually unpublished beforehand stay
          // unpublished — those are independent lifecycle decisions.
          let restored = 0;
          (target.versions || []).forEach((v) => {
            if (v.unpublishedByAppLevel && v.status === "unpublished") {
              v.status = "published";
              v.publishedAt = "just now";
              delete v.unpublishedAt;
              delete v.unpublishedByAppLevel;
              restored++;
            }
          });
          target.reviewActivity = [...(target.reviewActivity || []), {
            ...
            note: restored > 0 ?
            `Re-published — ${restored} version${restored === 1 ? "" : "s"} restored.` :
            "Re-published — open again for new subscriptions and version pushes."
          }];
    ...
    <window.ConfirmDialog
        ...
        onConfirm={() => {
          ...
          if (target) {
            ...
            // Cascade to every currently-published version. Each gets a
            // marker so a later app-level re-publish knows which ones to
            // restore (vs. ones the publisher had unpublished by hand).
            let cascaded = 0;
            (target.versions || []).forEach((v) => {
              if (v.status === "published") {
                v.status = "unpublished";
                v.unpublishedAt = "just now";
                v.unpublishedByAppLevel = true;
                cascaded++;
              }
            });
            target.reviewActivity = [...(target.reviewActivity || []), {
              ...
              note: cascaded > 0 ?
              `Withdrawn — ${cascaded} version${cascaded === 1 ? "" : "s"} also taken offline. Subscribers notified by email.` :
              "Withdrawn — subscribers notified by email."
            }];
          }
  ...
  return (
    <div className="app-overview">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
        ...
        {/* Supported device models — declared on the app, sits above the
           per-version screenshots so the targeting context is visible first. */}
        <SupportedModelsCard app={app} />
        ...
        hint={<>from <span className="mono">{latest.name}</span> · captured at upload</>}>
          <CollapsibleScreenshots app={app} version={latest} count={4} size="md" />
        </window.Card>
        }
...
function SupportedModelsCard({ app }) {
  const models = app.devices || [];
  return (
    <window.Card title="Supported device models"
    hint={models.length > 0 ? <><span className="mono num">{models.length}</span> model{models.length === 1 ? "" : "s"} declared</> : null}>
      {models.length > 0 ?
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {models.map((d) => {
          const meta = (window.DEVICE_MODELS || []).find((m) => m.id === d);
          return (
            <span key={d} title={meta?.label || d} style={{
              display: "inline-flex", alignItems: "center", gap: 5,
              ...
              fontSize: 11.5, color: "var(--color-text-secondary)"
                ...
                <span className="mono" style={{ fontWeight: 500, color: "var(--color-text-primary)" }}>{d}</span>
                {meta?.family && meta.family !== d &&
              <span style={{ fontSize: 10, color: "var(--color-text-tertiary)" }}>{meta.family}</span>
              }
              </span>);
...
function SubscribedAppOverview({ app, subscription, navigate, onPull, route }) {
  ...
  // Lifecycle drives banner / subscription-card / hidden CTAs.
  const lifecycle = subscription?.lifecycle || "live";
  const publisherUnpublished = lifecycle === "publisher-unpublished";
  const isoUnpublished = lifecycle === "iso-unpublished";
  const anyUnpublished = publisherUnpublished || isoUnpublished;
  // Distinguish app-level vs version-level publisher unpublish for the
  // banner copy — same UX consequences, different reason.
  const versionUnpublished = subscription?.versionUnpublished && app.status !== "unpublished";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{
        ...
        borderRadius: "var(--radius-md)",
        color: "var(--color-warning-700)", fontSize: 12.5, lineHeight: 1.55
      }}>
          <window.Ico name="alert" size={14} stroke={2.2} style={{ flexShrink: 0, marginTop: 2 }} />
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 600, marginBottom: 2 }}>
              {versionUnpublished ?
            <>Version <span className="mono">{cur?.name}</span> withdrawn by publisher</> :
            <>Withdrawn by publisher</>
            }
            </div>
            <div style={{ color: "var(--color-text-secondary)" }}>
              {versionUnpublished ?
            <>{(window.TENANT_NAMES || {})[app.publisherTenantId] || "The publisher"} has unpublished version <span className="mono">{cur?.name}</span> — the exact snapshot you’re running. The version stays on terminals it’s already installed on, but <b>no new pushes to additional terminals are accepted</b>. Only the publisher can restore it.</> :
            <>{(window.TENANT_NAMES || {})[app.publisherTenantId] || "The publisher"} has unpublished <b>{app.name}</b>. Your current snapshot <span className="mono">{cur?.name}</span> keeps running on terminals it is already installed on, but <b>no new version pushes will arrive</b> and you can’t roll it out to additional terminals. Only the publisher can re-publish this app.</>
            }
            </div>
          </div>
        </div>
      ...
      <div style={{
        ...
        color: "var(--color-text-primary)", fontSize: 12.5, lineHeight: 1.55
          ...
          <window.Ico name="alert" size={14} stroke={2.2} style={{ flexShrink: 0, marginTop: 2, color: "var(--color-text-tertiary)" }} />
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 600, marginBottom: 2 }}>Unpublished in your App Store</div>
            <div style={{ color: "var(--color-text-secondary)" }}>
              You’ve taken <b>{app.name}</b> off your App Store. The current snapshot <span className="mono">{cur?.name}</span> keeps running on terminals it is already installed on, but you can’t roll it out to new terminals until you re-publish. Use the <b>Re-publish</b> action in the header (or in Settings) to restore.
            </div>
          </div>
        </div>
      }
      ...
      <div className="app-overview">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
          ...
          {/* Supported device models — declared on the app, shown above the
             screenshots so subscribers see targeting before the imagery. */}
          <SupportedModelsCard app={app} />

          <window.Card title="Screenshots"
          hint={<>from <span className="mono">{cur?.name}</span> · your current snapshot</>}>
            <CollapsibleScreenshots app={app} version={cur} count={4} size="md" />
          </window.Card>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
          ...
          <window.Card title="Subscription">
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              ...
              value={anyUnpublished ?
              <span style={{ fontWeight: 500, color: publisherUnpublished ? "var(--color-warning-700)" : "var(--color-text-tertiary)" }}>
                  — {publisherUnpublished ? "withdrawn" : "paused locally"}
                </span> :
              subscription?.isOutdated ?
...
function AssignMerchantModal({ tenant, app, excludedMerchantIds, defaultVersion,
  ...
  const toggleAll = () => {
    const next = new Set(picked);
    if (allFilteredPicked) filtered.forEach((m) => next.delete(m.id));else
...
function AppVersions({ app, navigate, openPublishWizard, currentSubscribedId, isOwnApp, onPullVersion, route }) {
  ...
  //   own apps  → any / published / unpublished
  ...
  // App versions have only two states — show the real one.
  const effectiveStatusKey = (v) => v.status;
  ...
  return (
    <div>
      ...
      <window.Card title="Version list"
      padding={0}>
        <div className="table-wrap">
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
          <thead>
            <tr style={{ textAlign: "left" }}>
              {["Version", "Code", "Size", "Uploaded", "Published", "Status", ""].map((h, i) =>
                <th key={i} style={{
          ...
          <tbody>
            ...
            {versionsPager.slice.map((v) => {
                ...
                //   own apps   → Available / Unpublished
                //   external   → Available / Waiting Approval / Unpublished
                ...
                null;
                const effectiveStatusKey = v.status;
...
function AppSubscribers({ app }) {
  ...
  // Filter state — list-kit draft↔applied: ISO name search, status, version.
  const lf = window.useListFilters({ q: "", status: "any", version: "any" });

  // Sticky condition area + table header (list-kit): measure the panel so the
  // list card's toolbar and the table thead pin directly beneath it on scroll.
  const condRef = useRefS(null);
  ...
  const totals = useMemoS(() => {
    ...
    rows.forEach((r) => {
      ...
      if (r.status.label === "Up to date") counts.onLatest++;else
      if (r.status.label === "Pending approval") counts.pending++;else
      if (r.status.label === "Behind") counts.behind++;else
      if (r.status.label === "Unsubscribed") counts.unsubscribed++;
    });
    return counts;
  }, [rows]);

  const { q: aQ, status: aStatus, version: aVersion } = lf.applied;
  const filtered = useMemoS(() => rows.filter((r) => {
    if (aVersion === "any") {/* keep */} else
    if (aVersion === "none") {if (r.version !== null) return false;} else
    {if (!r.version || r.version.id !== aVersion) return false;}
    if (aStatus !== "any") {
      const want = { onLatest: "Up to date", pending: "Pending approval",
        unsubscribed: "Unsubscribed" }[aStatus];
      if (r.status.label !== want) return false;
    }
    if (aQ && !`${r.iso.name} ${r.iso.region}`.toLowerCase().includes(aQ.toLowerCase())) return false;
    return true;
  }), [rows, aQ, aStatus, aVersion]);

  const subsPager = window.usePaginated(filtered, 10,
  `${app.id}|${aQ}|${aVersion}|${aStatus}`);

  // Applied-filter chips (list-kit).
  const SUB_STATUS_LABELS = { onLatest: "Up to date", pending: "Pending approval", unsubscribed: "Unsubscribed" };
  const subVersionLabel = (id) => id === "none" ? "Unsubscribed"
    : (publishedVersions.find((v) => v.id === id) || {}).name || id;
  const subChips = [
    aQ && { key: "q", label: "Search", value: aQ, onRemove: () => lf.clearKey("q") },
    aStatus !== "any" && { key: "status", label: "Status", value: SUB_STATUS_LABELS[aStatus] || aStatus, onRemove: () => lf.clearKey("status") },
    aVersion !== "any" && { key: "version", label: "Version", value: subVersionLabel(aVersion), onRemove: () => lf.clearKey("version") }].
  filter(Boolean);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* Condition area (list-kit) — sticky so it never scrolls off */}
      <window.ListConditionPanel innerRef={condRef} sticky chips={subChips} onClearAll={lf.resetAll}>
        <window.ListSearchInput
          value={lf.draft.q}
          onChange={(v) => lf.setDraft("q", v)}
          onEnter={lf.runSearch}
          onClear={() => lf.clearKey("q")}
          placeholder="Search ISO name or region…"
          width={300} />
        <window.ListFilterSelect
          value={lf.draft.status} defaultValue="any"
          onChange={(v) => lf.setDraft("status", v)}
          onClear={() => lf.clearKey("status")}
          width={170}>
          <option value="any">All statuses</option>
          <option value="onLatest">Up to date</option>
          <option value="pending">Pending approval</option>
          <option value="unsubscribed">Unsubscribed</option>
        </window.ListFilterSelect>
        <window.ListFilterSelect
          value={lf.draft.version} defaultValue="any"
          onChange={(v) => lf.setDraft("version", v)}
          onClear={() => lf.clearKey("version")}
          width={180}>
          <option value="any">All versions</option>
          {publishedVersions.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}
      ...
      <window.ListCard
        ...
        {(theadTop) =>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
          <thead style={{ position: "sticky", top: theadTop, zIndex: 10 }}>
            <tr>
              {[
...
// navigation tight.
function AppSettings({ app, navigate }) {
  const tenant = window.useActiveTenant();
  const isPublished = app.status === "published";
  const [deleteOpen, setDeleteOpen] = useStateS(false);

  // Delete rule: an app is only deletable while it has no uploaded versions.
  // Once even one APK has been uploaded we lock the action — deleting would
  // strand any pulls, deployments, or subscriber snapshots that reference it.
  const versionCount = (app.versions || []).length;
  const canDelete = versionCount === 0;
    ...
    <div className="page-content page-content--narrow" style={{ display: "flex", flexDirection: "column", gap: 16, paddingBottom: dirty ? 76 : 0 }}>
      ...
      <window.Card title="Danger zone">
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "6px 0" }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12.5, fontWeight: 500 }}>Delete this app</div>
            <div style={{ fontSize: 11.5, color: "var(--color-text-tertiary)", marginTop: 2, lineHeight: 1.5 }}>
              {canDelete ?
              <>This app has no uploaded versions yet. Deleting removes the entry from your catalog — nothing else is affected.</> :

              <>Cannot delete: <span className="mono num">{versionCount}</span> version{versionCount === 1 ? "" : "s"} {versionCount === 1 ? "has" : "have"} already been uploaded. Apps with uploaded versions stay in your catalog to keep subscriber snapshots and deployments intact. If you want to take it off the public pool, unpublish it instead.</>
              }
            </div>
          </div>
          <window.Button danger
          disabled={!canDelete}
          title={canDelete ? "Delete this app" : "Delete is disabled — this app has uploaded versions"}
      ...
      <window.ConfirmDialog
        ...
        body={
        <>
            This empty app entry will be removed from your catalog. You can
            create a new one at any time. This action cannot be undone.
          </>
        }
        ...
        onConfirm={() => {
          setDeleteOpen(false);
          const idx = (window.APPS || []).findIndex((a) => a.id === app.id);
          if (idx >= 0) window.APPS.splice(idx, 1);
          window.showToast?.(`${app.name} deleted`, "success");
          if (typeof navigate === "function") navigate({ screen: "appPublish" });
        }} />
...
// ─── App Store availability card (ISO Settings tab) ─────────────────────────
// Surfaces the same local pause/resume action that lives in the version
// detail's More menu, in the more discoverable Settings location.
// `subscription.lifecycle` decides which CTA (or empty state) renders.
function AppStoreAvailabilityCard({ app, subscription }) {
  const tenant = window.useActiveTenant();
  const lifecycle = subscription?.lifecycle || "live";
  const publisherUnpublished = lifecycle === "publisher-unpublished";
  const isoUnpublished = lifecycle === "iso-unpublished";
  ...
  const toggle = (next) => {
    ...
    if (!target) return;
    if (next) {
      target.isoUnpublished = true;
      target.isoUnpublishedAt = "just now";
      window.showToast?.(`${app.name} unpublished from your App Store`, "warning");
    } else {
      target.isoUnpublished = false;
      delete target.isoUnpublishedAt;
      window.showToast?.(`${app.name} re-published to your App Store`, "success");
  ...
  let pill;
  let cta;
  if (publisherUnpublished) {
    pill = <window.Pill tone="warning" size="sm" dot>Unpublished by publisher</window.Pill>;
    bodyText = subscription?.versionUnpublished && app.status !== "unpublished" ?
    <>The publisher withdrew version <span className="mono">{subscription?.subscribedVersion?.name}</span> — your current snapshot. Only the publisher can restore it. You can\u2019t pause or resume this app locally while that\u2019s in effect.</> :
    <>The publisher has withdrawn <b>{app.name}</b>. Only the publisher can re-publish it. You can\u2019t pause or resume this app locally while that\u2019s in effect.</>;
    cta = <window.Button danger disabled title="Publisher has unpublished this app">Unpublish</window.Button>;
  } else if (isoUnpublished) {
    pill = <window.Pill tone="neutral" size="sm" dot>Unpublished in your App Store</window.Pill>;
    bodyText = <>You took <b>{app.name}</b> off your App Store on <b>{subscription.isoUnpublishedAt || "—"}</b>. Existing snapshots keep running on terminals, but you can\u2019t roll out to new ones until you re-publish.</>;
    cta = <window.Button primary icon="refresh" onClick={() => toggle(false)}>Re-publish</window.Button>;
  } else {
    pill = <window.Pill tone="success" size="sm" dot>Live in your App Store</window.Pill>;
    bodyText = <>This app is currently live in your App Store. You can pause it here at any time — existing installs keep running; new rollouts are blocked until you re-publish.</>;
    cta = <window.Button danger icon="alert" onClick={() => toggle(true)}>Unpublish</window.Button>;
  }

  return (
    <window.Card title="App Store availability"
    hint="Controls whether this app appears in YOUR App Store. Independent of the publisher's lifecycle.">
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div>{pill}</div>
...
function SubscriptionSettings({ app, subscription, onUnsubscribe }) {
  ...
  return (
    <div className="page-content page-content--narrow" style={{ display: "flex", flexDirection: "column", gap: 16, paddingBottom: dirty ? 76 : 0 }}>
      ...
      {/* App Store availability ─ ISO's local pause / resume of this app.
          Mirrors the action in the version detail's More menu so operators
          have a discoverable entry point in Settings. Disabled while the
          publisher has unpublished (app- or version-level) — only the
          publisher can restore in that case. */}
      <AppStoreAvailabilityCard app={app} subscription={subscription} />
      ...
      <div style={{ background: "var(--color-bg-2)", borderBottom: "1px solid var(--color-border-subtle)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
            ...
            {/* Publisher (ISV) Re-publish — restores a withdrawn version
                 by mutating the GLOBAL v.status. Hidden when the whole app
                 is unpublished — the operator must re-publish at the app
                 level first, which cascades-restore every version that was
                 taken offline by that app-level action. Hidden for
                 subscribers, who must use their own subscription-level
                 Re-publish (see below) which only flips their local
                 isoUnpublished flag. */}
            {isOwnApp && version.status === "unpublished" && app.status !== "unpublished" &&
            ...
            onClick={() => {
              ...
              if (tVer) {
                ...
                // Clear cascade marker if this version was offlined by an
                // app-level unpublish — re-publishing it individually
                // promotes it out of the cascaded set.
                delete tVer.unpublishedByAppLevel;
              }
              if (tApp) {
            ...
            {/* Subscriber (ISO) Re-publish — only when THEY took it off
                 their own App Store (subscription.isoUnpublished) AND this
                 version is the active snapshot. Toggles the local flag;
                 never touches global v.status. */}
            {!isOwnApp && isCurrentSnapshot && subscriptionRaw?.isoUnpublished &&
            <window.Button primary icon="refresh"
            onClick={() => {
              const subs = (window.SUBSCRIBED_APPS || {})[tenant.id] || [];
              const target = subs.find((s) => s.appId === app.id);
            ...
            <div style={{ position: "relative" }}>
              ...
              <>
                  ...
                  onClick={() => {
                    ...
                    {/* ISV publisher actions — mutate the global version state
                         (v.status flips for everyone). */}
                  ...
                  <>
                    ...
                    {/* ISO subscriber action — ONLY when this version is the
                         active snapshot. Flips subscription.isoUnpublished
                         locally; never touches the publisher's v.status. */}
                    {!isOwnApp && isCurrentSnapshot && !subscriptionRaw?.isoUnpublished && version.status !== "unpublished" && app.status !== "unpublished" &&
                  <>
                        <div style={{ height: 1, background: "var(--color-border-subtle)", margin: "4px 0" }} />
                        <MenuItem icon="alert" tone="danger"
                    label="Unpublish from my App Store"
                    hint="Pause this app locally · merchants keep running it · re-publish anytime"
                    onClick={() => {
                      setMoreOpen(false);
                      const subs = (window.SUBSCRIBED_APPS || {})[tenant.id] || [];
                      const target = subs.find((s) => s.appId === app.id);
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "20px 24px 28px", background: "var(--color-bg-1)" }}>
        <div className="page-content" style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <window.Card title={
            <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                  <window.Ico name={(counts.critical || 0) + (counts.high || 0) > 0 ? "shield" : "shieldCheck"} size={14}
                style={{ color: (counts.critical || 0) + (counts.high || 0) > 0 ? "var(--error)" :
                  (counts.medium || 0) > 0 ? "var(--info)" : "var(--success)" }} /> Vulnerability scan
                </span>
            }
            ...
            <window.Button size="sm" iconRight="arrowR" onClick={() => setFindingsOpen(true)}>
                ...
                <>
                  <div style={{ marginBottom: 8 }}>
                    <window.SeverityBar counts={counts} height={8} />
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                    {["critical", "high", "medium", "low", "info"].map((k) => {
                    const v = counts[k] || 0;
                ...
                <div style={{
                  ...
                  background: "var(--error-bg)", border: "1px solid oklch(58% 0.20 25 / 0.25)",
                  borderRadius: "var(--radius-md)", fontSize: 11.5, color: "var(--color-error-700)", lineHeight: 1.5
                }}>
                      <b>Action required:</b> Critical/high findings present in this build. Resolve or document an exception before publishing.
                    </div>
                }
                </>
              </window.Card>
      ...
      <window.ConfirmDialog
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18, fontSize: 12.5, lineHeight: 1.7, color: "var(--color-text-secondary)" }}>
              ...
              <li>Devices already running this version are <b>not affected</b>.</li>
...
Object.assign(window, {
  ...
  computeTerminalStatus, TerminalEventsModal,
