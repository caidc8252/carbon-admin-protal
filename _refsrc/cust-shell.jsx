...
const I = {
  ...
  battery: <><rect x="2" y="8" width="16" height="9" rx="2"/><path d="M18 11h2v3h-2"/><path d="M5 11v3M8 11v3"/></>,
...
function Sidebar({ route, navigate, collapsed = false }) {
  ...
  // Firmware lives alongside App Store under Manage — both are NPT→ISO
  // publish surfaces. ISO-only: pure-ISV tenants don't manage device fleets.
  if (hasISO) appItems.push({ id: "firmware",   icon: "shield",   label: "Firmware" });
  ...
  const sections = [
    ...
    // Support — customer-facing service tickets + proactive pre-warnings.
    // Both live for ISO tenants who need to triage merchant/device issues.
    ...(hasISO ? [{
      title: "Support", items: [
        { id: "deviceHealth",  icon: "activity", label: "Device Health" },
        { id: "batteryHealth", icon: "battery",  label: "Battery Health" },
        { id: "tickets",       icon: "alert",    label: "Tickets" },
      ],
    }] : []),
    // Develop tooling — ISV-only sandbox so an ISV team can try their
    // builds against sample hardware + sample orders, without depending
    // on a real merchant fleet.
    ...(hasISV ? [{
      title: "Develop", items: [
  ...
  const isFirmwareRoute = window.isFirmwareRoute?.(route.screen) || false;
  const isActive = (id) => {
    ...
    if (id === "firmware")  return isFirmwareRoute;
    if (id === "batteryHealth") return window.isBatteryHealthRoute?.(route.screen) || false;
    return route.screen === id;
  };
...
function TopBar({ crumbs = [], actions, sidebarCollapsed, onToggleSidebar, onBellClick, bellCount }) {
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
        ...
        <button onClick={onBellClick}
          ...
          title={bellCount > 0 ? `${bellCount} device-health warning${bellCount === 1 ? "" : "s"}` : "Device Health"}>
...
function usePaginated(items, defaultSize = 10, resetKey = "") {
  ...
  // totalPages / pageStart / pageEnd feed the standard window.Pagination (pagination.jsx)
  return { slice, page: safePage, pageSize, setPage, setPageSize, total, totalPages: pageCount, pageStart, pageEnd };
}
...
// NOTE: window.Pagination is now owned by the standard component in pagination.jsx
// (loaded after shell.jsx). The local Pagination above is kept only for reference.
