...
function buildPoolInventory(tenant, allowInviteAppIds) {
  ensurePoolPublishers();

  const subscribedIds = new Set(((window.SUBSCRIBED_APPS || {})[tenant.id] || []).map(s => s.appId));
  const allow = allowInviteAppIds || new Set();

  // From the existing catalog: anything published (lifecycle), not owned
  // by this tenant, not already subscribed. Specified-ISO apps (publishMode
  // === "private") are hidden from the public browse list unless an
  // invite token explicitly grants access (allowInviteAppIds).
  const fromCatalog = (window.APPS || [])
    .filter(a => a.status === "published")
    .filter(a => a.publisherTenantId !== tenant.id)
    .filter(a => !subscribedIds.has(a.id))
    .filter(a => (a.publishMode || "public") !== "private" || allow.has(a.id))
    .map(a => ({
      app: a,
...
function BrowsePoolScreen({ onClose, navigate, presetAppId, inviteToken }) {
  ...
  // App-level invites point at the app, not a specific version. Resolve
  // to the latest published version at click time.
  const inviteVersion = useMemoBP(() => {
    if (!inviteApp) return null;
    return (inviteApp.versions || []).find(v => v.current && v.status === "published")
        || (inviteApp.versions || []).find(v => v.status === "published")
        || (inviteApp.versions || [])[0]
        || null;
  }, [inviteApp]);
  const inviteValid = inviteRec && inviteRec.status === "active";

  // For an invite-only context: inject the invited app at the top of
  // inventory (and allow it past the invite-only filter).
  const inventory = useMemoBP(() => {
    const allow = inviteValid && inviteApp ? new Set([inviteApp.id]) : new Set();
    const base = buildPoolInventory(tenant, allow);
  ...
  // Pagination for the left list pane — keep page sizes small since this
  // is a 2-pane browser with limited list height.
  const bpager = window.usePaginated(filtered, 12, `bp|${q}|${category}|${inventory.length}`);
  ...
  const handleSubscribed = (config) => {
    ...
    // Reflect the new subscription on the publisher's side: push the
    // subscribing tenant's id into their app's subscriberIds so the
    // Subscribers tab shows the freshly-added ISO immediately. The chip
    // renderer accepts both ISO_COMPANIES ids and real tenant ids
    // (see AppSubscribers in screens.jsx).
    const publisherApp = (window.APPS || []).find(a => a.id === selected.app.id);
    if (publisherApp) {
      publisherApp.subscriberIds = publisherApp.subscriberIds || [];
      if (!publisherApp.subscriberIds.includes(tenant.id)) {
        publisherApp.subscriberIds.push(tenant.id);
      }
    }
    ...
    window.showToast?.(
      ...
      "success");
    // Per spec: ask whether to roll out now. Yes → Rollout wizard for the
    // just-subscribed version; No → version detail page for that version.
    // If the operator doesn't have rollout permission, skip the prompt and
    // land on the version detail page directly.
    const perms = window.TENANT_PERMISSIONS?.[tenant.id] || {};
    const targetVersionId = selected.latestVersion?.id;
    const fallback = targetVersionId
      ? { screen: "versionDetail", appId: selected.app.id, versionId: targetVersionId, from: "appStore" }
      : { screen: "appDetail", appId: selected.app.id, tab: "overview", from: "appStore" };
    if (perms.canRolloutMerchants && targetVersionId) {
      window.__rolloutPrompt = {
        appId: selected.app.id,
        versionId: targetVersionId,
        fromEntry: "appStore",
        fallback,
        sourceLabel: `Subscribed to ${selected.app.name}`,
      };
      // Defer one tick so the modal opens AFTER navigation context settles.
      setTimeout(() => window.dispatchEvent(new Event("__open_rollout_prompt")), 0);
      // Don't pre-navigate — the user makes the choice in the modal.
      return;
    }
    navigate(fallback);
  };
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
            {filtered.length === 0 ? (
              ...
              <div role="list">
                {bpager.slice.map((it, i) => (
                  <PoolListRow
                    key={it.app.id}
                    item={it}
                    active={it.app.id === (selected?.app.id)}
                    onClick={() => setSelectedId(it.app.id)}
                    last={i === bpager.slice.length - 1} />
                ))}
              </div>
            )}
          </div>
          <window.Pagination
            page={bpager.page} pageSize={bpager.pageSize} total={bpager.total}
            onChange={bpager.setPage} onPageSizeChange={bpager.setPageSize}
            pageSizes={[12, 24, 48]} />
        </div>
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <window.Pill tone="accent" size="sm" dot>Specified ISOs</window.Pill>
