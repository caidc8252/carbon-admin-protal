...
const CustomerList = ({ customers, onOpen, onNew }) => {
  ...
  const filtered = useMemoCL(() => {
    ...
    if (applied.contract !== 'All') rows = rows.filter(c => c.contracts.some(k => k.kind === applied.contract && window.effectiveStatus(k, c.timezone) !== 'TERMINATED'));
    if (applied.status !== 'All') {
      rows = rows.filter(c => {
        ...
        switch (applied.status) {
          ...
          case 'HAS_PILOT':      return c.contracts.some(k => window.effectiveStatus(k, c.timezone) === 'PILOT');
    ...
    <div className="page">
      ...
      <div ref={toolbarRef} className="list-toolbar-wrap">
        ...
        <window.ActiveFilters
          ...
          {applied.contract !== 'All' ? <window.FilterChip label={`Contract: ${applied.contract}`} onRemove={() => { setApplied(a => ({ ...a, contract: 'All' })); setDraft(d => ({ ...d, contract: 'All' })); setPage(1); }}/> : null}
          {applied.status !== 'All' ? <window.FilterChip label={`Status: ${_statusLabel(applied.status)}`} onRemove={() => { setApplied(a => ({ ...a, status: 'All' })); setDraft(d => ({ ...d, status: 'All' })); setPage(1); }}/> : null}
        </window.ActiveFilters>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={c.id} onClick={() => onOpen(c.id)}>
                  ...
                  <td>
                    <div style={{ fontSize: 14 }}>{fmtDate(c.registeredAt)}</div>
                  </td>
                  <td>
                    <div className="badge-row">
                      {liveContracts.length === 0 ? (
                        <span className="muted" style={{ fontSize: 12 }} title="No live contracts">—</span>
