    ...
    <div className="page">
      <div className="page__head">
        <div>
          ...
          <p className="page__sub">The hardware catalog. Each model defines its vendor, OS, configurable attributes and the artwork shown across the admin, customer portal and packing slips.</p>
        </div>
      ...
      <div className="stats" style={{ gridTemplateColumns: 'repeat(4, minmax(0, 1fr))' }}>
        <button type="button" className={`stat is-clickable ${applied.os === 'All' ? 'is-active' : ''}`} onClick={() => applyOsShortcut('All')}>
        ...
        <button type="button" className={`stat is-clickable ${applied.os === 'ANDROID' ? 'is-active' : ''}`} onClick={() => applyOsShortcut('ANDROID')}>
        ...
        <button type="button" className={`stat is-clickable ${applied.os === 'LINUX' ? 'is-active' : ''}`} onClick={() => applyOsShortcut('LINUX')}>
        ...
        <button type="button" className={`stat is-clickable ${applied.os === 'RTOS' ? 'is-active' : ''}`} onClick={() => applyOsShortcut('RTOS')}>
              ...
              return (
                <tr key={m.id} onClick={() => onEdit(m.id)}>
                  ...
                  <td>
                    ...
                    <div className="cust-meta">{m.description}</div>
                  </td>
                  <td>
                    <span className="dm-vendor">{vendorName(m.vendorPartnerId)}</span>
                  </td>
...
const AttributeAdder = ({ available, onAdd }) => {
  ...
  const fullDefault = (def) => {
    if (def.type === 'boolean') return true;
    if (def.type === 'single-enum') return (def.options || [])[0];
    return [...(def.options || [])];
  };
  const add = (def) => {
    const value = fullDefault(def);
  ...
  return (
    <div className="dm-vals">
      <div className="dm-vals__tags">
        ...
        <button type="button" className={`dm-vals__addbtn ${picking ? 'is-open' : ''}`} onClick={() => setPicking((p) => !p)}>
      ...
      {picking && (
        <div className="dm-vals__picker">
          ...
          <div className="dm-cands">
            {remaining.length === 0 && <span className="dm-vals__hint">All presets added — type a new one below.</span>}
            {remaining.map((o) => (
...
const DeviceModelForm = ({ initial, allModels, onCancel, onSave }) => {
  ...
  const canSave = codeTrim && !dupCode;

  const submit = () => {
    if (!canSave) {
      toast({ kind: 'error', title: dupCode ? 'Model code already exists.' : 'Model code is required.' });
      return;
    }
  ...
  return (
    <div className="page">
      <div className="page__head">
        <div>
          <h1 className="page__title">{initial ? `Edit · ${initial.modelCode}` : 'New device model'}</h1>
          <p className="page__sub">Maintain the vendor, OS, configurable attributes and artwork for this hardware model.</p>
        </div>
        <div className="page__actions">
      ...
      <div className="wizard-grid">
          ...
          <div className="info-card">
            ...
            <div className="info-card__body">
              <div className="form-grid form-grid--2">
                ...
                <Field label="Vendor" required hint="Manufacturing partner (厂家). Defaults to Newland.">
                  <Select value={m.vendorPartnerId} onChange={(e) => update('vendorPartnerId', e.target.value)}>
                    {DEVICE_VENDORS.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}
                  </Select>
                </Field>
              </div>
        ...
        <aside className="wizard-aside">
          ...
          <dl>
            ...
            <dt>Attributes</dt>
            <dd>
              {attrSummaryChips(m).length === 0
                ? <span className="muted">None</span>
                : <span className="dm-attrchips">{attrSummaryChips(m).map((c, i) => <span key={i} className="dm-attrchip">{c}</span>)}</span>}
            </dd>
