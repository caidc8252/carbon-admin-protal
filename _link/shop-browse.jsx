            ...
            return (
              <article key={p.id} className={`shop-card shop-card--${p.type === 'DEVICE' ? (p.deviceVariant === 'SAMPLE' ? 'sample' : 'production') : 'other'}`} onClick={() => onOpen(p.id)}>
                <div className="shop-card__img">
                  ...
                  {p.type === 'DEVICE' && p.deviceVariant === 'SAMPLE' && (
                    <div className="shop-card__corner shop-card__corner--sample">Sample</div>
                  )}
                </div>
...
.shop-filters { position: sticky; top: 0; z-index: 20; background: var(--color-bg-1); padding: 6px 0 12px; margin-bottom: 4px; }
...
.cart-pop__chip-tag { font-size: 9.5px; font-weight: 600; padding: 1px 5px; border-radius: 999px; text-transform: uppercase; letter-spacing: 0.04em; line-height: 1.4; }
