...
// ─── Device Model reference + snapshot (引用 vs 快照) ────────────
// A DEVICE product references the admin Device Model entity by id
// (window.SEED_DEVICE_MODELS — strict 7-field model). At bind / publish time
// it copies a SNAPSHOT of the model so later model edits don't silently mutate
// the product. Operators re-sync manually.
//   Snapshot fields  : modelCode + os + attributes
//   Comparison basis : modelCode + os + attributes  (logo / vendor excluded)
const deviceModelById = (id) =>
  (window.SEED_DEVICE_MODELS || []).find((m) => m.id === id) || null;

const buildDeviceModelSnapshot = (model) => {
...
// Order-independent fingerprint of an attributes array, for comparison.
const _attrFingerprint = (attrs) => (attrs || [])
  .map((a) => `${a.key}=${Array.isArray(a.value) ? [...a.value].sort().join('|') : a.value}`)
  .sort()
  .join(';');

// True when the live model has diverged from the product's snapshot (or there
// is no snapshot yet). Only meaningful for DEVICE products with a linked model.
const isModelOutOfSync = (product, model) => {
  if (!product || product.type !== 'DEVICE' || !product.deviceModelId) return false;
  if (!model) return false;
  const snap = product.deviceModelSnapshot;
  if (!snap) return true;
  return snap.modelCode !== model.modelCode
    || snap.os !== model.os
    || _attrFingerprint(snap.attributes) !== _attrFingerprint(model.attributes);
};

// Refresh a product's snapshot from the live model (manual sync action).
const syncProductSnapshot = (product, model) => ({
  ...product,
  deviceModelSnapshot: buildDeviceModelSnapshot(model),
  updatedAt: new Date().toISOString(),
});

// ─── DEVICE product specs bound to model attributes (规格挂钩 Attributes) ──
// Rule A: a DEVICE product's variant axes may only come from the linked model's
// attributes, and values may not exceed what the model declares. We stay
// faithful to the model — no synthetic groupings the model never declared.
//   - Variant-eligible = attribute whose value is an array with ≥2 options
//     (multi-enum / multi-enum-or-other). These can be offered as variant axes,
//     limited to a subset of the declared options.
//   - Everything else (booleans like WiFi / Cellular, single-enum, 1-item
//     arrays) is a FIXED spec — displayed read-only exactly as the model
//     declares it. WiFi and Cellular are shown as separate Yes/No specs; they
//     are NOT merged into an invented "Connectivity" axis, and "WiFi + 4G" is
//     never synthesized.

const modelVariantAttrs = (model) => {
  if (!model) return [];
  const catalog = window.ATTRIBUTE_CATALOG || [];
  return (model.attributes || [])
    .filter((a) => Array.isArray(a.value) && a.value.length >= 2)
    .map((a) => {
      const def = catalog.find((d) => d.key === a.key);
      return { key: a.key, label: def ? def.label : a.key, options: [...a.value] };
    });
};

const modelFixedSpecs = (model) => {
  if (!model) return [];
  const catalog = window.ATTRIBUTE_CATALOG || [];
  return (model.attributes || [])
    .filter((a) => !(Array.isArray(a.value) && a.value.length >= 2))
    .map((a) => {
      const def = catalog.find((d) => d.key === a.key);
      const label = def ? def.label : a.key;
      let display, options;
      if (typeof a.value === 'boolean') {
        display = a.value ? 'Yes' : 'No';
        // Offer only the capability the device actually has — its label is the
        // value shown in the combination (e.g. "WiFi"), not "Yes".
        options = a.value ? [label] : [];
      } else if (Array.isArray(a.value)) {
        display = a.value.join(', ');
        options = [...a.value];
      } else {
        display = String(a.value);
        options = (a.value != null && a.value !== '') ? [String(a.value)] : [];
      }
      return { key: a.key, label, display, options, bool: typeof a.value === 'boolean', combineGroup: def && def.combineGroup ? def.combineGroup : null };
    });
};

// A spec axis is "model-bound" when tagged with the attribute key it came from;
// untagged axes are legacy/custom (shown read-only on DEVICE products).
const isModelBoundAxis = (axis) => !!(axis && typeof axis.attrKey === 'string');
...
// A model-bound variant axis (attrKey tags which model attribute it came from).
// Values must be a subset of what the model declares for that attribute.
const buildModelAxis = (attrKey, name, valueLabels) => ({
  id: newAxisId(), attrKey, key: 'attr:' + attrKey, name,
  values: valueLabels.map((label) => ({ id: newValueId(), label })),
});
...
const buildGridVariants = (productId, axes, basePrice) => {
  ...
  // Capability axes (single-value booleans like WiFi / Cellular) combine as the
  // non-empty SUBSETS of the enabled capabilities, not a forced "all present"
  // row: {WiFi}, {Cellular}, {WiFi + Cellular}. Regular multi-value axes keep a
  // plain cartesian product. The two are then multiplied together.
  const regular = axes.filter((a) => !a.isCapability);
  const caps = axes.filter((a) => a.isCapability);

  const regularCombos = regular.reduce((acc, axis) => {
    const next = [];
    acc.forEach((partial) => {
      axis.values.forEach((val) => { next.push([...partial, { axis, val }]); });
    });
    return next;
  }, [[]]);
  ...
  if (caps.length === 0) {
    ...
    capSubsets = [];
    for (let mask = 1; mask < (1 << caps.length); mask++) {        // 1.. → skip empty subset
      const sel = [];
      for (let i = 0; i < caps.length; i++) {
        if (mask & (1 << i)) sel.push({ axis: caps[i], val: caps[i].values[0] });
      }
      capSubsets.push(sel);
    }
    capSubsets.sort((a, b) => a.length - b.length);              // singles first, combos after
  }

  const combos = [];
  regularCombos.forEach((rc) => capSubsets.forEach((sc) => combos.push([...rc, ...sc])));
...
const _seedProducts = () => {
  ...
  // 1. N950 Sample · LISTED · Storage axis (model-bound) + Integration mode
  {
    const id = 'p-001';
    const axis = buildModelAxis('storage', 'Storage', ['32G', '64G']);
    push({
      id, name: 'N950 Sample',
      ...
      variants: buildVariants(id, axis, 459, {
        '32G': { price: 459, stock: 3 },
        '64G': { price: 499, stock: 2 },
      }),
  ...
  // 2. N950 Production · LISTED · Storage axis (model-bound)
  {
    const id = 'p-002';
    const axis = buildModelAxis('storage', 'Storage', ['32G', '64G']);
    push({
  ...
  // 4. S90 Production · LISTED
  {
    ...
    push({
      ...
      variants: buildVariants(id, axis, 329, {
        '32G': { price: 329 },
        '64G': { price: 379, stockNote: 'Limited stock' },
      }),
      status: 'LISTED', listFrom: ts('2026-04-05'),
      createdAt: ts('2026-04-05'), createdBy: 'sarah@npt',
      updatedAt: ts('2026-04-05'), updatedBy: 'sarah@npt',
  ...
  {
    const id = 'p-010';
    const axis = buildModelAxis('secondScreen', 'Second-screen', ['4-inch', '6-inch']);
    push({
  ...
  const _axisFor = (kind) => {
    ...
    if (kind === 'storage') return buildModelAxis('storage', 'Storage', ['32G', '64G']);
  ...
  // Bind each DEVICE product to a snapshot of its admin Device Model entity.
  // We no longer promote "Connectivity" axes to model-bound: the device model
  // never declares Connectivity (only the WiFi / Cellular booleans), so any
  // such axis stays a plain product-defined variant axis — not a model claim.
  out.forEach((p) => {
    if (p.type === 'DEVICE' && p.deviceModelId) {
      p.deviceModelSnapshot = buildDeviceModelSnapshot(deviceModelById(p.deviceModelId));
    }
  });
...
const blankProduct = () => {
  ...
  const defModelId = (window.DEVICE_MODELS || []).find((m) => DEVICE_MODEL_IDS_ALLOWED.includes(m.id))?.id || null;
  return {
    id, name: '',
    type: 'DEVICE',
    deviceModelId: defModelId,
    deviceModelSnapshot: buildDeviceModelSnapshot(deviceModelById(defModelId)),
...
Object.assign(window, {
  ...
  deviceModelById, buildDeviceModelSnapshot, isModelOutOfSync, syncProductSnapshot,
  modelVariantAttrs, modelFixedSpecs, isModelBoundAxis,
});
