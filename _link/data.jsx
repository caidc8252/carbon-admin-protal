...
function getEntityView(entityId) {
  ...
  const contractRows = (_CONTRACTS_BY_ENTITY[entityId] || []).map(c => {
    ...
    // not a status. The legacy UI keys off `status === 'PILOT'` everywhere
    // (badge tone, countdown banner, Extend / Convert actions), so we
    // re-project a pilot's persisted ACTIVE status to the legacy 'PILOT'
    // status here. The real persisted status stays available as `rawStatus`.
    ...
    const legacyStatus = (isPilot && String(c.status).toUpperCase() === 'ACTIVE')
      ? 'PILOT'
      : c.status;
    return {
      id: c.id,
      ...
      status: legacyStatus,                      // legacy: 'PILOT' for active pilots
